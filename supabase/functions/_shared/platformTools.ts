 ...
 *
 * Re-exports tool definitions and execution from specialized modules:
 * - platformToolDefs: Tool definitions in OpenAI format
 * - platformToolHandlers: Orchestrator router for execution
 */

export { PLATFORM_TOOLS } from "./platformToolDefs.ts";
export { executePlatformTool } from "./platformToolHandlers.ts";
...
export const PLATFORM_TOOLS = [
  ...
  { type: "function", function: { name: "get_holding_pattern", description: "Get contacts in the holding pattern (first_touch_sent/holding).", parameters: { type: "object", properties: { source_type: { type: "string", enum: ["wca", "crm", "prospect", "all"] }, country_code: { type: "string" }, min_days_waiting: { type: "number" }, max_days_waiting: { type: "number" }, limit: { type: "number" } } } } },
