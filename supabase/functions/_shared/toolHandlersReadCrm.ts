...
export function createCrmReadHandlers(supabase: SupabaseClient) {
  async function executeCheckBlacklist(args: Record<string, unknown>) {
    let query = supabase
      ...
      .select("company_name, country, city, total_owed_amount, claims, status, blacklist_no, matched_partner_id");
  ...
  async function executeListReminders(args: Record<string, unknown>, userId?: string) {
    let query = supabase
      .from("reminders")
      .select("id, title, description, due_date, priority, status, partner_id, created_at")
    ...
    let results: Array<Record<string, unknown> & { partner: string }> = (data || []).map(
      (r: Record<string, unknown>) => ({
        ...
        priority: r.priority,
        status: r.status,
        partner: nameMap[r.partner_id as string] || "Sconosciuto",
      }),
    );
  ...
  async function executeSearchContacts(args: Record<string, unknown>, userId?: string) {
    ...
    let query = supabase
      ...
      .select(
        isCount
          ? "id"
          : "id, name, company_name, email, phone, mobile, country, city, origin, lead_status, position, deep_search_at, company_alias, contact_alias, created_at",
  ...
  async function executeSearchProspects(args: Record<string, unknown>, userId?: string) {
    ...
    let query = supabase
      ...
      .select(
        isCount
          ? "id"
          : "id, company_name, city, province, region, codice_ateco, descrizione_ateco, fatturato, dipendenti, email, phone, pec, website, lead_status, partita_iva, forma_giuridica, rating_affidabilita, created_at",
        isCount ? { count: "exact", head: true } : undefined,
      );
    if (userId) query = query.eq("user_id", userId);
    if (args.company_name) query = query.ilike("company_name", `%${escapeLike(args.company_name)}%`);
    ...
    if (args.has_email === true) query = query.not("email", "is", null);
    query = query
      .order("fatturato", { ascending: false, nullsFirst: false })
  ...
  async function executeListActivities(args: Record<string, unknown>, userId?: string) {
    let query = supabase
      .from("activities")
      .select(
        "id, title, description, activity_type, status, priority, due_date, source_type, source_meta, partner_id, created_at, completed_at, email_subject",
      )
    ...
    return {
      ...
      activities: results.map((a: Record<string, unknown>) => ({
        ...a,
  ...
  async function executeSearchBusinessCards(args: Record<string, unknown>, userId?: string) {
    let query = supabase
      .from("business_cards")
      .select(
        "id, company_name, contact_name, email, phone, event_name, met_at, location, match_status, match_confidence, matched_partner_id, matched_contact_id, tags, created_at",
      )
      .order("created_at", { ascending: false })
    ...
    const partnerIds = [
      ...new Set(
        (data || [])
          .filter((c: Record<string, unknown>) => c.matched_partner_id)
          .map((c: Record<string, unknown>) => c.matched_partner_id),
      ),
    ] as string[];
    ...
    if (partnerIds.length > 0) {
      ...
      if (userId) pq = pq.eq("user_id", userId);
      const { data: partners } = await pq;
      for (const p of (partners || []) as Array<{ id: string; company_name: string }>)
        partnerNames[p.id] = p.company_name;
    }
    return {
      count: data?.length || 0,
      cards: (data || []).map((c: Record<string, unknown>) => ({
        ...
        matched_partner: c.matched_partner_id
          ? partnerNames[c.matched_partner_id as string] || c.matched_partner_id
          : null,
        tags: c.tags,
      })),
    };
