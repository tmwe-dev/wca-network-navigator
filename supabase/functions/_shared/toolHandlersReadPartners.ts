...
export function createPartnerReadHandlers(supabase: SupabaseClient) {
  async function executeSearchPartners(args: Record<string, unknown>, userId?: string) {
    ...
    if (args.network_name) {
      let q = supabase
        ...
        .select("partner_id")
        .ilike("network_name", `%${escapeLike(args.network_name)}%`);
      if (userId) q = q.eq("user_id", userId);
      const { data } = await q;
    ...
    let query = supabase
      ...
      .select(
        isCount
          ? "id"
          : "id, company_name, city, country_code, country_name, email, phone, rating, wca_id, website, profile_description, raw_profile_markdown, is_favorite, office_type, has_branches, member_since",
  ...
  async function executeCountryOverview(args: Record<string, unknown>) {
    ...
    else if (sortBy === "missing_emails")
      stats.sort((a, b) => (b.total_partners - b.with_email || 0) - (a.total_partners - a.with_email || 0));
  ...
  async function executeDirectoryStatus(args: Record<string, unknown>) {
    ...
    for (const r of (dirData || []) as Array<{ country_code: string; member_count: number; is_verified: boolean }>)
      ...
      return {
        ...
        status:
          !dir && !db
            ...
            : !dir
              ? "no_directory"
              : (db?.total_partners || 0) >= (dir?.members || 0) && (db?.without_profile || 0) === 0
                ? "completato"
                : "incompleto",
      };
    }
  ...
  async function executePartnerDetail(args: Record<string, unknown>, userId?: string) {
    ...
    const [contactsRes, networksRes, servicesRes, certsRes, socialsRes, blacklistRes, bcaRes, importedRes] =
      await Promise.all([
        applyOwner(
          supabase
        ...
        supabase
          ...
          .select("company_name, total_owed_amount, claims, status")
    ...
    return {
      ...
      profile_summary: partner.profile_description
        ...
        : partner.raw_profile_markdown
          ? String(partner.raw_profile_markdown).substring(0, 2000)
          : null,
      ...
      networks: (networksRes.data || []).map((n: Record<string, unknown>) => ({
        name: n.network_name,
        expires: n.expires,
      })),
      ...
      social_links: (socialsRes.data || []).map((s: { platform: string; url: string }) => ({
        platform: s.platform,
        url: s.url,
      })),
      blacklist_matches: (blacklistRes.data || []).map((b: Record<string, unknown>) => ({
  ...
  async function executeGlobalSummary() {
    ...
    const dirTotal = dirRows.reduce(
      (sum: number, r: Record<string, unknown>) => sum + (Number(r.member_count) || 0),
      0,
    );
    return {
  ...
  async function executePartnersWithoutContacts(args: Record<string, unknown>) {
    let query = supabase
      ...
      .select("wca_id, company_name, city, country_code, retry_count, scraped_at")
      .eq("resolved", false)
