...
export function createEnterpriseHandlers(supabase: SupabaseClient) {
  ...
  async function executeSaveMemory(args: Record<string, unknown>, userId: string) {
    ...
    const { data, error } = await supabase.from("ai_memory").insert({
      user_id: userId, content: String(args.content),
  ...
  async function executeCreateWorkPlan(args: Record<string, unknown>, userId: string) {
    ...
    const { data, error } = await supabase.from("ai_work_plans").insert({
      ...
      steps: steps.map((s: any, i: number) => ({ ...s, index: i, status: "pending" })),
      tags: Array.isArray(args.tags) ? args.tags as string[] : [],
    }).select("id").single();
    if (error) return { error: error.message };
    return { success: true, plan_id: data.id, steps_count: steps.length, message: `Piano "${args.title}" creato con ${steps.length} step.` };
  }
  ...
  async function executeExecutePlanStep(args: Record<string, unknown>, userId: string, authHeader?: string) {
    const { data: plan, error: fetchErr } = await supabase.from("ai_work_plans")
      .select("*").eq("id", args.plan_id).eq("user_id", userId).single();
    if (fetchErr || !plan) return { error: "Piano non trovato" };
    const steps = plan.steps as any[];
    const stepIndex = plan.current_step;
  ...
  async function executeGetActivePlans(userId: string) {
    const { data, error } = await supabase.from("ai_work_plans")
      ...
      .eq("user_id", userId).in("status", ["running", "paused"]).limit(10);
    if (error) return { error: error.message };
    return {
      count: data?.length || 0,
      plans: (data || []).map((p: any) => ({
        ...
        next_step: (p.steps as any[])[p.current_step]?.description || null,
        tags: p.tags,
      })),
    };
  }
  ...
  async function executeSaveAsTemplate(args: Record<string, unknown>, userId: string) {
    const { data: plan } = await supabase.from("ai_work_plans")
      .select("title, description, steps, tags").eq("id", args.plan_id).eq("user_id", userId).single();
    if (!plan) return { error: "Piano non trovato" };
    const { data, error } = await supabase.from("ai_plan_templates").insert({
      user_id: userId, name: args.name ? String(args.name) : plan.title,
      description: args.description ? String(args.description) : plan.description,
      steps_template: (plan.steps as any[]).map((s: any) => ({ description: s.description, tool: s.tool, args_template: s.args })),
      tags: plan.tags || [],
    }).select("id").single();
    if (error) return { error: error.message };
  ...
  async function executeSaveKbRule(args: Record<string, unknown>, userId: string) {
    ...
    const { data, error } = await supabase.from("kb_entries").insert({
      user_id: userId, title, content, category, tags, priority, chapter: chapter || "", is_active: true,
    }).select("id").single();
    if (error) return { error: error.message };
  ...
  async function executeSaveOperativePrompt(args: Record<string, unknown>, userId: string) {
    ...
    const { data, error } = await supabase.from("operative_prompts").insert({
      user_id: userId, name, objective, procedure, criteria, priority, is_active: true,
    }).select("id").single();
    if (error) return { error: error.message };
  ...
  async function executeListWorkflows(args: Record<string, unknown>, userId: string) {
    ...
    if (partnerId && !templatesOnly) {
      const { data: state } = await supabase.from("partner_workflow_state")
        ...
        .eq("user_id", userId).eq("partner_id", partnerId).order("started_at", { ascending: false });
    ...
    const { data: workflows } = await supabase.from("commercial_workflows")
      ...
      .or(`user_id.eq.${userId},is_template.eq.true`).eq("is_active", true)
  ...
  async function executeStartWorkflow(args: Record<string, unknown>, userId: string) {
    ...
    const { data: wf } = await supabase.from("commercial_workflows")
      .select("id, name, gates").or(`user_id.eq.${userId},is_template.eq.true`)
    ...
    await supabase.from("partner_workflow_state").update({ status: "paused" })
      .eq("user_id", userId).eq("partner_id", partnerId).eq("status", "active");
    const { data: state, error } = await supabase.from("partner_workflow_state").insert({
      ...
      workflow_id: (wf as any).id, current_gate: 0, status: "active",
    ...
    return {
      success: true, state_id: state?.id, workflow_name: (wf as any).name,
  ...
  async function executeAdvanceWorkflowGate(args: Record<string, unknown>, userId: string) {
    ...
    const { data: state } = await supabase.from("partner_workflow_state")
      ...
      .eq("user_id", userId).eq("partner_id", partnerId).eq("status", "active").maybeSingle();
    if (!state) return { error: "Nessun workflow attivo per questo partner" };
    const gates = Array.isArray((state as any).commercial_workflows?.gates) ? (state as any).commercial_workflows.gates : [];
    ...
    const { error } = await supabase.from("partner_workflow_state").update(update).eq("id", (state as any).id);
    if (error) return { error: error.message };
    const targetGate = gates[newGate] || {};
    return {
      success: true, workflow_name: (state as any).commercial_workflows?.name,
  ...
  async function executeApplyPlaybook(args: Record<string, unknown>, userId: string) {
    ...
    const { data: pb } = await supabase.from("commercial_playbooks").select("*")
      .or(`user_id.eq.${userId},is_template.eq.true`).eq("code", code).eq("is_active", true).maybeSingle();
    ...
    return {
      success: true,
      playbook: {
        code: (pb as any).code, name: (pb as any).name, description: (pb as any).description,
        workflow_code: (pb as any).workflow_code, prompt_template: (pb as any).prompt_template,
        suggested_actions: (pb as any).suggested_actions,
      },
      kb_loaded: kbContext.length, kb_entries: kbContext,
      message: `Playbook "${(pb as any).name}" attivato.`,
    };
  }
