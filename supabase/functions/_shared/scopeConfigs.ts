...
const CONTACTS_EXTRA_TOOLS = [
  {
    ...
    function: {
      ...
      description: "Advanced search imported_contacts with full filters.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Count contacts matching filters.",
      parameters: {
        type: "object",
        properties: {
          company_name: { type: "string" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get aggregated statistics by country/origin/status.",
      parameters: {
        type: "object",
        properties: { group_by: { type: "string", enum: ["country", "origin", "status"] }, limit: { type: "number" } },
        additionalProperties: false,
      },
    },
  },
...
const IMPORT_EXTRA_TOOLS = [
  {
    ...
    function: {
      name: "list_imports",
      description: "Lista import recenti.",
      parameters: {
        type: "object",
        properties: {
          limit: { type: "number" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Dettaglio di un import.",
      parameters: {
        type: "object",
        properties: { import_log_id: { type: "string" } },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Cerca contatti importati.",
      parameters: {
        type: "object",
        properties: {
          import_log_id: { type: "string" },
          is_transferred: { type: "boolean" },
          has_email: { type: "boolean" },
          has_phone: { type: "boolean" },
          company_name: { type: "string" },
          country: { type: "string" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Lista errori di import.",
      parameters: {
        type: "object",
        properties: {
          import_log_id: { type: "string" },
          status: { type: "string", enum: ["pending", "corrected", "dismissed"] },
          limit: { type: "number" },
        },
        required: ["import_log_id"],
        additionalProperties: false,
  ...
  },
  {
    ...
    function: {
      ...
      description: "Statistiche aggregate import.",
      parameters: { type: "object", properties: { import_log_id: { type: "string" } }, additionalProperties: false },
    },
  },
];
...
async function contactsToolHandler(
  name: string,
  args: Record<string, unknown>,
  supabase: ScopedSupabase,
  ...
  switch (name) {
    case "search_contacts_advanced": {
      ...
      const cols = idsOnly
        ? "id"
        : "id, company_name, name, email, phone, mobile, country, city, origin, lead_status, interaction_count, position, created_at";
...
async function importToolHandler(
  name: string,
  args: Record<string, unknown>,
  supabase: ScopedSupabase,
): Promise<unknown | null> {
  switch (name) {
    ...
    case "search_imported_contacts": {
      let q = supabase
        .from("imported_contacts")
        .select("id, company_name, name, email, country, city, is_selected, is_transferred");
      if (args.import_log_id) q = q.eq("import_log_id", args.import_log_id);
    ...
    case "get_import_stats": {
      if (args.import_log_id) {
        ...
        } = { total: data?.length || 0, transferred: 0, selected: 0, with_email: 0, with_phone: 0, by_country: {} };
    ...
    case "cockpit":
      return {
        ...
        buildPrompt: (body, basePrompt) => {
          ...
          const _contactSummary = contacts
            .map((c) => {
        ...
        postProcess: (content) => {
          try {
            ...
            try {
              ...
              return {
                actions: [],
                message: content.replace(/```[\s\S]*?```/g, "").trim() || "Non ho capito il comando.",
              };
            }
          }
        },
...
In entrambe le modalità: NESSUNA modifica viene applicata senza approvazione esplicita dell'utente.`,
    ...
    case "deep-search":
      return {
        systemPrompt:
          "Sei un assistente di ricerca approfondita. Usa i tool di search e enrichment per trovare informazioni dettagliate sui partner WCA. Rispondi sempre in italiano, conciso e basato su dati reali.",
        tools: PLATFORM_TOOLS,
    ...
    case "chat":
      return {
        systemPrompt:
          "Sei un assistente conversazionale per agenti autonomi. Rispondi in modo conciso e operativo, in italiano. Usa i tool quando servono dati reali dal database.",
        tools: PLATFORM_TOOLS,
    ...
    case "mission-builder":
      return {
        systemPrompt:
          "Sei il configuratore di missioni outreach. Guida l'utente nella creazione di una nuova missione, una domanda alla volta. Rispondi in italiano, sii sintetico e propositivo.",
        tools: [],
        temperature: 0.5,
