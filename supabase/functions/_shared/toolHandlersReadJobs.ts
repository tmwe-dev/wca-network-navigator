  ...
  async function executeCheckJobStatus(args: Record<string, unknown>, userId?: string) {
    ...
    if (args.job_id) {
      let jq = supabase
        .from("download_jobs")
        .select(
          "id, country_code, country_name, status, job_type, current_index, total_count, contacts_found_count, contacts_missing_count, created_at, updated_at, completed_at, last_processed_company, error_message, network_name",
        )
        .eq("id", args.job_id);
    ...
    let activeQ = supabase
      .from("download_jobs")
      .select(
        "id, country_name, country_code, status, current_index, total_count, job_type, last_processed_company, error_message, created_at",
      )
    ...
    let recentQ = supabase
      .from("download_jobs")
      .select(
        "id, country_name, country_code, status, current_index, total_count, contacts_found_count, contacts_missing_count, completed_at, error_message",
      )
