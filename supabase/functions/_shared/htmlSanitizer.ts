/**
 * HTML Sanitizer per edge functions Deno.
 * Specchio funzionale di src/lib/security/htmlSanitizer.ts
 * (mantenuto duplicato perché Deno edge non può importare src/).
 *
...
function sanitizeStyle(style: string): string {
  return style
    ...
    .map((decl) => {
      ...
      if (/url\s*\(/i.test(value) && !/url\s*\(\s*['"]?https?:/i.test(value)) return null;
...
export function sanitizeHtml(html: string): string {
  ...
  let out = html.replace(/<(script|style|iframe|object|embed|form|noscript|svg|math)\b[^>]*>[\s\S]*?<\/\1>/gi, "");
  out = out.replace(/<(script|style|iframe|object|embed|form|noscript|link|meta|base)\b[^>]*\/?>/gi, "");
  out = out.replace(/<!--[\s\S]*?-->/g, "");

  out = out.replace(/<(\/?)([a-zA-Z][a-zA-Z0-9]*)\b([^>]*)>/g, (_match, slash, tag, attrs) => {
