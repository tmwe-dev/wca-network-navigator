...
const tools = [
  {
    ...
    function: {
      ...
      description: "Search imported_contacts with filters. Returns matching contacts (max 200). Use to find contacts, get IDs for selection, or answer questions about data.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "count_contacts",
      description: "Count contacts matching filters. Same filters as search_contacts but returns only count.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Update lead_status for a list of contact IDs. ALWAYS confirm with user before calling this.",
      parameters: {
        type: "object",
        properties: {
          contact_ids: { type: "array", items: { type: "string" }, description: "UUIDs of contacts to update" },
          status: { type: "string", enum: ["new", "contacted", "in_progress", "negotiation", "converted", "lost"] },
        },
...
async function executeCountContacts(args: Record<string, unknown>) {
...
async function executeGetContactStats(args: Record<string, unknown>) {
  ...
  const filtered = (data || [])
    .filter((r: any) => r.group_type === groupBy)
    .slice(0, limit)
    .map((r: any) => ({
      group: r.group_label,
      ...
      with_deep_search: r.with_deep_search,
      with_alias: r.with_alias,
    }));
...
async function executeUpdateLeadStatus(args: Record<string, unknown>) {
  ...
  if (!ids?.length) return { error: "No contact IDs provided" };
  ...
  if (error) return { error: error.message };
  return { success: true, updated: ids.length, new_status: status };
}

async function executeTool(name: string, args: Record<string, unknown>) {
  switch (name) {
    case "search_contacts": return executeSearchContacts(args);
    case "count_contacts": return executeCountContacts(args);
    case "get_contact_stats": return executeGetContactStats(args);
