...
const VALID_PARTNER_TYPES = ["freight_forwarder", "customs_broker", "carrier", "nvocc", "3pl", "courier"];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!byok) {
      ...
      if (!credits || credits.balance < 5) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "Crediti insufficienti. Acquista crediti extra o aggiungi le tue chiavi API.",
          }),
    ...
    if (!partnerId || !profileData) {
      return new Response(JSON.stringify({ success: false, error: "partnerId and profileData required" }), {
        status: 400,
        headers: { ...dynCors, "Content-Type": "application/json" },
      });
    }
