...
Deno.serve(async (req) => {
  trackUsage("send-linkedin", "quarantine", { note: "Q2 bonifica, scadenza 2026-10-02" });
  ...
  try {
    ...
    if (!/^https?:\/\/([\w-]+\.)?linkedin\.com\/(in|pub)\//i.test(recipient)) {
      return new Response(
        JSON.stringify({
          ...
          detail: "Recipient must be a linkedin.com/in/... profile URL",
        }),
        {
          status: 400,
    ...
    if (scheduled_for) {
      ...
      if (isNaN(d.getTime())) {
        return edgeErrorWithStatus("VALIDATION_ERROR", "invalid_scheduled_for", 400, {
          ...corsHeaders,
          "Content-Type": "application/json",
        });
      }
