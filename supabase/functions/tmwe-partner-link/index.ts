...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "unlink") {
      const { error } = await svc.from("tmwe_partner_links").delete().eq("partner_id", partner_id);
      if (error) throw error;
      await logTmweAudit(svc, {
        op_name: "partner-unlink",
        identity: "user",
      ...
      .upsert(
        {
          partner_id,
          tmwe_client_id,
          tmwe_vat: tmwe_vat ?? null,
          match_confidence,
          linked_by_user_id: auth.userId,
