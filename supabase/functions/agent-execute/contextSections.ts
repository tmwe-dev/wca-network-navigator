...
import { sanitizeForPrompt, wrapUntrusted, type SanitizeFinding } from "../_shared/promptSanitizer.ts";
...
export async function appendUserSettingsSection(
  ...
  if (timingSettings.length) {
    ...
    if (approvalSetting?.value === "true") {
      ctx.text +=
        "APPROVAZIONE OBBLIGATORIA: Ogni azione (email, WhatsApp, LinkedIn) DEVE essere messa in coda con status 'pending' per approvazione umana. Non eseguire direttamente.\n";
    }
  }
}
...
export async function appendTeamSection(
  ...
  if (allAgents?.length) {
    const { data: allAssignmentsData } = await supabase
      ...
      .eq("user_id", userId);
    const allAssignments = asArray(
      allAssignmentsData as Array<Pick<ClientAssignmentRow, "agent_id" | "source_id">> | null,
    );
  ...
  if (myClients.length) {
    ...
    for (const client of myClients.slice(0, 10)) {
      ...
      if (client.source_type === "partner") {
        const { data } = await supabase.from("partners").select("email").eq("id", client.source_id).maybeSingle();
      ...
      if (clientMsgs.length) {
        ...
        for (const [addr, msgs] of byClient) {
          ...
          for (const msg of msgs.slice(0, 3)) {
            ...
            const subjSafe = sanitizeForPrompt(subjNorm.text, {
            ...
            if (msg.body_text) {
              ...
              const bodySafe = sanitizeForPrompt(bodyNorm.text, {
                source: "email-inbound",
                maxChars: 150,
                policy: "redact",
              });
...
export async function appendDirectorViewSection(
  ...
  if (
    (allAgents?.find((a) => a.id === agentId)?.role as string | undefined) === "director" ||
    (allAgents?.find((a) => a.id === agentId)?.role as string | undefined) === "account"
  ) {
    if (allAgents?.length) {
      const otherAgentIds = allAgents.filter((a) => a.id !== agentId).map((a) => a.id);
