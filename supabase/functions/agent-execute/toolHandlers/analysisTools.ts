...
const EmailAnalysisSchema = z.object({
  ...
  suggested_action: z
    .enum(["follow_up", "escalation", "close", "schedule_call", "respond_info", "ignore"])
    .default("ignore"),
...
export async function handleGetDirectoryStatus(
  ...
  const { count: importsRunning } = await supabase
    .from("download_jobs")
...
export async function handleGetGlobalSummary(supabase: SupabaseClient): Promise<unknown> {
  const { count: totalPartners } = await supabase.from("partners").select("id", { count: "exact", head: true });
  const { count: withProfile } = await supabase
    .from("partners")
    .select("id", { count: "exact", head: true })
...
export async function handleListJobs(supabase: SupabaseClient, args: Record<string, unknown>): Promise<unknown> {
  const { data, error } = await supabase
    .from("download_jobs")
    .select("id, country_name, status, current_index, total_count, contacts_found_count")
...
export async function handleCheckJobStatus(supabase: SupabaseClient, args: Record<string, unknown>): Promise<unknown> {
  if (args.job_id) {
    const { data } = await supabase
      .from("download_jobs")
      .select("id, status, current_index, total_count, contacts_found_count, last_processed_company, error_message")
      .eq("id", args.job_id)
      .single();
...
export async function handleGetOperationsDashboard(supabase: SupabaseClient, userId: string): Promise<unknown> {
  const [dlJobs, emailQ, agTasks, acts] = await Promise.all([
    supabase
      .from("download_jobs")
      .select("id, country_name, status, current_index, total_count, contacts_found_count, error_message, created_at")
      .order("created_at", { ascending: false })
      .limit(10),
    supabase
  ...
  return {
    ...
    activities: {
      pending: activities.filter((a: { status: string; scheduled_at?: string | null }) => a.status === "pending")
        .length,
...
export async function handleGetSystemAnalytics(supabase: SupabaseClient, userId: string): Promise<unknown> {
  const results: Record<string, unknown> = {};
  const { count: totalPartners } = await supabase.from("partners").select("id", { count: "exact", head: true });
  const { count: partnersWithEmail } = await supabase
    .from("partners")
    .select("id", { count: "exact", head: true })
    .not("email", "is", null);
  const { count: partnersWithProfile } = await supabase
    .from("partners")
    .select("id", { count: "exact", head: true })
    .not("raw_profile_html", "is", null);
  const { count: partnersConverted } = await supabase
    .from("partners")
    .select("id", { count: "exact", head: true })
    .eq("lead_status", "converted");
  const { count: partnersContacted } = await supabase
    .from("partners")
    .select("id", { count: "exact", head: true })
    .eq("lead_status", "first_touch_sent");
  results.partners = {
    total: totalPartners,
    with_email: partnersWithEmail,
    with_profile: partnersWithProfile,
    converted: partnersConverted,
    contacted: partnersContacted,
  };
  ...
  for (const t of (taskData || []) as Array<{ status: string }>) {
    taskCounts[t.status] = (taskCounts[t.status] || 0) + 1;
  }
  results.agent_tasks = taskCounts;

  const { count: activitiesPending } = await supabase
    .from("activities")
    .select("id", { count: "exact", head: true })
    .eq("status", "pending");
  const { count: activitiesOverdue } = await supabase
    .from("activities")
    .select("id", { count: "exact", head: true })
    .eq("status", "pending")
...
export async function handleAnalyzeIncomingEmail(
  ...
  const { data: msg } = await supabase
    ...
    .select("from_address, to_address, subject, body_text, email_date, partner_id")
  ...
  if (msg.from_address) {
    ...
    if (rule && !rule.exclusive_agent_id) {
      ...
      if (executingAgentId) {
        await supabase
          .from("email_address_rules")
          .insert({ email_address: fromAddr, user_id: userId, exclusive_agent_id: executingAgentId, category: "auto" });
        const { data: ag } = await supabase.from("agents").select("name").eq("id", executingAgentId).single();
        exclusiveAgentName = ag?.name || null;
      }
    ...
    messages: [
      {
        role: "system",
        content:
          'Analizza questa email e rispondi SOLO con un JSON valido: {"sentiment": "positive|neutral|negative", "intent": "interest|info_request|refusal|ooo|auto_reply|spam|other", "suggested_action": "follow_up|escalation|close|schedule_call|respond_info|ignore", "urgency": 1-5, "summary": "breve riassunto in italiano"}',
      },
      {
        role: "user",
        content: `Da: ${msg.from_address}\nOggetto: ${msg.subject}\n\n${msg.body_text?.substring(0, 2000) || "(vuoto)"}`,
      },
    ],
    max_tokens: 500,
  });
...
export async function handleGetApprovalDashboard(supabase: SupabaseClient, userId: string): Promise<unknown> {
