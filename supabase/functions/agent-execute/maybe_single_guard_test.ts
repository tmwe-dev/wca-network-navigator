...
Deno.test("agent-execute: agent_tasks lookup uses maybeSingle()", async () => {
  ...
  const taskLookupBlock = src.match(
    /from\("agent_tasks"\)[\s\S]*?\.eq\("id", task_id\)[\s\S]*?(maybeSingle|single)\(\)/,
  );
...
Deno.test("agent-autonomous-cycle: partner lookups use maybeSingle()", async () => {
  const src = await Deno.readTextFile(new URL("../agent-autonomous-cycle/index.ts", import.meta.url));
  // Nessuna occorrenza di `.single()` su lookup partners by id
  const violations = [...src.matchAll(/from\("partners"\)[^;]*\.eq\("id",[^;]*\.single\(\)/g)];
  assert(violations.length === 0, `Found ${violations.length} partners.single() lookup(s) — must be maybeSingle()`);
});
