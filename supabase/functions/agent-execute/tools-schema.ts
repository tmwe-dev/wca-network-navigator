/**
 * tools-schema.ts — OpenAI tool/function definitions for agent-execute
 *
 * Estratto da `index.ts` in sessione #24 (Ondata 2, Fase 4 Vol. I — split
 * dei file monolitici). Contenuto puro-dato: 31 definizioni di tool con
 * schema JSON parameters. Nessun comportamento, nessuna I/O.
 *
 * Il consumatore è `index.ts` che esegue `executeTool(name, args, ...)`
 * e usa `ALL_TOOLS` solo come schema da passare all'LLM / per validare
 * l'esistenza del tool name. Se aggiungi un tool qui, aggiungi anche il
 * case corrispondente nello switch di `executeTool` in index.ts.
 */
