...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (auth.authMethod === "jwt" && auth.userId !== "extension-anon") {
      await supabase
        .from("user_ra_sessions")
        .upsert({ user_id: auth.userId, cookie, status: "ok", updated_at: now }, { onConflict: "user_id" });
    }

    await supabase
      .from("app_settings")
      .upsert({ key: "ra_session_cookie", value: cookie, updated_at: now }, { onConflict: "key" });
    await supabase
      .from("app_settings")
      .upsert({ key: "ra_session_status", value: "ok", updated_at: now }, { onConflict: "key" });
    await supabase
      .from("app_settings")
      .upsert({ key: "ra_session_checked_at", value: now, updated_at: now }, { onConflict: "key" });
