...
serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      supabase.from("partners").select("*", { count: "exact", head: true }).eq("lead_status", "contacted"),
      supabase.from("imported_contacts").select("*", { count: "exact", head: true }).eq("lead_status", "contacted"),
