...
export async function executeRunKbAudit(args: Record<string, unknown>, userId: string): Promise<unknown> {
  const level = String(args.audit_level || "all");
  try {
    const resp = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/kb-supervisor`, {
      method: "POST",
      headers: {
    ...
    return {
      ...
      } problemi trovati (${result.summary?.critical || 0} critici, ${result.summary?.high || 0} alti).`,
    };
  } catch (err: unknown) {
