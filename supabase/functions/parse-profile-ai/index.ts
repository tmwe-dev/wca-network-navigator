...
Deno.serve(async (req) => {
  trackUsage("parse-profile-ai", "quarantine", { note: "Q2 bonifica, scadenza 2026-10-02" });
    ...
    if (partner.ai_parsed_at && !forceReparse) {
      return new Response(
        JSON.stringify({
          success: true,
          skipped: true,
          message: "Already AI-parsed. Use forceReparse=true to re-run.",
        }),
        { headers: { ...dynCors, "Content-Type": "application/json" } },
      );
    }
...
${truncated}`;
    ...
    const response = await aiFetch({
      ...
      messages: [
        {
          role: "system",
          content: "Sei un esperto di parsing dati logistici. Rispondi sempre con JSON strutturato tramite tool call.",
        },
