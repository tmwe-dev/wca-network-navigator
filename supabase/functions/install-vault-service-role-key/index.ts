...
Deno.serve(async (req) => {
  ...
  if (!serviceKey) {
    return edgeErrorWithStatus("INTERNAL_ERROR", "service_key_not_in_env", 500, { ...corsHeaders, "Content-Type": "application/json" });
  }
  ...
  if (u.searchParams.get("verify") === "1") {
    ...
    if (error)
      return edgeErrorWithStatus("INTERNAL_ERROR", error.message, 500, { ...corsHeaders, "Content-Type": "application/json" });
