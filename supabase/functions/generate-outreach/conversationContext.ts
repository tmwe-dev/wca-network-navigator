...
export async function loadConversationContextOutreach(
  ...
  const [ctxRes, rulesRes, classRes] = await Promise.all([
    supabase
      ...
      .select("conversation_summary, last_exchanges, response_rate, avg_response_time_hours, dominant_sentiment")
      .eq("user_id", userId)
    ...
    supabase
      ...
      .select(
        "tone_override, topics_to_emphasize, topics_to_avoid, custom_prompt, category, email_prompts(instructions)",
      )
      .eq("user_id", userId)
      .eq("email_address", emailAddress)
      .maybeSingle(),
  ...
  if (rules) {
    if (
      (rules as Record<string, unknown>).custom_prompt &&
      typeof (rules as Record<string, unknown>).custom_prompt === "string"
    ) {
    ...
    if (
      (rules as Record<string, unknown>).category &&
      typeof (rules as Record<string, unknown>).category === "string"
    ) {
      ...
      const isHoldingPattern =
        isCat.includes("attesa") ||
        isCat.includes("hold") ||
        isCat.includes("paused") ||
        isCat.includes("on_hold") ||
        isCat.includes("holding") ||
        isCat.includes("pausa") ||
        isCat.includes("pending");

      // Detect priority signals
      const isPriority =
        isCat.includes("priority") ||
        isCat.includes("urgente") ||
        isCat.includes("vip") ||
        isCat.includes("prioritario");
  ...
  if (ctx) {
    ...
    const exchanges = Array.isArray(ctx.last_exchanges)
      ? (ctx.last_exchanges as Array<Record<string, unknown>>).slice(-5)
      : [];
    if (exchanges.length) {
      parts.push(
    ...
    parts.push(
      `RESPONSE PATTERN: Rate ${Math.round(ctx.response_rate ?? 0)}%, avg ${ctx.avg_response_time_hours != null ? `${Math.round(ctx.avg_response_time_hours)}h` : "N/A"}, sentiment: ${ctx.dominant_sentiment || "neutral"}`,
    );
  }
  ...
  if (rules) {
    ...
    if (
      ...
      ((rules as Record<string, unknown>).email_prompts as Record<string, unknown>)?.instructions
    )
      parts.push(
        `SENDER PROMPT: ${((rules as Record<string, unknown>).email_prompts as Record<string, unknown>).instructions}`,
      );
  }
