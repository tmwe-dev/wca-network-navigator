...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: link } = await svc
      ...
      .select("tmwe_client_id")
      .eq("partner_id", partner_id)
      .maybeSingle();
    if (!link) {
      return new Response(
        JSON.stringify({ error: "PARTNER_NOT_LINKED", message: "Collega prima il partner a un cliente TMWE." }),
        {
          status: 409,
          headers: { ...headers, "Content-Type": "application/json" },
        },
    ...
    const { data: snap } = await svc
      ...
      .eq("tmwe_client_id", tmweClientId)
      .maybeSingle();
