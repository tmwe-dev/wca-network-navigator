...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!cookie || typeof cookie !== "string" || cookie.length < 20 || cookie.length > 500 || /[<>"']/.test(cookie)) {
    ...
    await supabase
      .from("app_settings")
      .upsert({ key: "linkedin_li_at", value: encryptedCookie, updated_at: now }, { onConflict: "key" });
    await supabase
      .from("app_settings")
      .upsert({ key: "linkedin_session_status", value: "ok", updated_at: now }, { onConflict: "key" });
    await supabase
      .from("app_settings")
      .upsert({ key: "linkedin_session_checked_at", value: now, updated_at: now }, { onConflict: "key" });
