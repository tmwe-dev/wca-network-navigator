-- FIX 1+9: Stop creating follow_up activities for unmatched/system inbound messages.
-- The old trigger created a follow_up activity for EVERY inbound message, including
-- newsletters, system emails, and messages with no partner match. This polluted
-- the "Da Inviare" queue with garbage rows.
--
-- Fix: Only create activity if we matched a partner_id.
...
BEGIN
  ...
  -- Skip pending schedule cancellation and activity creation if no partner matched.
  -- This prevents spam/newsletter/system emails from creating garbage activities.
