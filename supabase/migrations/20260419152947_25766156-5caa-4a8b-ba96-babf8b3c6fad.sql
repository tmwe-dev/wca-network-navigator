-- Update get_country_stats RPC: use profile_description as source of truth (sync WCA), not raw_profile_html (legacy scraper)
...
AS $function$
  WITH partner_base AS (
    SELECT 
      ...
      p.country_code,
      p.office_type,
      CASE WHEN p.profile_description IS NOT NULL AND length(trim(p.profile_description)) > 0 THEN 1 ELSE 0 END as has_profile,
