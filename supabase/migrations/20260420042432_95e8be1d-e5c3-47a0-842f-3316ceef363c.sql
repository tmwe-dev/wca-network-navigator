-- Fix EVO-0A #2: get_dashboard_snapshot
-- 3 bug:
--  1. partners.lead_status = 'contacted' (stato morto) → 'first_touch_sent'
--  2. imported_contacts.lead_status = 'contacted' (stato morto) → 'first_touch_sent'
--  3. agent_tasks.status = 'running' (enum non valido) → 'in_progress'
-- Nomi JSON mantenuti per zero impatto frontend.
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'contacted_partners',       (SELECT count(*) FROM partners WHERE lead_status = 'first_touch_sent'),
