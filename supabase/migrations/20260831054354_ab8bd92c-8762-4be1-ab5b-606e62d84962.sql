...
CREATE OR REPLACE FUNCTION public.v3_directory(
  ...
  _fonti text[] DEFAULT NULL,
  _paesi text[] DEFAULT NULL,
  _stati text[] DEFAULT NULL,
  _aziende text[] DEFAULT NULL,
  _solo_email boolean DEFAULT false,
  _ordine text DEFAULT 'recente',
  _discendente boolean DEFAULT true,
...
AS $function$
  ...
  arricchito as (
    select
      b.*,
      count(*) over (partition by lower(btrim(coalesce(b.azienda, ''))))::int as colleghi_raw
    from base b
  ),
  filtered as (
    select a.*
    from arricchito a
    where (_fonti is null or cardinality(_fonti) = 0 or a.fonte = any(_fonti))
      and (_paesi is null or cardinality(_paesi) = 0 or a.paese = any(_paesi) or upper(coalesce(a.paese_code, '')) = any(_paesi))
      and (_stati is null or cardinality(_stati) = 0 or coalesce(a.stato, '') = any(_stati))
      and (_aziende is null or cardinality(_aziende) = 0 or lower(btrim(coalesce(a.azienda, ''))) = any(_aziende))
  ...
  from filtered f
  order by
    case when _discendente then
      case _ordine
        when 'nome' then lower(f.nome)
        when 'azienda' then lower(f.azienda)
        when 'paese' then lower(f.paese)
        when 'fonte' then f.fonte
        when 'stato' then lower(f.stato)
      end
    end desc nulls last,
    case when not _discendente then
      case _ordine
        when 'nome' then lower(f.nome)
        when 'azienda' then lower(f.azienda)
        when 'paese' then lower(f.paese)
    ...
    case when _ordine = 'interazioni' and _discendente then f.interazioni end desc nulls last,
    case when _ordine = 'interazioni' and not _discendente then f.interazioni end asc nulls last,
    case when _ordine = 'recente' and _discendente then f.ultima_interazione end desc nulls last,
    case when _ordine = 'recente' and not _discendente then f.ultima_interazione end asc nulls last,
    lower(f.nome) asc nulls last
  offset greatest(_offset, 0)
  limit least(greatest(_limit, 1), 200);
$function$;

GRANT EXECUTE ON FUNCTION public.v3_directory(text, text[], text[], text[], text[], boolean, text, boolean, integer, integer) TO authenticated;