...
- Edge IMAP/SMTP multi-mailbox attive: `check-inbox`, `email-imap-proxy` (test/fetch/send), `mark-imap-seen` e `send-email` accettano header opzionale `x-mailbox-id`. Senza header → casella personale (legacy invariato). Con header → credenziali via `_shared/resolveMailbox.ts` (mappa slug → ENV `IMAP_PASSWORD_<SLUG>` / `SMTP_PASSWORD_<SLUG>`). `email_sync_state` e `channel_messages` filtrati per `(user_id, mailbox_id)`. Frontend: `useApproveAndDispatch` legge `useActiveMailbox` e passa l'header; `useFunnemailInbox` filtra le query per `mailbox_id`.
