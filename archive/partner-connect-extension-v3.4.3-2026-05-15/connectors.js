...
const Connectors = {
  ...
  async configure(id, config) {
    ...
    const requiredFields = connector.configSchema.filter((field) => field.required).map((field) => field.key);
    ...
    test: async (config) => {
      try {
        const response = await fetch(config.webhookUrl, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text: "FireScrape test message" }),
        });
        return { success: response.ok, status: response.status };
      } catch (e) {
        return { success: false, error: e.message };
