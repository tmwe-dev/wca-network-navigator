  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Definisci le regole operative che il Supervisor AI e tutti gli agenti seguono. Queste regole guidano la
        generazione dei messaggi, il timing dei follow-up e la qualità delle comunicazioni.
      </p>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Switch
              checked={strategy.supervisorEnabled}
              onCheckedChange={(v) => setStrategy((s) => ({ ...s, supervisorEnabled: v }))}
            />
          </div>
          <div>
            <Label>Nome agente Supervisor</Label>
            <Input
              value={strategy.supervisorAgentName}
              onChange={(e) => setStrategy((s) => ({ ...s, supervisorAgentName: e.target.value }))}
              placeholder="Gigi"
              className="mt-1"
            />
          </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input
              type="number"
              value={strategy.dailyContactLimit}
              onChange={(e) => setStrategy((s) => ({ ...s, dailyContactLimit: +e.target.value }))}
              className="mt-1"
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              value={strategy.messageMaxLines}
              onChange={(e) => setStrategy((s) => ({ ...s, messageMaxLines: +e.target.value }))}
              className="mt-1"
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              value={strategy.followUpDays}
              onChange={(e) => setStrategy((s) => ({ ...s, followUpDays: +e.target.value }))}
              className="mt-1"
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              value={strategy.escalationDays}
              onChange={(e) => setStrategy((s) => ({ ...s, escalationDays: +e.target.value }))}
              className="mt-1"
            />
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <Textarea
            value={strategy.dataSources}
            onChange={(e) => setStrategy((s) => ({ ...s, dataSources: e.target.value }))}
            rows={3}
            placeholder="Da dove prendere i contatti…"
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <Textarea
            value={strategy.toneOfVoice}
            onChange={(e) => setStrategy((s) => ({ ...s, toneOfVoice: e.target.value }))}
            rows={3}
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <Textarea
            value={strategy.ethicalRules}
            onChange={(e) => setStrategy((s) => ({ ...s, ethicalRules: e.target.value }))}
            rows={4}
          />
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-3">
          {strategy.customRules.length === 0 && (
            <p className="text-xs text-muted-foreground">
              Nessuna regola personalizzata. Clicca "Aggiungi" per crearne una.
            </p>
          )}
