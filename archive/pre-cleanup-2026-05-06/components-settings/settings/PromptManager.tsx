...
export function PromptManager() {
  ...
  const goals: ContentItem[] = useMemo(() => {
    try {
      return JSON.parse(settings?.workspace_goals || "null") || DEFAULT_GOALS;
    } catch (e) {
      log.debug("fallback used after parse failure", { error: e instanceof Error ? e.message : String(e) });
  ...
  const proposals: ContentItem[] = useMemo(() => {
    try {
      return JSON.parse(settings?.workspace_proposals || "null") || DEFAULT_PROPOSALS;
    } catch (e) {
      log.debug("fallback used after parse failure", { error: e instanceof Error ? e.message : String(e) });
  ...
  const handleImproveWithAI = async () => {
    ...
    setImproving(true);
    try {
      const data = await invokeEdge<Record<string, unknown>>("improve-email", {
        body: { html: editPrompt, tone: editTone, improveType: "prompt" },
        context: "PromptManager.improve_email",
      });
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-1.5">
                        <Badge
                          className={`text-[8px] px-1.5 py-0 border ${CATEGORY_COLORS[item.category] || CATEGORY_COLORS.altro}`}
                        >
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <Input
                ...
                className="w-12 h-9 text-center text-lg px-1"
                maxLength={2}
              />
              <Input
                value={editName}
            ...
            <div className="flex gap-2">
              ...
              <Select value={editSource} onValueChange={(v) => setEditSource(v as "email_type" | "goal" | "proposal")}>
                ...
                <SelectContent>
                  <SelectItem value="email_type" className="text-xs">
                    Tipo Email
                  </SelectItem>
                  <SelectItem value="goal" className="text-xs">
                    Goal
                  </SelectItem>
                  <SelectItem value="proposal" className="text-xs">
                    Proposta
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <div className="flex justify-end mt-1">
                <Button
                  ...
                  disabled={improving || !editPrompt.trim()}
                >
