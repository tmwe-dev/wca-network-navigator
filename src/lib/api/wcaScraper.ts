  ...
  try {
    ...
    return {
      ...
      partner: {
        ...
        networks: (p.networks || []).map((n: unknown) =>
          typeof n === "string" ? { name: n } : (n as { name: string; expires?: string })
        ),
        contacts: (p.contacts || []) as Array<{ title: string; name?: string; email?: string; phone?: string; mobile?: string }>,
      },
