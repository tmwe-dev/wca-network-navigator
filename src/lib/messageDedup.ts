...
function normalizeText(text: string): string {
  return (text || "").trim().replace(/\s+/g, " ").toLowerCase().slice(0, 200); // Cap length for ID stability
}
 ...
 */
export function buildDeterministicId(prefix: string, contact: string, text: string, timestamp?: string): string {
