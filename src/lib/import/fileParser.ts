...
function detectHeader(rows: string[][]): boolean {
  ...
  const avgNonNumeric =
    rest.reduce((sum, row) => sum + row.filter((v) => isNaN(Number(v)) && v.trim() !== "").length, 0) / rest.length;
  if (firstNonNumeric > avgNonNumeric + 1) return true;
  const textRatio = first.filter((v) => typeof v === "string" && v.match(/^[a-zA-ZÀ-ÿ_\s.]+$/)).length / first.length;
