...
export function prependMessage<T extends ChannelKind>(
  ...
  if (incoming.message_id_external) {
    const dupByExternal = existing.find((m) => m.message_id_external === incoming.message_id_external);
    if (dupByExternal) {
 ...
 */
export function countUnread<T extends ChannelKind>(messages: ReadonlyArray<ChannelMessage<T>>): number {
