...
function EnrichmentRow({ on, label, age }: Row) {
  return (
    <div className="flex items-center gap-1 text-[10px] leading-tight">
      <span className={cn("shrink-0 w-3 text-center", on ? "text-success" : "text-muted-foreground")}>
        {on ? "✓" : "○"}
      </span>
      <span className={on ? "text-foreground" : "text-muted-foreground"}>{label}</span>
      {age != null && <span className="text-muted-foreground ml-auto pl-1">{age}gg fa</span>}
    </div>
  );
}

export default function EnrichmentStatusBadges({ partnerId }: { partnerId: string | null }) {
