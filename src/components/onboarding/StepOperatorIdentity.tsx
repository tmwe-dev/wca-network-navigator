...
export function StepOperatorIdentity({ initial, email, saving, onSubmit }: Props) {
  ...
  useEffect(() => {
    if (hydrated) return;
    const hasAny = initial.displayName || initial.phone || initial.whatsapp || initial.linkedinUrl;
    if (hasAny) {
      setValues(initial);
      setHydrated(true);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 flex items-start gap-2 text-xs text-amber-700 dark:text-amber-300">
        ...
        <div>
          <strong>Dati aziendali.</strong> Inserisci email, telefono e WhatsApp <em>aziendali</em>: saranno visibili al
          team e usati per inviare comunicazioni a tuo nome. Le chiavi API tecniche sono già configurate automaticamente
          dal tuo accesso TMWE.
        </div>
      </div>
