    ...
    <div className="p-5 space-y-3">
      ...
      <div className="bg-gradient-to-br from-primary/5 via-card to-primary/5 backdrop-blur-sm border border-primary/10 rounded-2xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Rating + Trophy + Membership badges */}
            <div className="flex items-center gap-2.5 mt-1.5 flex-wrap">
              {partner.rating > 0 && <MiniStars rating={Number(partner.rating)} size="w-3.5 h-3.5" />}
              {years > 0 && <TrophyRow years={years} />}
              {partner.wca_id && (
                <span className="text-[10px] text-muted-foreground font-mono">#{partner.wca_id}</span>
              )}
              <span className={cn(
            ...
            <div className="flex items-center gap-3 mt-2 text-xs flex-wrap">
              {partner.phone && (
                <a href={`tel:${partner.phone}`} className="flex items-center gap-1 text-foreground/80 hover:text-foreground transition-colors">
                  <Phone className="w-3 h-3" strokeWidth={1.5} /> {partner.phone}
                </a>
              )}
              {partner.email && (
                <a href={`mailto:${partner.email}`} className="flex items-center gap-1 text-foreground/80 hover:text-foreground transition-colors">
                  <Mail className="w-3 h-3" strokeWidth={1.5} /> {partner.email}
                </a>
              )}
              {partner.website && (
                <a href={partner.website.startsWith("http") ? partner.website : `https://${partner.website}`}
                  ...
                  <Globe className="w-3 h-3" strokeWidth={1.5} /> {partner.website}
                </a>
              )}
            </div>
            <div className="mt-1">
      ...
      {partner.profile_description && (
        <Section>
          ...
          <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-line">{partner.profile_description}</p>
        </Section>
      )}
