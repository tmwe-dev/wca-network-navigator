    ...
    <>
      ...
      {partner.profile_description && (
        <Section>
          ...
          <p className="text-sm text-foreground leading-relaxed whitespace-pre-line">{String(partner.profile_description)}</p>
        </Section>
      )}
            ...
            {contacts.map((c) => (
              <div key={c.id} className="bg-card/60 border border-primary/10 rounded-lg p-2.5 space-y-1">
                ...
                <div className="flex flex-col gap-0.5 ml-5">
                  ...
                  {c.direct_phone && (
                    <a href={`tel:${c.direct_phone}`} className="flex items-center gap-1.5 text-xs text-foreground hover:text-foreground">
                      <Phone className="w-3 h-3" /> {c.direct_phone}
                    </a>
                  )}
                  {c.mobile && (
                    <a href={`tel:${c.mobile}`} className="flex items-center gap-1.5 text-xs text-foreground hover:text-foreground">
                ...
                <Section>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {partnerCertifications.map((c, i: number) => (
                      <span key={i} className="flex items-center gap-1 bg-card/60 border border-primary/10 rounded-lg px-2 py-1 text-xs text-foreground">
                ...
                <Section>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {branchCountries.map(({ code, name }) => (
                      <span key={code} className="flex items-center gap-1.5 text-xs bg-card/60 border border-primary/10 rounded-lg py-1 px-2 text-foreground">
                ...
                <Section>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {((enrichment as Record<string, unknown>).key_markets as string[]).map((market: string, i: number) => {
                      ...
                      return (
                        <span key={i} className="flex items-center gap-1 text-xs bg-card/60 border border-primary/10 rounded-lg py-1 px-2 text-foreground">
