      ...
      {partner.profile_description && (
        <Section>
          ...
          <p className="text-sm text-foreground leading-relaxed whitespace-pre-line">
            {String(partner.profile_description)}
          </p>
        </Section>
      )}
            ...
            {contacts.map((c) => (
              <div key={c.id} className="bg-card/60 border border-primary/10 rounded-lg p-2.5 space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {c.is_primary && (
                    <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 font-semibold uppercase">
                      Primary
                    </span>
                  )}
                </div>
                {c.title && <p className="text-[11px] text-foreground ml-5">{c.title}</p>}
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleSendEmail({
                          email: c.email!,
                          name: c.name,
                          company: String(partner.company_name || ""),
                          partnerId: String(partner.id),
                          contactId: c.id,
                        })
                      }
                      className="flex items-center gap-1.5 text-xs text-foreground hover:text-foreground hover:underline text-left"
                    >
          ...
          <div className="flex flex-wrap gap-1.5">
            {allServices.map((s, i: number) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-center gap-1.5 bg-card/60 border border-primary/10 rounded-lg px-2.5 py-1.5"
                >
                  <Icon className="w-3.5 h-3.5 text-primary" strokeWidth={1.5} />
                  <span className="text-[10px] text-foreground">{formatServiceCategory(s.service_category)}</span>
                </div>
              );
            })}
          </div>
        ...
        <Collapsible>
          <CollapsibleTrigger className="w-full">
            <div className="flex items-center gap-2 cursor-pointer hover:bg-primary/5 rounded-xl px-3 py-2 transition-colors border border-primary/10">
              ...
              <span className="text-xs font-medium text-foreground">Dettagli Avanzati</span>
          ...
          <CollapsibleContent>
            <div className="space-y-3 mt-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {partner.member_since && (
                      <div className="flex items-center gap-2 text-xs text-foreground">
                        ...
                        <span>
                          Membro dal {format(new Date(String(partner.member_since)), "MMMM yyyy", { locale: it })}
                        </span>
                      </div>
                    )}
                    {partner.fax && (
                ...
                <Section>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {partnerCertifications.map((c, i: number) => (
                      <span
                        key={i}
                        className="flex items-center gap-1 bg-card/60 border border-primary/10 rounded-lg px-2 py-1 text-xs text-foreground"
                      >
                ...
                <Section>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {branchCountries.map(({ code, name }) => (
                      <span
                        key={code}
                        className="flex items-center gap-1.5 text-xs bg-card/60 border border-primary/10 rounded-lg py-1 px-2 text-foreground"
                      >
              ...
              {enrichment &&
                Array.isArray((enrichment as Record<string, unknown>).key_markets) &&
                ((enrichment as Record<string, unknown>).key_markets as string[]).length > 0 && (
                  <Section>
                    <SectionTitle icon={Globe}>Mercati Principali</SectionTitle>
                    <div className="flex flex-wrap gap-1.5">
                      {((enrichment as Record<string, unknown>).key_markets as string[]).map(
                        (market: string, i: number) => {
                          const code = resolveCountryCode(market);
                          return (
                            <span
                              key={i}
                              className="flex items-center gap-1 text-xs bg-card/60 border border-primary/10 rounded-lg py-1 px-2 text-foreground"
                            >
              ...
              {enrichment &&
                Array.isArray((enrichment as Record<string, unknown>).key_routes) &&
                (
                  (enrichment as Record<string, unknown>).key_routes as {
                    from?: string;
                    origin?: string;
                    to?: string;
                    destination?: string;
                    ...
                    <div className="space-y-1">
                      {(
                        ...
                        return (
                          <div
                            key={i}
                            className="flex items-center justify-center gap-3 bg-card/60 border border-primary/10 rounded-lg py-1.5 px-3"
                          >
