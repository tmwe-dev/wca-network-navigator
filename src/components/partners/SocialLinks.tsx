...
export function SocialLinks({ partnerId, contactId, compact }: SocialLinksProps) {
  ...
  const filtered = contactId ? links.filter((l) => l.contact_id === contactId) : links.filter((l) => !l.contact_id);
        ...
        return (
          <Tooltip key={link.id}>
            <TooltipTrigger asChild>
              ...
              >
                {config?.icon || <ExternalLink className={cn(compact ? "w-3 h-3" : "w-3.5 h-3.5")} />}
                {!compact && <span className="text-sm font-medium">{config?.label || link.platform}</span>}
              </a>
            </TooltipTrigger>
            <TooltipContent>{config?.label || link.platform}</TooltipContent>
          </Tooltip>
        );
      })}
    </div>
