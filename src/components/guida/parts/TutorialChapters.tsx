/**
 * Parte 2 · Tutorial operativo — capitoli 0..8.
  ...
  return (
    <>
        ...
        <div className="space-y-8 max-w-4xl">
          <div className="space-y-3">
            ...
            <p className="text-lg text-white/50 leading-relaxed">
              I capitoli seguono l'ordine esatto del menu (le 7 macro-aree: Comando, Esplora, Pipeline, Comunica,
              Cervello, Lab, Config). Per ogni sezione trovi cosa fa, le operazioni possibili e una checklist di test
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-white/10 bg-white/5 p-5 space-y-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <h3 className="font-bold text-white">Struttura capitolo</h3>
              </div>
              <p className="text-sm text-white/50">
                Cosa fa → Operazioni possibili → Test di verifica. Il percorso della pagina è indicato in alto a destra.
              </p>
            </div>
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-5 space-y-2">
              <div className="flex items-center gap-2 text-primary">
                <FlaskConical className="w-5 h-5" />
                <h3 className="font-bold text-white">Legenda test</h3>
              </div>
              <p className="text-sm text-white/50">
                Ogni test è <span className="text-white/70">Azione</span> +{" "}
                <span className="text-success font-medium">Atteso</span>. Se l'esito non corrisponde, la funzione va
                segnalata.
              </p>
            </div>
          </div>
      ...
      <TutorialChapter
        ...
        icon={Command}
        title="Command"
        subtitle="L'agente AI in linguaggio naturale"
        path="/v2/command"
        ...
        tests={[
          {
            action: "Scrivi 'quanti partner abbiamo a Malta' e invia.",
            expect: "Risposta numerica con fonte/tool citato, niente '0 risultati' se i dati esistono.",
          },
          {
            action: "Chiedi 'scrivi una mail di presentazione a un partner di Malta'.",
            expect: "L'AI mantiene il contesto e propone una bozza, non perde il riferimento al paese.",
          },
          {
            action: "Premi il toggle voce (icona Radio) accanto al microfono.",
            expect: "Si attiva la modalità real-time; in caso di errore socket parte il retry WebRTC.",
          },
          {
            action: "Dai un comando che modifica dati (es. crea agente).",
            expect: "Compare un pannello di approvazione prima dell'esecuzione.",
          },
        ]}
        screenshotContent={
          <div className="space-y-3">
      ...
      <TutorialChapter
        ...
        subtitle="Automazioni con KPI, budget e approvazioni"
        ...
        tests={[
          {
            action: "Crea una nuova missione e salva.",
            expect: "La missione compare nell'elenco con stato attivo e KPI a zero.",
          },
          {
            action: "Apri una missione e controlla budget/cadenza.",
            expect: "I valori impostati sono persistiti e mostrati correttamente.",
          },
          { action: "Metti in pausa la missione.", expect: "Lo stato passa a 'in pausa' e non genera nuove azioni." },
        ]}
        screenshotContent={
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2 rounded bg-white/5 text-xs text-white/60">
              <span>Outreach Malta Q3</span>
              <span className="text-success">● attiva</span>
            </div>
            <div className="flex items-center justify-between p-2 rounded bg-white/5 text-xs text-white/60">
              <span>Follow-up no-reply</span>
              <span className="text-warning">● in pausa</span>
            </div>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        area="Esplora"
        icon={Search}
        title="Esplora Network"
        subtitle="Ricerca e gestione partner"
        ...
        tests={[
          {
            action: "Applica un filtro paese (es. Italia).",
            expect: "La lista si aggiorna mostrando solo i partner del paese.",
          },
          {
            action: "Avvia una Deep Search su un partner.",
            expect: "Barra di progresso con possibilità di stop; al termine compaiono nuovi contatti.",
          },
          {
            action: "Seleziona più partner e apri le azioni batch.",
            expect: "Le azioni disponibili agiscono su tutti i selezionati.",
          },
        ]}
        screenshotContent={
          <div className="space-y-3">
            <div className="flex gap-2 flex-wrap">
              {["Italia", "IATA ✓", "ISO 9001 ✓", "★★★★☆"].map((f) => (
                <span key={f} className="px-2 py-1 rounded-full bg-white/5 text-xs text-white/60">
                  {f}
                </span>
              ))}
            </div>
            <div className="p-3 rounded bg-white/5 border border-white/10 text-xs text-white/60">
              ABC Logistics · Milano · ★★★★☆
            </div>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        icon={Rocket}
        title="Cockpit"
        subtitle="Pipeline Kanban dei lead"
        path="/v2/cockpit"
        reversed
        ...
        tests={[
          {
            action: "Trascina un lead in un altro stage.",
            expect: "Lo stato si aggiorna e resta persistito dopo refresh.",
          },
        ...
        screenshotContent={
          <div className="grid grid-cols-3 gap-2">
            {["Nuovo", "In corso", "Cliente"].map((s) => (
              <div key={s} className="p-2 rounded bg-white/5 text-xs text-white/60 text-center">
                {s}
              </div>
            ))}
          </div>
      ...
      <TutorialChapter
        ...
        icon={Trash2}
        title="Cestinone"
        subtitle="Soft-delete e ripristino"
        path="/v2/cestinone"
        reversed
        ...
        tests={[
          {
            action: "Elimina un elemento da un'altra sezione e apri il Cestinone.",
            expect: "L'elemento compare tra i soft-deleted, non è perso.",
          },
          { action: "Ripristina l'elemento.", expect: "Torna visibile nella sezione di origine." },
        ]}
        screenshotContent={
          <div className="p-3 rounded bg-white/5 border border-white/10 text-xs text-white/60">
            Partner · ABC Srl · eliminato 2 giorni fa · <span className="text-primary">Ripristina</span>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        icon={MessagesSquare}
        title="Comms"
        subtitle="WhatsApp + LinkedIn (stealth sync)"
        path="/v2/comms"
        ...
        tests={[
          {
            action: "Apri Comms con estensione WhatsApp attiva.",
            expect: "Le conversazioni recenti compaiono sincronizzate.",
          },
          { action: "Genera una risposta AI.", expect: "Passa per la revisione editoriale prima dell'invio." },
        ]}
        screenshotContent={
          <div className="space-y-2">
            <div className="flex items-center gap-2 p-2 rounded bg-white/5 text-xs text-white/60">
              <MessagesSquare className="w-4 h-4 text-success" /> WhatsApp · 4 nuove
            </div>
            <div className="flex items-center gap-2 p-2 rounded bg-white/5 text-xs text-white/60">
              <Contact className="w-4 h-4 text-info" /> LinkedIn · 2 nuove
            </div>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        tests={[
          ...
          {
            action: "Apri un'email molto grande.",
            expect: "Viene mostrata almeno una parte del testo, non un errore.",
          },
        ]}
        screenshotContent={
          <div className="p-3 rounded bg-white/5 border border-white/10 text-xs text-white/60">
            Da: luca.arcana@… · Oggetto: Richiesta preventivo
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        icon={Mail}
        title="Email"
        subtitle="Composizione e invio con revisione editoriale"
        path="/v2/email"
        ...
        tests={[
          {
            action: "Genera una bozza AI per un partner.",
            expect: "Viene prodotta e mostrata la revisione editoriale prima dell'invio.",
          },
          {
            action: "Invia un'email di test a un destinatario.",
            expect: "Conferma di invio e tracciamento; nessun invio duplicato.",
          },
        ]}
        screenshotContent={
          <div className="p-3 rounded bg-white/5 border border-white/10 text-xs text-white/60">
            Subject: Partnership — {"{{city}}"} corridor
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        subtitle="Classificazione risposte ed escalation"
        path="/v2/email-intelligence"
        reversed
        ...
        tests={[
          {
            action: "Apri un'email classificata e correggi la categoria.",
            expect: "La correzione viene salvata e usata per l'apprendimento.",
          },
          { action: "Simula una risposta positiva.", expect: "Il lead status si aggiorna/escala coerentemente." },
        ]}
        screenshotContent={
          <div className="flex gap-2 flex-wrap">
            {["Interessato", "Info", "Bounce"].map((c) => (
              <span key={c} className="px-2 py-1 rounded-full bg-white/5 text-xs text-white/60">
                {c}
              </span>
            ))}
          </div>
      ...
      <TutorialChapter
        ...
        icon={Sparkles}
        title="Funnemail"
        subtitle="Claim e smistamento"
        path="/v2/funnemail-inbox"
        ...
        operations={["Reclamare (claim) un'email da lavorare", "Smistare per gruppo/competenza", "Rilasciare un claim"]}
        tests={[
        ...
        screenshotContent={
          <div className="p-3 rounded bg-white/5 border border-white/10 text-xs text-white/60">
            3 email in coda · <span className="text-primary">Claim</span>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        area="Comunica"
        icon={Contact}
        title="Rubriche WhatsApp / LinkedIn"
        subtitle="Contatti dei canali social"
        path="/v2/rubrica/whatsapp"
        reversed
        ...
        screenshotContent={
          <div className="p-2 rounded bg-white/5 text-xs text-white/60">
            +39 333 … · Mario Rossi · collegato a ABC Srl
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        subtitle="Creazione, persona, capabilities e tool"
        ...
        tests={[
          {
            action: "Crea un nuovo agente dal Command ('crea agente …').",
            expect: "Compare un pannello di approvazione; dopo conferma l'agente è creato.",
          },
          {
            action: "Modifica la persona di un agente e salva.",
            expect: "Le modifiche sono persistite e applicate alle nuove conversazioni.",
          },
          {
            action: "Disabilita un tool 'send' per un agente.",
            expect: "L'agente non può più eseguire quell'azione senza riabilitazione.",
          },
        ]}
        screenshotContent={
          <div className="space-y-2">
            <div className="flex items-center gap-2 p-2 rounded bg-white/5 text-xs text-white/60">
              🤖 Agente Outreach · ruolo: outreach
            </div>
            <div className="flex items-center gap-2 p-2 rounded bg-white/5 text-xs text-white/60">
              🧠 Agente Classificatore · ruolo: classification
            </div>
          </div>
        }
      />
      ...
      <TutorialChapter
        ...
        icon={Cpu}
        title="Intelligence"
        subtitle="Configurazione dell'IA: provider, voce, memoria, KB"
        ...
        tests={[
          {
            action: "Cambia provider/modello e salva.",
            expect: "Il profilo AI è salvato (univoco per utente) e usato nelle nuove risposte.",
          },
          {
            action: "Avvia una risposta vocale.",
            expect: "Audio riprodotto; se la voce primaria fallisce parte il fallback.",
          },
      ...
      <TutorialChapter
        ...
        icon={FlaskConical}
        title="Lab"
        subtitle="Prompt Lab, test e observability"
        path="/v2/lab"
        ...
        screenshotContent={
          <div className="flex gap-2 flex-wrap">
            {["Tests", "Observability", "Design System"].map((g) => (
              <span key={g} className="px-2 py-1 rounded-full bg-white/5 text-xs text-white/60">
                {g}
              </span>
            ))}
          </div>
      ...
      <TutorialChapter
        ...
        area="Config"
        icon={Settings}
        title="Config (Impostazioni)"
        subtitle="Tutta la configurazione del sistema"
        path="/v2/settings"
        reversed
        ...
        tests={[
          ...
          {
            action: "Apri la tab Development con '↳ Apri pagina'.",
            expect: "Si apre la pagina dedicata come per le altre sezioni.",
          },
        ]}
        screenshotContent={
          <div className="space-y-1.5">
            {["Generale", "Connessioni", "Provider AI", "Token AI", "Memoria AI", "Ruoli"].map((t, i) => (
              <div
                key={t}
                className={`px-3 py-1.5 rounded text-xs ${i === 1 ? "bg-primary/10 text-primary" : "bg-white/5 text-white/60"}`}
              >
                {t}
              </div>
            ))}
      ...
      <TutorialChapter
        ...
        area="Automazioni"
        icon={Workflow}
        title="Automazioni & Ciclo autonomo"
        subtitle="Come il sistema lavora da solo, sotto governance"
        ...
        tests={[
          {
            action: "Avvia una missione di outreach con cadenza.",
            expect: "I primi messaggi vengono prodotti (con revisione) e accodati.",
          },
          {
            action: "Lascia passare il tempo di follow-up su un no-reply.",
            expect: "Viene generato il follow-up previsto dalla cadenza, senza duplicati.",
          },
          { action: "Ricevi una risposta.", expect: "Viene classificata e il lead status aggiornato automaticamente." },
          {
            action: "Controlla i guardrail di costo.",
            expect: "L'enrichment automatico resta entro il gate; i token sono tracciati.",
          },
        ]}
        screenshotContent={
          <div className="space-y-2 text-xs text-white/60">
            <div className="flex items-center gap-2">
              <Search className="w-4 h-4 text-primary" /> Lead trovato → arricchito
            </div>
            <div className="flex items-center gap-2">
              <Mail className="w-4 h-4 text-primary" /> Primo messaggio inviato
            </div>
            <div className="flex items-center gap-2">
              <Workflow className="w-4 h-4 text-primary" /> Follow-up · classificazione · escalation
            </div>
          </div>
        }
      />
