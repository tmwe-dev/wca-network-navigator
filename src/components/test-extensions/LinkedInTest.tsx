...
export function LinkedInTest() {
  ...
  const runWithCooldown = useCallback(
    async (fn: () => Promise<void>, cooldownMs: number = LI_COOLDOWN_MS) => {
      setRunning(true);
  ...
  const testReadInbox = () =>
    runWithCooldown(async () => {
      ...
      if (opt) {
        ...
        log(
          `🤖 Optimus: piano [${tag}] · confidence ${conf} · ${threads.length} estratti in ${lat}${dropped}`,
          opt.cached ? "ok" : "info",
        );
      ...
      if (r?.success && threads.length) {
        ...
        threads.forEach((t) =>
          log(
            `  • ${t.name}: ${((t.lastMessage as string) || "").slice(0, 60) || "—"} ${t.unread ? "🔴" : ""}`,
            "info",
          ),
        );
        ...
        if (String(r?.error || "").includes("Timeout")) {
          log(
            "💡 Worker tab probabilmente in cold start. Premi 🚀 Pre-warm per scaldarla, poi riprova: sarà istantanea.",
            "warn",
          );
        }
      }
  ...
  const testSendMessage = () =>
    runWithCooldown(async () => {
      if (!sendUrl.trim()) {
        log("⚠️ URL fisso LinkedIn mancante: inseriscilo una volta e premi 📌 Fissa test", "warn");
        return;
      }
      if (!sendText.trim()) {
      ...
      try {
        ...
        if (installed !== required) {
          log(
            `⚠️ Estensione installata: v${installed} — richiesta v${required}. Rimuovi la vecchia da chrome://extensions e ricarica lo zip aggiornato.`,
            "warn",
          );
        } else {
      ...
      if (r?.success) {
        ...
        log(
          `  • clickMethod: ${String(diag.clickMethod ?? diag.method ?? "?")} | verified: ${String(diag.verified ?? "?")}`,
          "info",
        );
        log(`  • selectorUsed: ${String(diag.selectorUsed ?? "(regex)")}`, "info");
        log(
          `  • schemaSource: ${String(diag.schemaSource ?? "?")} | schemaAgeMs: ${String(diag.schemaAgeMs ?? "?")} | relearned: ${String(diag.relearned ?? false)}`,
          "info",
        );
        ...
        if (probeMatch) {
          ...
          try {
            ...
            log(
              `  • msg-overlay: ${probe.msgOverlay} | dialog aperti: ${probe.dialogs} | <main>: ${probe.hasMain}`,
              "info",
            );
        ...
        if (/composer_not_open_background_mode/i.test(e)) {
          log(
            "💡 Background mode: apri manualmente la chat LinkedIn col destinatario, lascia il box messaggio visibile, poi riprova. Oppure passa a modalità Interactive.",
            "warn",
          );
        } else if (/open_composer_failed_interactive/i.test(e)) {
          log(
            "💡 Interactive mode: il bottone 'Messaggia' non è stato trovato. Verifica captcha/login challenge LinkedIn.",
            "warn",
          );
        } else if (/composer_gate_failed_interactive/i.test(e)) {
          log(
            "💡 Interactive mode: composer non montato entro 30s. Verifica che la pagina profilo sia raggiungibile e non bloccata.",
            "warn",
          );
  ...
  const testSendWithMethod = (
    method: "physical_click" | "form_submit" | "keyboard_shortcut" | "cdp_physical_click" | "cdp_ctrl_enter",
    emoji: string,
    label: string,
  ) =>
    runWithCooldown(async () => {
      ...
      if (r?.success) {
        ...
        if (/no_existing_linkedin_tab/i.test(errStr)) {
          log(
            "💡 Apri almeno una tab LinkedIn (qualsiasi pagina) e riprova: l'estensione la userà in background.",
            "warn",
          );
        }
      }
  ...
  const testReadThread = () =>
    runWithCooldown(async () => {
      const targetThreadUrl = resolveThreadTarget();
      if (!targetThreadUrl) {
        log("⚠️ Inserisci un URL LinkedIn valido oppure usa il destinatario fisso già salvato", "warn");
        return;
      }
  ...
  const testBackfillThread = () =>
    runWithCooldown(async () => {
      ...
      const r = await liMsg(
        ...
        { threadUrl: targetThreadUrl, lastKnownText: stopText, maxScrolls: 20 },
        180000,
      );
      const messages = (r?.messages || []) as Array<Record<string, unknown>>;
      if (r?.success) {
        log(
          `✅ ${messages.length} messaggi totali · ${r.scrollCount || 0} scroll · foundLast=${r.foundLast ? "sì" : "no"} · method=${r.method || "?"}`,
          "ok",
        );
        messages
          .slice(0, 3)
          .forEach((m) =>
            log(`  📩 [${String(m.sender || "—").slice(0, 20)}] ${String(m.text || "").slice(0, 80)}`, "info"),
          );
  ...
  const pinFixedRecipient = () => {
    ...
    localStorage.setItem(
      LI_FIXED_RECIPIENT_KEY,
      JSON.stringify({ url: fixedUrl, savedAt: new Date().toISOString() } satisfies StoredLiTestRecipient),
    );
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            title="Apre la worker tab persistente su /messaging/. Una volta sola, poi tutte le azioni sono istantanee."
          >
            🚀 Pre-warm
          </Button>
          ...
          <Button onClick={testSyncCookie} disabled={running} size="sm">
            🍪 Sync Cookie
          </Button>
          <Button onClick={testAutoLogin} disabled={running} size="sm">
            🔐 Auto-Login
          </Button>
          <Button onClick={testSearchProfile} disabled={running} size="sm">
            🔎 Search
          </Button>
          <Button onClick={testReadInbox} disabled={running} size="sm">
            📨 Leggi Inbox
          </Button>
          <Button onClick={testDiagnosticDom} disabled={running} size="sm">
            🔬 Diagnostica DOM
          </Button>
          <Button
            ...
            size="sm"
            variant="outline"
            title="L'AI rilegge il DOM e salva schemi freschi per l'invio. Usalo se l'invio fallisce dopo un aggiornamento di LinkedIn."
          >
            🔧 Rimappa DOM invio
          </Button>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <span
            className={`text-xs font-mono px-2 py-0.5 rounded ${actionsLastHour > 15 ? "bg-red-500/20 text-red-400" : actionsLastHour > 8 ? "bg-yellow-500/20 text-yellow-400" : "bg-green-500/20 text-green-400"}`}
          >
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="flex-1"
        />
        <Button onClick={testExtractProfile} disabled={running} size="sm">
          👤 Estrai Profilo
        </Button>
      </div>
      ...
      <div className="p-3 rounded-lg border border-border bg-muted/30 space-y-2">
        <p className="text-xs font-medium text-muted-foreground">
          💬 Test Lettura Thread (diagnostico — NON salva nulla)
        </p>
          ...
          >
            ...
            {foundThreads.map((t, i) => (
              <option key={i} value={t.threadUrl || ""} disabled={!t.threadUrl}>
                {t.name}
                {t.threadUrl ? "" : " (threadUrl mancante dalla lettura inbox)"}
              </option>
            ))}
          </select>
        )}
        <div className="flex gap-2">
          ...
          <Button
            ...
            title={
              !resolveThreadTarget()
                ? "threadUrl mancante: seleziona un thread dalla Inbox o incolla un URL"
                : undefined
            }
          >
            💬 Leggi Thread
      ...
      <div className="p-3 rounded-lg border border-border bg-muted/30 space-y-2">
        ...
        {foundThreads.length > 0 && (
          ...
          >
            ...
            {foundThreads.map((t, i) => (
              <option key={i} value={t.threadUrl || ""}>
                {t.name}
                {t.threadUrl ? "" : " (no URL)"}
              </option>
            ))}
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={running || !sendUrl.trim() || !sendText.trim()}
            size="sm"
            variant="default"
            title={
              !sendUrl.trim()
                ? "URL fisso mancante: inseriscilo e premi Fissa test"
                : "Invia al destinatario fisso LinkedIn (pipeline backup 3.9.48)"
            }
          >
            📤 Invia LI
          </Button>
        ...
        <div className="flex flex-wrap gap-2 pt-1 border-t border-border/50 mt-1">
          ...
          <Button
            onClick={() => testSendWithMethod("physical_click", "🖱️", "Click fisico")}
            disabled={running}
            size="sm"
            variant="outline"
            title="pointerdown/mousedown/click con coordinate reali"
          >
            🖱️ Click fisico
          </Button>
          <Button
            onClick={() => testSendWithMethod("form_submit", "📋", "Form submit")}
            disabled={running}
            size="sm"
            variant="outline"
            title="form.requestSubmit() sul .msg-form"
          >
            📋 Form submit
          </Button>
          <Button
            onClick={() => testSendWithMethod("keyboard_shortcut", "⌨️", "Ctrl+Enter")}
            disabled={running}
            size="sm"
            variant="outline"
            title="Ctrl+Enter (Cmd+Enter su Mac)"
          >
            ⌨️ Ctrl+Enter
          </Button>
          <Button
            onClick={() => testSendWithMethod("cdp_physical_click", "🎯", "CDP click")}
            disabled={running}
            size="sm"
            variant="outline"
            title="Chrome DevTools Protocol: mousePressed/mouseReleased sul bottone Invia"
          >
            🎯 CDP click
          </Button>
          <Button
            onClick={() => testSendWithMethod("cdp_ctrl_enter", "⌘", "CDP Ctrl+Enter")}
            disabled={running}
            size="sm"
            variant="outline"
            title="Chrome DevTools Protocol: Ctrl/Cmd+Enter nativo"
          >
            ⌘ CDP Ctrl+Enter
          </Button>
        </div>
      ...
      {quality && (
        <div className="p-3 rounded-lg border border-border bg-muted/30 space-y-2 text-xs">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div>
              <span className="text-muted-foreground">Thread visti:</span> <b>{quality.rawCandidates}</b>
            </div>
            <div>
              <span className="text-muted-foreground">Thread accettati:</span> <b>{quality.threadsAccepted}</b>
            </div>
            <div>
              <span className="text-muted-foreground">Messaggi salvati:</span> <b>{quality.messagesAccepted}</b>
            </div>
            <div>
              <span className="text-muted-foreground">Confidence media:</span>{" "}
              <b>{(quality.avgConfidence * 100).toFixed(0)}%</b>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {Object.entries(quality.methods)
              .filter(([, v]) => v > 0)
              .map(([k, v]) => (
                <span key={k} className="px-2 py-0.5 rounded bg-primary/10 text-primary">
                  {k}: {v}
                </span>
              ))}
          </div>
          {(Object.entries(quality.threadsDropped).some(([, v]) => v > 0) ||
            Object.entries(quality.messagesDropped).some(([, v]) => v > 0)) && (
            <div className="flex flex-wrap gap-2">
              {Object.entries(quality.threadsDropped)
                .filter(([, v]) => v > 0)
                .map(([k, v]) => (
                  <span key={`t-${k}`} className="px-2 py-0.5 rounded bg-yellow-500/10 text-yellow-500">
                    thread/{k}: {v}
                  </span>
                ))}
              {Object.entries(quality.messagesDropped)
                .filter(([, v]) => v > 0)
                .map(([k, v]) => (
                  <span key={`m-${k}`} className="px-2 py-0.5 rounded bg-yellow-500/10 text-yellow-500">
                    msg/{k}: {v}
                  </span>
                ))}
            </div>
          ...
          {quality.warnings.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {quality.warnings.map((w) => (
                <span key={w} className="px-2 py-0.5 rounded bg-orange-500/10 text-orange-500">
                  ⚠ {w}
                </span>
              ))}
            </div>
