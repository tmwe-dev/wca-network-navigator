...
export default function ActiveMissionsPanel() {
  ...
  const handlePause = async (missionId: string) => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: queryKeys.missions.activeActions() });
      toast.success("Missione in pausa");
    } catch {
  ...
  const handleResume = async (missionId: string) => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: queryKeys.missions.activeActions() });
      toast.success("Missione ripresa");
    } catch {
  ...
  return (
    <div className="space-y-2">
      ...
      {Array.from(grouped.entries()).map(([missionId, missionActions]) => {
        ...
        const currentAction =
          missionActions.find((a) => a.status === "executing") || missionActions.find((a) => a.status === "approved");
        ...
        return (
          <div key={missionId} className="p-2.5 rounded-lg border border-border/20 bg-muted/10 space-y-1.5">
            <div className="flex items-center gap-2">
              <StatusIcon
                className={`w-3.5 h-3.5 ${currentAction ? statusColor[currentAction.status] : "text-emerald-500"} ${currentAction?.status === "executing" ? "animate-spin" : ""}`}
              />
            ...
            <div className="flex items-center gap-1 pt-0.5">
              {paused ? (
                <Button
                  ...
                  onClick={() => handleResume(missionId)}
              ...
              <Button
                size="sm"
                variant="ghost"
                className="h-5 text-[9px] gap-1 px-1.5 text-destructive"
                onClick={() => handleCancel(missionId)}
                disabled={acting === missionId}
              >
