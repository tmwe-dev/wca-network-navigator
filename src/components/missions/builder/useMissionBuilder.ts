...
export function useMissionBuilder() {
    ...
    async (text: string) => {
      ...
      try {
        ...
        setMessages((prev) => {
          const last = prev[prev.length - 1];
          if (last?.role === "assistant")
            return prev.map((m, i) => (i === prev.length - 1 ? { ...m, content: cleanText, widgets } : m));
  ...
  const generatePlan = useCallback((): MissionPlan => {
    ...
    if (stepData.deepSearch?.enabled)
      actions.push({
        type: "deep_search",
        label: `Deep Search su ${countries.length} paesi`,
        details: "Scraping e arricchimento dati",
      });
    ...
    if (stepData.schedule !== "immediate")
      actions.push({
        type: "schedule",
        label: "Pianifica invio",
        details: `Programmato per ${stepData.scheduleDate || "data da definire"}`,
      });
    return {
  ...
  const handlePlanApprove = useCallback(async () => {
    ...
    try {
      ...
      const mission = await insertOutreachMission({
        ...
        title,
        status: "active",
        target_filters: toJsonValue(stepData.targets || {}),
        channel: stepData.channel || "email",
        total_contacts: totalContacts,
        agent_assignments: toJsonValue(stepData.agents || []),
        schedule_config: toJsonValue({ type: stepData.schedule, date: stepData.scheduleDate }),
        idempotency_key: pendingPlan.idempotencyKey,
        plan_json: toJsonValue(pendingPlan),
  ...
  const handlePlanCancel = useCallback(() => {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "assistant", content: "❌ Piano annullato. Puoi modificare la configurazione e riprovare." },
    ]);
  }, []);
