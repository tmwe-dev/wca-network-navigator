...
export function AttachmentStep({ data, onChange }: MissionStepProps) {
  ...
  useEffect(() => {
    findEmailTemplatesShort().then((d) => {
      if (d) setTemplates(d as TemplateEntry[]);
    });
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        {templates.length === 0 ? (
          <p className="text-xs text-muted-foreground italic">
            Nessun template caricato. Vai su Impostazioni → Template.
          </p>
        ) : (
          <div className="flex flex-wrap gap-2 max-h-[100px] overflow-y-auto">
            {templates.map((t) => (
              <button
                key={t.id}
                onClick={() => {
                  const ids = att.templateIds.includes(t.id)
                    ? att.templateIds.filter((x) => x !== t.id)
                    : [...att.templateIds, t.id];
                  set({ templateIds: ids });
                }}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                  att.templateIds.includes(t.id)
      ...
      <div>
        ...
        {images.length === 0 ? (
          <p className="text-xs text-muted-foreground italic">
            Nessuna immagine. Caricale in Email Composer → tab Immagini.
          </p>
        ) : (
          <div className="flex gap-2 overflow-x-auto pb-2">
            {images.slice(0, 10).map((img) => (
              <button
                key={img.name}
                onClick={() => {
                  const ids = att.imageIds.includes(img.name)
                    ? att.imageIds.filter((x) => x !== img.name)
                    : [...att.imageIds, img.name];
                  set({ imageIds: ids });
                }}
                className={`flex-shrink-0 w-16 h-16 rounded-lg border-2 overflow-hidden transition-all ${
                  att.imageIds.includes(img.name)
                    ? "border-primary ring-2 ring-primary/30"
                    : "border-border hover:border-primary/50"
                }`}
              >
                <OptimizedImage src={img.url} alt={img.name} className="w-full h-full object-cover" />
              </button>
            ))}
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {att.links.map((l, i) => (
              <Badge
                ...
                onClick={() => set({ links: att.links.filter((_, j) => j !== i) })}
              >
                {l.substring(0, 30)}... ✕
              </Badge>
            ))}
      ...
      <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/30 cursor-pointer">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            Aggiunge automaticamente la firma visiva configurata nelle impostazioni
          </div>
        </div>
      </label>
    </div>
