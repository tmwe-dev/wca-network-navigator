...
export function CommunicationStep({ data, onChange }: MissionStepProps) {
  ...
  useEffect(() => {
    findActiveEmailPrompts().then((d) => {
      if (d) setEmailTypes(d as unknown as EmailPromptEntry[]);
    });
  }, []);
  ...
  const modes: { key: CommunicationConfig["templateMode"]; label: string; desc: string }[] = [
    {
      key: "ai_generate",
      label: "🤖 AI genera in tempo reale",
      desc: "L'AI crea un messaggio personalizzato per ogni contatto basandosi sui dati",
    },
    {
      key: "preset",
      label: "📋 Scegli un tipo email",
      desc: "Usa un modello di comunicazione già configurato (es. Hook → CTA)",
    },
    {
      key: "custom",
      label: "✏️ Scrivi tu il modello",
      desc: "Definisci oggetto e corpo — l'AI li adatterà per ogni destinatario",
    },
  ];
    ...
    <div className="space-y-4">
      ...
      {comm.templateMode === "custom" && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            💡 L'AI adatterà questo modello per ogni destinatario, personalizzando il tono e i dettagli.
          </p>
        </div>
      )}
      ...
      {comm.templateMode === "ai_generate" && (
        <div className="bg-muted/20 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
          <p>
            💡 <strong>L'AI genererà un messaggio unico per ogni contatto</strong>, usando:
          </p>
