  ...
  const toggle = (code: string) => {
    ...
    onChange({
      ...
      targets: {
        ...data.targets,
        countries: cur,
        types: data.targets?.types || [],
        ratings: data.targets?.ratings || [],
        hasEmail: data.targets?.hasEmail ?? true,
      },
    });
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto">
        {countries.length === 0 && (
          <p className="text-xs text-muted-foreground italic">Chiedi all'AI di caricare le statistiche...</p>
        )}
        {countries.map((c) => (
          <button
            key={c.code}
      ...
      {selected.length > 0 && (
        <p className="text-xs text-primary font-medium">
          {selected.length} paesi —{" "}
          {countries.filter((c) => selected.includes(c.code)).reduce((s, c) => s + c.count, 0)} contatti
        </p>
      )}
    </div>
  );
