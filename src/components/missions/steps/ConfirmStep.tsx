    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <SummaryCard
          label="Qualità"
          value={
            data.toneConfig?.quality === "premium"
              ? "💎 Premium"
              : data.toneConfig?.quality === "fast"
                ? "⚡ Rapida"
                : "✨ Standard"
          }
        />
        ...
        <SummaryCard
          label="Lingua"
          value={data.toneConfig?.language === "auto" ? "🌍 Auto" : data.toneConfig?.language || "auto"}
        />
      </div>
      ...
      {data.communication && (
        <div className="bg-muted/30 rounded-lg p-3">
          ...
          <div className="text-sm">
            ...
            {data.communication.templateMode === "preset" &&
              `📋 ${data.communication.emailType || "Tipo email selezionato"}`}
            {data.communication.templateMode === "custom" &&
              `✏️ Modello personalizzato: "${data.communication.customSubject || "..."}"`}
          </div>
        </div>
      )}

      {data.attachments &&
        (data.attachments.templateIds.length > 0 ||
          data.attachments.imageIds.length > 0 ||
          data.attachments.links.length > 0) && (
          <div className="bg-muted/30 rounded-lg p-3">
            <div className="text-xs text-muted-foreground mb-1">Allegati</div>
            <div className="flex gap-3 text-xs">
              {data.attachments.templateIds.length > 0 && (
                <span>📎 {data.attachments.templateIds.length} documenti</span>
              )}
      ...
      {data.agents && data.agents.length > 0 && (
        <div className="bg-muted/30 rounded-lg p-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {data.agents.map((a) => (
              <Badge key={a.agentId} variant="secondary">
                {a.agentName}
              </Badge>
            ))}
          </div>
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 text-xs text-primary">
        Crediti stimati: ~
        {totalContacts * (data.toneConfig?.quality === "premium" ? 15 : data.toneConfig?.quality === "fast" ? 3 : 8)}{" "}
        crediti
      </div>
    </div>
  );
