...
export function AgentStep({ data, onChange, agentsList }: MissionStepProps) {
  ...
  const toggleAgent = (agent: (typeof agents)[0]) => {
    ...
    if (exists) {
      ...
      onChange({
        ...data,
        agents: [...assigned, { agentId: agent.id, agentName: agent.name, countries: agent.territories }],
      });
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {agents.length === 0 && (
        <p className="text-xs text-muted-foreground italic">Nessun agente configurato. Vai su Agenti per crearne.</p>
      )}
