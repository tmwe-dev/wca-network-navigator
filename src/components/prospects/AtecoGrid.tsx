    ...
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div className="space-y-2">
        <FilterMultiSelect
          ...
          options={grid.regionOptions}
          selected={regionFilter}
          onToggle={grid.handleRegionToggle}
          onClear={grid.handleRegionClear}
        />
        <FilterMultiSelect
          label="Provincia"
          placeholder="Tutte le province"
          options={grid.provinceOptions}
          selected={provinceFilter}
          onToggle={grid.handleProvinceToggle}
          onClear={grid.handleProvinceClear}
        />
      </div>
      ...
      >
        <div className="flex items-center gap-2">
          <span className={`text-[11px] font-medium ${grid.onlyInDb ? "text-primary" : "text-muted-foreground"}`}>
            Solo nel DB
          </span>
          {grid.totalInDb > 0 && (
            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">
              {grid.totalInDb.toLocaleString()}
            </span>
          )}
        </div>
      ...
      {(() => {
        const hasRankFilter =
          rankingFilters &&
          (rankingFilters.rank_volume_min > 0 ||
            rankingFilters.rank_valore_min > 0 ||
            rankingFilters.rank_intl.length > 0 ||
            rankingFilters.rank_score_min > 0);
        if (!hasRankFilter || !onSelectMultiple) return null;
        const visibleCodes = groups
          .filter((g) => passesRankingFilter(getAtecoRank(g.codice), rankingFilters))
          .map((g) => g.codice);
          ...
          {grid.filteredSections.length === 0 ? (
            ...
            grid.filteredSections.map((section) => (
              <AtecoSectionRow
                ...
                selectedSet={grid.selectedSet}
                nodeCount={grid.nodeCount}
                onlyInDb={grid.onlyInDb}
                rankingFilters={rankingFilters}
                onToggleExpand={grid.toggleExpand}
                onToggleBranch={grid.toggleBranch}
                onToggleCode={onToggle}
              />
            ))
          )}
        </div>
