    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-2 space-y-3">
        <div className="flex flex-wrap gap-2">
          <button onClick={() => onToggleRegion("__ALL__")} className={chip(regions.length === 0)}>
            🇮🇹 Tutta Italia
          </button>
          ...
          <div className={`${card} p-3 space-y-2`}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {visibleProvinces.map((p) => (
                <button
                  key={p.sigla}
                  onClick={() => onToggleProvince(p.sigla)}
                  className={`px-2 py-1 rounded-lg text-[11px] border transition-all ${provinces.includes(p.sigla) ? "bg-emerald-500/20 text-success border-emerald-500/30" : "bg-muted/20 text-muted-foreground border-border hover:border-primary/20"}`}
                >
