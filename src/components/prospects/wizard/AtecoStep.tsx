...
export function AtecoStep({
  ...
  const btn = (variant: "primary" | "ghost") => {
    if (variant === "primary")
      return `flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all bg-primary/20 text-primary hover:bg-primary/30 border border-primary/30`;
      ...
      <div className="flex-shrink-0 px-4 pt-4 pb-2 space-y-2">
        ...
        {atecoCodes.length > 0 && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs bg-primary/10 border border-primary/20 text-primary">
            ...
            <span>
              {atecoCodes.length} codici selezionati: {atecoCodes.slice(0, 5).join(", ")}
              {atecoCodes.length > 5 ? `...+${atecoCodes.length - 5}` : ""}
            </span>
          </div>
        )}
      </div>
        ...
        {ATECO_SECTIONS.map((section) => {
          ...
          return (
            <div key={section.codice} className={`${card} overflow-hidden`}>
              <button
                onClick={() => onExpandSection(isExpanded ? null : section.codice)}
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onToggleSection(section.codice);
                    }}
                    className={`text-[10px] px-2 py-1 rounded-lg font-medium transition-all border ${allSelected ? "bg-primary/20 text-primary border-primary/30" : "bg-muted/30 text-muted-foreground border-border hover:border-primary/20"}`}
                  >
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {groups.map((g) => {
                      ...
                      return (
                        <button
                          key={g.codice}
                          onClick={() => onToggleCode(g.codice)}
                          className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] border transition-all ${active ? "bg-primary/20 text-primary border-primary/30" : "bg-muted/20 text-muted-foreground border-border hover:border-primary/20"}`}
                        >
