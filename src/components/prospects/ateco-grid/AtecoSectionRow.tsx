...
export function AtecoSectionRow({
  ...
  const sRanks = childDivisions(section.codice)
    .map((d) => getAtecoRank(d.codice))
    .filter(Boolean);
  const sAvgScore =
    sRanks.length > 0 ? Math.round((sRanks.reduce((s, r) => s + calcScore(r!), 0) / sRanks.length) * 10) / 10 : 0;
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={() => onToggleExpand(section.codice)}>
      <div
        className={`flex items-center gap-1 rounded-xl px-2 py-1.5 transition-all ${sPriorityClass} ${isDark ? "hover:bg-white/[0.04]" : "hover:bg-muted/50"}`}
      >
        <CollapsibleTrigger className="flex items-center gap-2 flex-1 min-w-0 text-left">
          ...
          {isOpen ? (
            <ChevronDown className={`w-3.5 h-3.5 shrink-0 ${th.dim}`} />
          ) : (
            <ChevronRight className={`w-3.5 h-3.5 shrink-0 ${th.dim}`} />
          )}
        </CollapsibleTrigger>
        {sAvgScore > 0 && (
          <span
            className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border shrink-0 ${scoreBg(sAvgScore, isDark)} ${scoreColor(sAvgScore, isDark)}`}
            title={`Score medio: ${sAvgScore}`}
          >
            {sAvgScore.toFixed(0)}
          </span>
        )}
      ...
      <CollapsibleContent>
        <div
          className="ml-4 border-l border-dashed pl-2 space-y-0.5"
          style={{ borderColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)" }}
        >
          {childDivisions(section.codice)
            .filter((div) =>
              childGroups(div.codice).some(
                (g) =>
                  passesRankingFilter(getAtecoRank(g.codice), rankingFilters) &&
                  (!onlyInDb || (nodeCount.get(g.codice) || 0) > 0),
              ),
            )
            .map((div) => {
              ...
              return (
                <Collapsible key={div.codice} open={isDivOpen} onOpenChange={() => onToggleExpand(div.codice)}>
                  <div
                    className={`flex items-center gap-1 rounded-lg px-2 py-1 transition-all ${dPriorityClass} ${isDark ? "hover:bg-white/[0.04]" : "hover:bg-muted/50"}`}
                  >
                    <CollapsibleTrigger className="flex items-center gap-2 flex-1 min-w-0 text-left">
                      {isDivOpen ? (
                      ...
                      {isDivOpen ? (
                        <ChevronDown className={`w-3 h-3 shrink-0 ${th.dim}`} />
                      ) : (
                        <ChevronRight className={`w-3 h-3 shrink-0 ${th.dim}`} />
                      )}
                    </CollapsibleTrigger>
                    {dScore > 0 && (
                      <span
                        className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border shrink-0 ${scoreBg(dScore, isDark)} ${scoreColor(dScore, isDark)}`}
                        title={dRank ? `Vol:${dRank.volume} Val:${dRank.valore} ${dRank.intl} — ${dRank.note}` : ""}
                      >
                        {dScore.toFixed(0)}
                      </span>
                    )}
                    ...
                    <div className="ml-5 space-y-0.5">
                      {childGroups(div.codice)
                        ...
                        .map((grp) => {
                          ...
                          const gPriorityClass =
                            gHighPriority && !isSel ? "bg-primary/[0.06] border-l-2 border-primary/40" : "";
                          return (
                            <button
                              key={grp.codice}
                              onClick={() => onToggleCode(grp.codice)}
                              ...
                              title={
                                gRank
                                  ? `Vol:${"★".repeat(gRank.volume)}  Val:${"★".repeat(gRank.valore)}  ${gRank.intl}\n${gRank.note}`
                                  : ""
                              }
                            >
                              <div
                                className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${isSel ? "bg-primary border-primary" : isDark ? "border-white/15" : "border-border"}`}
                              >
                              ...
                              {gScore > 0 && (
                                <span
                                  className={`text-[8px] font-mono font-bold px-1 py-0.5 rounded border shrink-0 ${scoreBg(gScore, isDark)} ${scoreColor(gScore, isDark)}`}
                                >
                                  {gScore.toFixed(0)}
                                </span>
                              )}
