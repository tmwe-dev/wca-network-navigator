...
export function useAtecoGrid(opts: UseAtecoGridOpts) {
  ...
  const filteredSections = useMemo(() => {
    ...
    const sectionHasVisibleDiv = (secCode: string) => childDivisions(secCode).some((d) => divHasVisibleGroup(d.codice));
    ...
    if (search && search.length >= 2) {
      ...
      result = result.filter((s) => {
        ...
        return childDivisions(s.codice).some(
          (d) => matchesSearch(d, q) || childGroups(d.codice).some((g) => matchesSearch(g, q)),
        );
      });
    }
      ...
      for (const s of sections) {
        for (const d of childDivisions(s.codice)) {
          if (
            ...
            childGroups(d.codice).some(
              (g) => g.descrizione.toLowerCase().includes(q) || g.codice.toLowerCase().includes(q),
            )
          ) {
  ...
  const provinceOptions = useMemo(() => {
    const filtered =
      regionFilter.length > 0 ? PROVINCE_ITALIANE.filter((p) => regionFilter.includes(p.regione)) : PROVINCE_ITALIANE;
