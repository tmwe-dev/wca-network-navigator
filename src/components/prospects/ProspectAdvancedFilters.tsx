      ...
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <Star
              className={`w-4 h-4 ${
                star <= value ? "text-primary fill-primary" : isDark ? "text-white/15" : "text-muted-foreground"
              }`}
            />
          </button>
        ))}
      </div>
      {value > 0 && <span className="text-[9px] font-mono text-primary">≥{value}</span>}
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={`max-w-md p-0 border-0 overflow-hidden bg-card`}>
        ...
        <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto text-foreground">
          ...
          <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground">📊 Parametri Aziendali</p>
          ...
          <div className="border-t pt-4 space-y-3 border-border">
            <p className="text-[10px] uppercase tracking-wider font-bold text-emerald-500">📞 Disponibilità Contatti</p>
          ...
          <div className="border-t pt-4 space-y-3 border-border">
            <p className="text-[10px] uppercase tracking-wider font-bold text-primary">⭐ Ranking ATECO</p>
