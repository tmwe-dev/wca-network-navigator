...
export function ProspectImporter({ isDark, atecoCodes, regions, provinces, filters }: Props) {
  ...
  const dedupAgainstDb = async (results: SearchResult[]): Promise<SearchResult[]> => {
    ...
    const dbPivas = new Set((existing || []).map((e: { partita_iva?: string; company_name?: string }) => e.partita_iva?.trim()).filter(Boolean));
    const dbNames = new Set((existing || []).map((e: { partita_iva?: string; company_name?: string }) => e.company_name?.toLowerCase().trim()).filter(Boolean));
