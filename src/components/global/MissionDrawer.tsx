...
export function MissionDrawer({ open, onOpenChange }: MissionDrawerProps) {
  ...
  const startResize = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setDrawerWidth(Math.max(320, Math.min(window.innerWidth - ev.clientX, window.innerWidth * 0.8)));
    };
    const onUp = () => {
          ...
          : isExploreMap
            ...
            : isExploreSherlock
              ...
              : isExplore
                ? "Esplora"
                : "Mission Control";
  const contextSubtitle = isOutreach
    ? "Email, destinatari e invio"
    : isNetwork
      ? "Deep Search e arricchimento"
      : isCRM
        ? "Contatti e comunicazione"
        : isSettings
          ? "Azioni rapide"
          : isExploreMap
            ? "Naviga countries e partner sul globo"
            : isExploreSherlock
              ? "Indagini multi-livello su company / contact"
              : isExplore
                ? "Discovery, mappa e investigazione"
                : "Configura e vai";
    ...
    <PageErrorBoundary>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent
          side="right"
          className={cn(
            "p-0 flex flex-col border-l border-primary/10 bg-background/95 backdrop-blur-xl",
            !drawerWidth && "w-[90vw] sm:w-[520px] md:w-[600px] lg:w-[680px] sm:max-w-[700px]",
          )}
          style={drawerWidth ? { width: drawerWidth, maxWidth: "80vw" } : undefined}
        >
          {/* Header */}
            ...
            {(isOutreach || (!isNetwork && !isCRM && !isSettings && !isExplore)) && (
              <>
                <DrawerPresetManager
                  ...
                  onSetQuality={m.setQuality}
                />
                <div className="flex items-center gap-2">
                  <ActionIcon
                    icon={Target}
                    label="Obiettivo"
                    tooltip="Cosa vuoi ottenere con questa missione (es. fissare meeting, presentare il servizio)"
                    active={!!m.goal}
                  ...
                  <ActionIcon
                    icon={FileText}
                    label="Proposta"
                    tooltip="Il messaggio base / value proposition che l'AI userà come scheletro per le email"
                    active={!!m.baseProposal}
                    activeName={
                      m.baseProposal
                        ? m.baseProposal.length > 18
                          ? m.baseProposal.slice(0, 18) + "…"
                          : m.baseProposal
                        : undefined
                    }
                    color="from-muted-foreground/25 to-muted-foreground/5"
                ...
                <div className="flex items-center gap-2">
                  <ActionIcon
                    ...
                    label="Contesto"
                    tooltip="Da dove arriva il lead (fiera, referral, online…) — guida tono e apertura"
                    active={!!m.context}
                    activeName={
                      m.context ? (m.context.length > 18 ? m.context.slice(0, 18) + "…" : m.context) : undefined
                    }
                    color="from-primary/25 to-primary/5"
                    iconColor="text-primary"
                  ...
                  <ActionIcon
                    icon={Paperclip}
                    label="Docs"
                    tooltip="Allegati che l'AI può citare o inviare (PDF, presentazioni, listini)"
                  ...
                  <ActionIcon
                    icon={Link2}
                    label="Link"
                    tooltip="URL di riferimento (sito, case study, calendario booking)"
          ...
          <DrawerGoalEditor
            goalOpen={goalDialogOpen}
            proposalOpen={proposalDialogOpen}
            contextOpen={contextDialogOpen}
            onGoalOpenChange={setGoalDialogOpen}
            onProposalOpenChange={setProposalDialogOpen}
            onContextOpenChange={setContextDialogOpen}
            goal={m.goal}
            baseProposal={m.baseProposal}
            context={m.context}
            onSetGoal={m.setGoal}
            onSetBaseProposal={m.setBaseProposal}
            onSetContext={m.setContext}
          />
          <DrawerDocumentsPanel
            documents={m.documents}
            referenceLinks={m.referenceLinks}
            uploading={m.uploading}
            docsOpen={docsDialogOpen}
            linksOpen={linksDialogOpen}
            onDocsOpenChange={setDocsDialogOpen}
            onLinksOpenChange={setLinksDialogOpen}
            onUpload={m.upload}
            onRemoveDocument={m.removeDocument}
            onSetReferenceLinks={m.setReferenceLinks}
          />
...
function ActionIcon({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "w-8 h-8 rounded-lg bg-gradient-to-br flex items-center justify-center shrink-0 shadow-inner",
          color,
        )}
      >
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        {!active && !activeName && (count === undefined || count === 0) && (
          <p className="text-[10px] text-muted-foreground">Non impostato</p>
        )}
      </div>
