    ...
    <div className="flex-shrink-0 pb-1.5 mb-1 border-b border-border/30 space-y-1">
          ...
          <>
            <div className="flex-1 flex gap-1 overflow-x-auto scrollbar-none min-w-0">
              {recipients.map((r, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[9px] font-medium border border-primary/20 shrink-0 max-w-[140px]"
                >
            ...
            <Button
              ...
              size="sm"
              variant="ghost"
              className="h-5 px-1.5 text-[9px] text-muted-foreground shrink-0"
            >
      ...
      <div className="relative">
        ...
        <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {shouldSearch && <span className="text-[9px] text-muted-foreground tabular-nums">{currentCount}</span>}
          <Popover>
            <PopoverTrigger asChild>
              <button
                className={cn(
                  "p-1 rounded hover:bg-muted/60 transition-colors",
                  state.hideHolding || state.originFilter !== "all"
                    ? "text-primary"
            ...
            <PopoverContent className="w-56 p-3 space-y-3" align="end" side="bottom">
              ...
              {state.tab === "contacts" && originOptions.length > 0 && (
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => dispatch({ type: "SET_ORIGIN_FILTER", origin: v })}
                  >
                    <SelectTrigger className="h-7 text-[11px]">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={state.hideHolding}
                  onCheckedChange={(v) => dispatch({ type: "SET_HIDE_HOLDING", value: v })}
                  className="scale-75"
                />
              </div>
