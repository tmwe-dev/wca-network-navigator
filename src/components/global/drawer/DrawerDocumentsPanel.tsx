    ...
    <>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm flex items-center gap-2">
              <Paperclip className="w-4 h-4 text-primary" /> Documenti ({props.documents.length})
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {props.documents.map((doc) => (
              ...
              >
                ...
                <button
                  onClick={() => props.onRemoveDocument(doc.id)}
            ...
            {props.documents.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">Nessun documento</p>
            )}
          </div>
          <input
            ref={fileRef}
            ...
            className="hidden"
            onChange={(e) => {
              if (e.target.files?.[0]) props.onUpload(e.target.files[0]);
            }}
          />
          <Button
            variant="outline"
      ...
      <Dialog open={props.linksOpen} onOpenChange={props.onLinksOpenChange}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm flex items-center gap-2">
              <Link2 className="w-4 h-4 text-emerald-500" /> Link ({props.referenceLinks.length})
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {props.referenceLinks.map((url, i) => (
              ...
              >
                ...
                <button
                  onClick={() => props.onSetReferenceLinks((prev) => prev.filter((_, j) => j !== i))}
                  className="opacity-0 group-hover:opacity-100 p-1 hover:bg-destructive/10 rounded"
                >
                  <X className="w-3.5 h-3.5 text-destructive" />
                </button>
            ...
            {props.referenceLinks.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">Nessun link</p>
            )}
          </div>
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" variant="outline" onClick={addLink} className="h-9 px-3">
