    ...
    return (
      <>
        ...
        <button
          ...
          disabled={state.playingIdx != null}
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          <div className="flex items-center gap-0.5 bg-secondary/50 rounded-lg p-0.5 text-[10px]">
            ...
            <button
              ...
              aria-label="Modalità strategica"
          ...
          {messages.length > 0 && (
            <button
              ...
              className="p-1.5 rounded-lg transition-colors hover:bg-secondary/50 text-muted-foreground"
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto px-4 py-3 space-y-3">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-4 py-8">
              ...
              <p className="text-xs text-center text-muted-foreground">
                Chiedi di scaricare partner per paese,
                <br />
                aggiornare profili o verificare lo stato.
              </p>
              <div className="flex flex-wrap gap-1.5 justify-center">
                {QUICK_PROMPTS.map((q) => (
                  <button
                    ...
                    className="text-[10px] px-2.5 py-1 rounded-full border border-border text-muted-foreground hover:bg-muted/30 hover:text-foreground transition-colors"
                  >
                    {q}
                  </button>
                ))}
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[85%] rounded-xl px-3 py-2 text-xs leading-relaxed ${msg.role === "user" ? "bg-primary/20 text-foreground border border-primary/20" : "bg-muted/30 text-foreground border border-border/30"}`}
              >
          ...
          <div className="flex items-end gap-2">
            ...
            {speech.hasSpeechAPI && (
              <button
                ...
                aria-label={speech.listening ? "Stop dettatura" : "Detta con microfono"}
