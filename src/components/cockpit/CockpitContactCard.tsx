...
export function CockpitContactCard({
  ...
  const hasLinkedinLive = !!(
    enrichmentState?.linkedinProfile &&
    (enrichmentState.scrapingPhase === "reviewing" ||
      enrichmentState.scrapingPhase === "generating" ||
      enrichmentState.scrapingPhase === "idle")
  );
  const isAiProcessed = !!contact.deepSearchAt;
  const contactHeadline = (contact.enrichmentData?.contact_profile as Record<string, unknown> | undefined)
    ?.linkedin_title as string | undefined;
  const hasEnrichmentData = !!(
    contact.enrichmentData &&
    (contact.enrichmentData.contact_profile ||
      contact.enrichmentData.company_profile ||
      contact.enrichmentData.linkedin_url ||
      contact.enrichmentData.linkedin_profile_url)
  );
              ...
              <div className="mt-3 pt-3 border-t border-border/40 space-y-3">
                ...
                {hasEnrichmentData ? (
                  <ContactEnrichmentCard
                    enrichmentData={(contact.enrichmentData ?? null) as never}
