...
export function CockpitContactCard({ contact, flag, index, onDragStart, onDragEnd }: CockpitContactCardProps) {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className="group relative rounded-xl border border-border/50 bg-card/70 backdrop-blur-xl p-3.5 cursor-grab active:cursor-grabbing transition-shadow duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/30"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-muted-foreground/70">{contact.role}</div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              {contact.channels.map(ch => {
                ...
                return Icon ? (
                  <span key={ch} className="p-1 rounded-md bg-muted/50 text-muted-foreground/60 hover:text-primary hover:bg-primary/10 transition-colors">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button className="p-1 rounded-md text-muted-foreground/50 hover:text-primary hover:bg-primary/10 transition-colors" title="Deep Search">
                <Search className="w-3 h-3" />
              </button>
              <button className="p-1 rounded-md text-muted-foreground/50 hover:text-chart-3 hover:bg-chart-3/10 transition-colors" title="AI Generate">
