  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      if (attachments.length + accepted.length >= MAX_ATTACHMENTS) {
        toast.error(`Massimo ${MAX_ATTACHMENTS} allegati`);
        break;
      }
      totalAfter += f.size;
      if (totalAfter > MAX_TOTAL_BYTES) {
        toast.error("Limite 20MB totali superato");
        break;
      }
      accepted.push(f);
    }
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="h-7 gap-1.5 relative">
            ...
            {links.length > 0 && (
              <span className="ml-1 text-[9px] px-1 rounded-full bg-primary text-primary-foreground">
                {links.length}
              </span>
            )}
          </Button>
        ...
        <PopoverContent className="w-80 p-3 space-y-2" align="end">
              ...
              {links.map((l, i) => (
                <div key={i} className="flex items-center gap-1.5 text-xs bg-muted/40 rounded px-2 py-1">
                  ...
                  <span className="flex-1 truncate">
                    <b>{l.label}</b> · <span className="text-muted-foreground">{l.url}</span>
                  </span>
          ...
          <Input
            ...
            placeholder="Etichetta (es: Catalogo)"
          ...
          <p className="text-[10px] text-muted-foreground">
            L'AI li citerà nel testo alla prossima rigenerazione/Migliora.
          </p>
        </PopoverContent>
      </Popover>
