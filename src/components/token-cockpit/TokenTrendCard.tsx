  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-border last:border-b-0">
      ...
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-foreground">{formatTokenCount(current)}</span>
        <div className="flex items-center gap-1">
          ...
          <Badge variant={isPositive ? "secondary" : "default"} className="text-xs">
            {isPositive ? "+" : ""}
...
export function TokenTrendCard() {
  ...
  return (
    <Card className="p-6">
      <h3 className="text-lg font-semibold mb-4 text-foreground">Tendenza utilizzo</h3>
      <div>
        <TrendRow label="Oggi vs Ieri" current={trendDataMemoized.today} previous={trendDataMemoized.yesterday} />
        <TrendRow
