        ...
        <div className="flex-1 min-w-0">
                ...
                {classification.sentiment && (
                  <span
                    className={cn(
                      ...
                      SENTIMENT_COLORS[classification.sentiment] || "bg-gray-400",
                    )}
                  />
                )}
              </div>
          ...
          {classification.action_suggested && (
            <div className="flex items-start gap-1.5 mb-2 p-2 bg-orange-500/10 border border-orange-500/30 rounded">
              ...
              <span className="text-xs font-medium text-warning leading-tight">{classification.action_suggested}</span>
            </div>
          )}
          ...
          {classification.keywords && classification.keywords.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {classification.keywords.slice(0, 3).map((kw, i) => (
                <Badge
                  key={i}
                  className="text-xs bg-white/10 border border-white/15 text-white/90 rounded-full px-2 py-0.5"
                >
                  {kw}
                </Badge>
              ))}
