  ...
  return (
    <Tabs defaultValue="address-rules" className="flex-1 flex flex-col">
      <TabsList className="bg-muted/30 w-fit">
        <TabsTrigger value="address-rules" className="text-xs gap-1">
          <Mail className="h-3 w-3" />
          Regole Address
        </TabsTrigger>
        <TabsTrigger value="group-rules" className="text-xs gap-1">
      ...
      <TabsContent value="address-rules" className="flex-1 mt-3">
        <AddressRulesSection />
      </TabsContent>
      <TabsContent value="group-rules" className="flex-1 mt-3">
        <GroupRulesSection />
      </TabsContent>
      <TabsContent value="prompts" className="flex-1 mt-3">
              ...
              <div key={rule.id} className="bg-card/80 border border-border/50 rounded-xl p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    {rule.display_name && (
                      <span className="text-[10px] text-muted-foreground">{rule.display_name}</span>
                    )}
                    {(rule.email_count ?? 0) > 0 && (
                      <Badge variant="outline" className="text-[10px]">
                        {rule.email_count} email
                      </Badge>
                    )}
                  </div>
                </div>
                {rule.group_name && (
                  <Badge
                    className="text-[10px]"
                    style={{ backgroundColor: (rule.group_color || "#666") + "20", color: rule.group_color || "#666" }}
                  >
                ...
                <Button
                  ...
                  onClick={() => {
                    if (confirm("Eliminare?")) deleteMutation.mutate(rule.id);
                  }}
                >
          ...
          {editingRule && (
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                <Select
                  value={editingRule.group_id ?? "none"}
                  onValueChange={(v) => {
                    const g = groups.find((g) => g.id === v);
                    setEditingRule({
                      ...editingRule,
                      group_id: v === "none" ? null : v,
                      group_name: g?.nome_gruppo || null,
                      group_color: g?.colore || null,
                      group_icon: g?.icon || null,
                    });
                  }}
                >
              ...
              <div>
                <Label className="text-xs">Prompt Dedicato</Label>
                <Textarea
                  value={editingRule.custom_prompt ?? ""}
                  onChange={(e) => setEditingRule({ ...editingRule, custom_prompt: e.target.value })}
                  className="text-xs mt-1"
                  rows={4}
                  placeholder="Istruzioni specifiche per l'AI quando gestisce questo sender…"
                />
              </div>
              <div>
...
function GroupRulesSection() {
  ...
  const updateGroup = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: queryKeys.email.senderGroupsRules });
    },
  });
    ...
    <div className="space-y-2">
      ...
      {groups.map((g) => (
        <div key={g.id} className="bg-card/80 border border-border/50 rounded-xl p-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium" style={{ color: g.colore }}>
              {g.nome_gruppo}
            </p>
            {g.descrizione && <p className="text-[10px] text-muted-foreground">{g.descrizione}</p>}
          </div>
          <Badge variant="outline" className="text-[10px]">
            {groupCounts[g.nome_gruppo] || 0} address
          </Badge>
          <Select
            value={g.auto_action || "none"}
            onValueChange={(v) => updateGroup.mutate({ id: g.id, auto_action: v })}
          >
...
function PromptManagerSection() {
  const { findAllEmailPrompts, updateEmailPromptById, insertEmailPrompt, setEmailPromptActive, deleteEmailPrompt } =
    useEmailPromptsRepo();
  const qc = useQueryClient();
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: queryKeys.email.promptsTab4 });
      setSheetOpen(false);
    },
  ...
  const openEdit = (prompt?: EditablePrompt) => {
    setEditingPrompt(
      prompt ?? { title: "", scope: "global", scope_value: null, instructions: "", priority: 5, is_active: true },
    );
    setSheetOpen(true);
  };
  ...
  return (
    <div className="space-y-3">
            ...
            {prompts.map((p) => (
              <div key={p.id} className="bg-card/80 border border-border/50 rounded-xl p-3 flex items-center gap-3">
                ...
                <Button
                  ...
                  className="h-7 w-7 p-0 text-destructive"
                  onClick={() => {
                    if (confirm("Eliminare?")) deleteMutation.mutate(p.id);
                  }}
                >
                  <Trash2 className="h-3 w-3" />
                </Button>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-96 overflow-y-auto">
          <SheetHeader>
            <SheetTitle className="text-sm">{editingPrompt?.id ? "Modifica Prompt" : "Nuovo Prompt"}</SheetTitle>
          </SheetHeader>
          {editingPrompt && (
            <div className="space-y-4 mt-4">
              <div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setEditingPrompt({ ...editingPrompt, scope: v, scope_value: null })}
                >
                  <SelectTrigger className="h-8 text-xs mt-1">
                    <SelectValue />
                  </SelectTrigger>
              ...
              {editingPrompt.scope !== "global" && (
                <div>
                  ...
                  <Input
                    value={editingPrompt.scope_value ?? ""}
                    onChange={(e) => setEditingPrompt({ ...editingPrompt, scope_value: e.target.value })}
                    className="h-8 text-xs mt-1"
                    placeholder={editingPrompt.scope === "category" ? "Nome categoria" : "email@example.com"}
                  />
                </div>
              )}
              <div>
                ...
                <Textarea
                  value={editingPrompt.instructions}
                  onChange={(e) => setEditingPrompt({ ...editingPrompt, instructions: e.target.value })}
                  className="text-xs mt-1"
                  rows={8}
                />
              </div>
              <div>
                <Label className="text-xs">Priorità: {editingPrompt.priority}</Label>
                <Slider
                  value={[editingPrompt.priority]}
                  onValueChange={([v]) => setEditingPrompt({ ...editingPrompt, priority: v })}
                  min={1}
                  max={10}
                  step={1}
                  className="mt-2"
              ...
              <Button
                className="w-full h-8 text-xs"
                onClick={() => saveMutation.mutate(editingPrompt)}
                disabled={saveMutation.isPending}
              >
