...
const TestExtensionsContent = lazy(() =>
  import("@/components/test-extensions/TestExtensionsView").then((m) => ({ default: m.TestExtensionsContent })),
);
const CommandPalette = lazy(() => import("@/components/CommandPalette").then((m) => ({ default: m.CommandPalette })));
const MissionDrawer = lazy(() =>
  import("@/components/global/MissionDrawer").then((m) => ({ default: m.MissionDrawer })),
);
const FiltersDrawer = lazy(() =>
...
const AgentOperationsDashboard = lazy(() =>
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === "m" && (e.metaKey || e.ctrlKey)) {
        e.preventDefault();
        navigate("/v1/mission-builder");
      }
      if (e.key === "Escape") {
        setCommandOpen(false);
        setIntelliflowOpen(false);
    ...
    const handler = (e: Event) => {
      ...
      switch (detail.action_type) {
        ...
        case "start_download_job":
          if (detail.job_id) {
            toast({
              title: "🤖 Job creato dall'agente",
              description: `Job ${detail.job_id.slice(0, 8)}… pronto. Vai su Network per avviarlo.`,
            });
            navigate("/v1/network");
          }
          break;
    ...
    <ActiveOperatorProvider>
      <MissionProvider>
        <GlobalFiltersProvider>
          <div
            className="flex h-screen w-full bg-background overflow-hidden overscroll-none"
          ...
          >
            ...
            <div
              className={`fixed left-0 top-0 z-50 h-full transition-transform duration-200 ease-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col">
              <header
                className="sticky top-0 z-30 h-11 sm:h-12 border-b border-border bg-background/80 backdrop-blur-md"
                role="banner"
              >
                <TooltipProvider>
                  <div className="flex h-full items-center justify-between px-2 sm:px-4">
                    ...
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      <OperatorSelector />
                      <InfoTooltip content="Nuovo Contatto">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 sm:h-8 sm:w-8 text-foreground hover:text-primary transition-colors"
                      ...
                      <InfoTooltip content="Arricchimento">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="hidden sm:inline-flex h-7 w-7 sm:h-8 sm:w-8 text-foreground hover:text-primary transition-colors"
                          onClick={() => navigate("/settings?tab=enrichment")}
                          aria-label="Arricchimento"
                        >
                          <DatabaseZap className="h-4 w-4" />
                        </Button>
                      </InfoTooltip>
                      <InfoTooltip content="Operazioni Agenti">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="hidden sm:inline-flex h-7 w-7 sm:h-8 sm:w-8 text-foreground hover:text-primary transition-colors"
                          onClick={() => setAgentDashOpen(true)}
                          aria-label="Operazioni Agenti"
                        >
                          <Activity className="h-4 w-4" />
                        </Button>
                      </InfoTooltip>
                      <InfoTooltip content="Test Estensioni">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="hidden md:inline-flex h-7 w-7 sm:h-8 sm:w-8 text-foreground hover:text-primary transition-colors"
                          onClick={() => setTestExtOpen(true)}
                          aria-label="Test Estensioni"
                        >
                          <FlaskConical className="h-4 w-4" />
                        </Button>
                      </InfoTooltip>
                      <InfoTooltip content="Assistente Vocale LUCA">
                        <Button
                          variant="ghost"
                          size="icon"
                          className={cn(
                            "hidden sm:inline-flex h-7 w-7 sm:h-8 sm:w-8 transition-colors",
                            voiceActive
                          ...
                          onClick={() => {
                            setVoiceActive((v) => !v);
                            window.dispatchEvent(new CustomEvent("toggle-voice-fab"));
                          }}
                      ...
                      <InfoTooltip content="IntelliFlow AI (⌘J)">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 sm:h-8 sm:w-8 text-foreground hover:text-primary transition-colors"
                          onClick={() => setIntelliflowOpen(true)}
                          aria-label="IntelliFlow"
                        >
              ...
              <main
                ...
                role="main"
                className={cn(
                  "flex-1 min-h-0 overflow-hidden mx-2 sm:mx-[36px]",
                  isFullscreenRoute ? "" : "overflow-auto p-2 sm:p-4",
                )}
              >
                <Outlet />
              </main>
