    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-2 border-b border-border/40 flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <Badge variant="outline" className="text-[9px] px-1.5 h-4 text-primary border-primary/30">
            {stats.pending} attesa
          </Badge>
          <Badge variant="outline" className="text-[9px] px-1.5 h-4 text-emerald-500 border-emerald-500/30">
            {stats.completed} fatte
          </Badge>
        </div>
      </div>
      ...
      <ScrollArea className="flex-1 min-h-0">
        {isLoading ? (
          ...
          <EmptyState
            icon={CheckCircle2}
            title={
              statusFilter === "all" && typeFilter === "all" ? "Nessuna attività" : "Nessuna attività con questi filtri"
            }
          ...
          <div className="p-2 space-y-1.5">
            {filtered.map((item) => {
              ...
              const contactEmail = String(
                (meta as Record<string, string>).email || (meta as Record<string, string>).recipient_email || "",
              );
              const contactName = String(
                (meta as Record<string, string>).contact_name || (meta as Record<string, string>).recipient_name || "",
              );
                ...
                <Collapsible key={item.id} open={isExpanded} onOpenChange={() => handleToggle(item.id)}>
                  ...
                  <CollapsibleContent>
                    <div className="px-3 pb-3 pt-1 bg-muted/10 rounded-b-lg border-t border-border/20 space-y-3">
                      {/* Email content */}
                      {(item.activity_type === "send_email" || (item.activity_type as string) === "email") &&
                        item.email_subject && (
                      ...
                      {/* Description */}
                      {item.description &&
                        item.activity_type !== "send_email" &&
                        (item.activity_type as string) !== "email" && (
                          <div>
                              ...
                              onSelect={async (d) => {
                                ...
                                if (d && item.id) {
                                  try {
                                    ...
                                    await logAuditEntry({
                                      action_category: "activity_updated",
                                      action_detail: `Riprogrammato per ${format(d, "dd MMM yyyy", { locale: it })}`,
                                      decision_origin: "manual",
                                      target_type: "activity",
                                      target_id: item.id,
                                    });
