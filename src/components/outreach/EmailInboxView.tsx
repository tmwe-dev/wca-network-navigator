...
export function EmailInboxView({ operatorUserId }: { operatorUserId?: string }) {
  ...
  const mailboxFilter = useMemo<{ kind: "personal" } | { kind: "shared"; id: string } | undefined>(() => {
    if (!activeMailbox) return undefined;
    if (activeMailbox.kind === "shared") return { kind: "shared" as const, id: activeMailbox.mailbox_id };
  ...
  const {
    ...
    isLoading,
    pageSize,
  } = useChannelMessages("email", debouncedSearch, page, operatorUserId, mailboxFilter);
  const markAsRead = useMarkAsRead();
  const { isSyncing, progress } = useContinuousSync();
  const { data: emailCount = 0 } = useEmailCount(isSyncing, mailboxFilter);

  const unreadCount = useMemo(() => messages.filter((m) => m.direction === "inbound" && !m.read_at).length, [messages]);
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden border-r border-border">
      ...
      <div className="flex-shrink-0 border-b border-border bg-muted/40 px-3 py-1.5">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[11px] uppercase tracking-wide text-muted-foreground shrink-0">Inbox di</span>
          <span className="text-sm font-semibold text-foreground truncate">{activeMailbox?.label ?? "Personale"}</span>
          {activeMailbox?.email && (
            <span className="text-[11px] text-muted-foreground truncate">· {activeMailbox.email}</span>
          )}
          <span
            className={cn(
      ...
      <div className="flex-shrink-0 border-b border-border px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => g.setSortingFilter(g.filters.sortingFilter === "unreviewed" ? "all" : "unreviewed")}
        ...
        <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground">
          ...
          <span className="flex items-center gap-1">
            ...
            pag. <strong className="text-foreground">{page + 1}</strong> ·{" "}
            <strong className="text-foreground">{inbound.length}</strong> vis.
          </span>
          {unreadCount > 0 && (
            ...
            <div className="flex items-center gap-2 text-sm font-medium">
              {progress.status === "syncing" && (
                <>
                  ...
                  <span className="text-primary">Sincronizzazione...</span>
                </>
              )}
              {progress.status === "done" && (
                <>
                  <CheckCircle2 className="h-4 w-4 text-primary" />
                  <span className="text-primary">Completata</span>
                </>
              )}
          ...
          {progress.lastSubject && (
            <div className="truncate rounded bg-background/60 px-2 py-1 text-xs text-muted-foreground">
              ...
              <span className="italic">
                "{progress.lastSubject.slice(0, 80)}
                {progress.lastSubject.length > 80 ? "..." : ""}"
              </span>
            </div>
          )}
