...
import { Linkedin, Loader2, Search, Send, X, PanelLeftClose, PanelLeftOpen } from "lucide-react";
...
export function LinkedInInboxView({ operatorUserId }: { operatorUserId?: string } = {}) {
  ...
  const closeTab = useCallback(
    (contact: string, e?: React.MouseEvent) => {
      e?.stopPropagation();
  ...
  const handleSendReply = async () => {
    ...
    const threadUrl = activeThread?.messages?.map((m) => m.thread_id).find((tid) => tid && tid.startsWith("http"));
      ...
      {!sidebarOpen ? (
        ...
        <>
          <div className="flex-shrink-0 p-2 space-y-2 border-b border-border">
              ...
              <div className="space-y-1.5 bg-muted/30 rounded-md p-2 border border-border">
                <div className="text-[9px] text-muted-foreground space-y-0.5">
                  ...
                  {bfProgress.lastError && (
                    <p className="text-red-400 truncate" title={bfProgress.lastError}>
                      ❌ {bfProgress.lastError}
                    </p>
                  )}
                </div>
            ...
            {!(bfProgress.status === "running" || bfProgress.status === "paused") &&
              bfState &&
              bfState.totalChats > 0 && (
                <p className="text-[9px] text-muted-foreground">
                  Backfill: {bfState.completedChats}/{bfState.totalChats} thread completi
                  {bfState.oldestMessageAt &&
                    ` • dal ${new Date(bfState.oldestMessageAt).toLocaleDateString("it-IT", { day: "2-digit", month: "short", year: "numeric" })}`}
                </p>
              )}
                  ...
                  <div key={msg.id} className={cn("flex", isOut ? "justify-end" : "justify-start")}>
                    <div
                      className={cn(
                        ...
                        isOut ? "bg-blue-600 text-white rounded-br-sm" : "bg-card border border-border rounded-bl-sm",
                      )}
                    >
