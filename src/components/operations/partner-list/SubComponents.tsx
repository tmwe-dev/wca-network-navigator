    ...
    <Tooltip>
      ...
      <TooltipContent side="bottom" className="text-xs">
        {done
          ? `${label} ✓`
          : verifiedMissing
            ? `${label}: ${count} mancanti (verificato ✓)`
            : `${label}: ${count} mancanti`}
      </TooltipContent>
    </Tooltip>
  );
}
...
export function StatusDot({ ok, label, isDark: _isDark }: { ok: boolean; label: string; isDark: boolean }) {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className={cn("w-2 h-2 rounded-full transition-colors", ok ? "bg-emerald-500" : "bg-muted")} />
      </TooltipTrigger>
...
export function EnrichedStatusIcons({
  hasProfile,
  hasEmail,
  hasPhone,
  hasDeep,
  ...
  return (
    <div
      className={cn(
        ...
        done ? "bg-emerald-500/10" : active ? "bg-primary/15 ring-1 ring-primary/30" : "bg-muted/20 opacity-40",
      )}
    >
      <span
        className={cn(
      ...
      <span
        className={cn(
          "text-[9px] font-mono font-bold ml-auto shrink-0",
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "bg-primary/10 border-primary/40 ring-1 ring-primary/20" : "bg-card border-border hover:bg-muted/50",
      )}
    >
      <div
        className={cn(
...
export function FilterActionBar({
  ...
  const configs: Record<
    string,
    { icon: React.ElementType; label: string; action: () => void; disabled?: boolean; color: string }
  > = {
    profiles: {
      icon: Download,
      label: "Scarica Profili Filtrati",
      action: onDownload,
    ...
    alias_co: {
      icon: Building2,
      label: "Genera Alias Azienda",
      action: () => onGenerateAlias("company"),
      disabled: aliasGenerating,
      color: "bg-primary hover:bg-primary/90",
    },
    alias_ct: {
      icon: UserCircle,
      label: "Genera Alias Contatto",
      action: () => onGenerateAlias("contact"),
      disabled: aliasGenerating,
      color: "bg-primary hover:bg-primary/90",
    },
  };
