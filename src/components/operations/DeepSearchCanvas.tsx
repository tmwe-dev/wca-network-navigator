  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={cn("text-sm font-semibold text-foreground")}>Deep Search</span>
        <span className={cn("text-xs tabular-nums text-muted-foreground")}>
          {done}/{total}
        </span>
        {running && <Loader2 className={cn("w-3.5 h-3.5 animate-spin text-primary")} />}
      ...
      <div className="px-4 py-1.5 flex-shrink-0">
        ...
        <div className={cn("text-[10px] mt-0.5 tabular-nums text-muted-foreground")}>{pct}% completato</div>
      </div>

      {/* ═══ BODY ═══ */}
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-2 space-y-3">
        {/* Current partner card */}
        {current && running && <CurrentPartnerCard current={current} isDark={isDark} />}

        {/* Completed history */}
        {results.length > 0 && (
          <div>
            <div className={cn("text-[10px] uppercase tracking-wider font-semibold mb-1.5", "text-muted-foreground")}>
  ...
  return (
    <div
      className={cn(
        ...
        isDark ? "bg-primary/[0.06] border-primary/20" : "bg-primary/10 border-primary/20",
      )}
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className={cn("text-sm font-semibold truncate text-foreground")}>{current.companyName}</div>
...
function CompletedRow({ result, isDark }: { result: DeepSearchResult; isDark: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Company name */}
      <span className={cn("text-xs font-medium truncate flex-1 min-w-0 text-foreground")}>
      ...
      {result.socialLinksFound > 0 && (
        <span className={cn("inline-flex items-center gap-0.5 text-[10px] tabular-nums", "text-muted-foreground")}>
      ...
      {result.contactProfilesFound > 0 && (
        <span className={cn("inline-flex items-center gap-0.5 text-[10px] tabular-nums", "text-primary")}>
      ...
      {result.rateLimited && <span className="text-[9px] text-primary">⚡</span>}
    </div>
  );
}
