...
interface CardProps {
  ...
  onSendWhatsApp: (params: {
    phone: string;
    contactName?: string;
    companyName?: string;
    sourceType?: "contact" | "partner" | "prospect";
    sourceId?: string;
  }) => void;
...
export function BcaCompactCard({
  card,
  isSelected,
  isOpened,
  onToggle,
  ...
  return (
    <div
      className={cn(
        "group relative flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-all border-l-2",
        isOpened
          ? "bg-primary/[0.10] border-primary"
          : isSelected
    ...
    >
      ...
      {card.position && (
        <span className="text-[10px] text-muted-foreground truncate max-w-[120px]">{card.position}</span>
      )}
          ...
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <Mail className="w-3 h-3 text-primary hover:text-primary" />
                </button>
              </TooltipTrigger>
              <TooltipContent className="text-[10px]">Invia email a {card.email}</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
                ...
                <button
                  onClick={(e) => {
                    ...
                    onSendWhatsApp({
                      ...
                      contactName: card.contact_name || undefined,
                  ...
                  className="hover:scale-110 transition-transform"
                >
                  <MessageCircle className="w-3 h-3 text-emerald-500/60 hover:text-emerald-500" />
                </button>
              </TooltipTrigger>
              <TooltipContent className="text-[10px]">WhatsApp {card.mobile || card.phone}</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        isOpened
          ? "border-primary ring-1 ring-primary/50 bg-primary/[0.08] shadow-sm"
          : isSelected
...
export function BcaExpandedCard({
  card,
  isSelected,
  isOpened,
  onToggle,
  ...
  return (
    <div
      className={cn(
        "group relative rounded-lg border p-4 cursor-pointer transition-all hover:shadow-sm",
        isOpened
          ? "border-primary ring-1 ring-primary/50 bg-primary/[0.08]"
          : isSelected
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {card.email && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onSendEmail({ email: card.email!, name: card.contact_name || undefined, company: groupCompanyName });
              }}
              className="text-[10px] px-2 py-0.5 rounded bg-primary/10 text-primary font-mono hover:bg-primary/20 transition-colors cursor-pointer"
            >
              {card.email}
            </button>
          )}
          {(card.phone || card.mobile) && (
            <button
              ...
              className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500 font-mono hover:bg-emerald-500/20 transition-colors cursor-pointer"
            >
