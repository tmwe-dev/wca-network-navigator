...
export function AIAutomationToggle({ className }: AIAutomationToggleProps) {
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      toast.success(newPausedState ? "Automazioni AI sospese" : "Automazioni AI riprese");
    } catch (error) {
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          ...
          <span className={cn("w-1.5 h-1.5 rounded-full", isPaused ? "bg-red-500" : "bg-emerald-500")} />
        </button>
      </TooltipTrigger>
      <TooltipContent side="bottom" className="text-xs">
        {isPaused ? "Automazioni AI sospese — clicca per riprendere" : "Automazioni AI attive — clicca per sospendere"}
      </TooltipContent>
    </Tooltip>
  );
}
