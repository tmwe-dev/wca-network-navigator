...
export function AIUsageChart({ data, byType, loading = false }: AIUsageChartProps) {
  if (loading) {
    return (
      <Card className="p-4">
  ...
  return (
    <Card className="p-4">
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-foreground">Utilizzo AI</h3>
        ...
        <>
          ...
          {byType && Object.entries(byType).length > 0 && (
            <div className="mt-4 pt-4 border-t border-border/50">
              <p className="text-xs font-medium text-muted-foreground mb-2">Dettaglio per Tipo</p>
                ...
                {Object.entries(byType)
                  ...
                  .map(([type, count]) => (
                    <div key={type} className="flex items-center justify-between rounded-lg bg-muted/50 p-2">
                      ...
                      <span className="text-xs font-semibold text-foreground ml-2">{count}</span>
                    </div>
                  ))}
              </div>
            </div>
