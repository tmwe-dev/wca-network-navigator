        ...
        <div>
          <div className="flex items-center justify-between">
            <button
              ...
              className={`flex items-center gap-1.5 text-sm font-medium py-1 ${th.dim}`}
            >
          ...
          {queueOpen &&
            queued.map((j) => (
              <div
                key={j.id}
                className={`flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs ${isDark ? "hover:bg-white/5" : "hover:bg-black/5"}`}
              >
                <span className="text-base">{getCountryFlag(j.country_code)}</span>
                <span className={`flex-1 truncate ${th.body}`}>{j.country_name}</span>
                <span className={`font-mono tabular-nums ${th.dim}`}>
                  {j.current_index}/{j.total_count}
                </span>
                {j.status === "paused" && (
                  <Button
                    size="sm"
                    variant="ghost"
                    className={`h-5 px-1.5 text-[10px] ${th.btnResume}`}
                    onClick={() => pauseResume.mutate({ jobId: j.id, action: "resume" })}
                  >
          ...
          {historyOpen &&
            history.map((j) => (
              ...
              >
                ...
                <span className={`flex-1 truncate ${th.body}`}>{j.country_name}</span>
                <Badge
                  className={`text-[10px] px-1.5 py-0 border-0 ${j.status === "completed" ? "bg-emerald-600 text-white" : "bg-slate-500 text-white"}`}
                >
                  {j.status === "completed" ? (
