    ...
    return (
      <div className="flex items-start gap-3 p-3 rounded-lg border border-border/50 bg-card hover:bg-accent/30 transition-colors">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {m.pending_promotion && (
            <>
              <Button size="icon" variant="ghost" className="h-7 w-7 text-green-500" onClick={() => promoteMutation.mutate({ id: m.id, approve: true })}>
                <Check className="h-3.5 w-3.5" />
              </Button>
              <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500" onClick={() => promoteMutation.mutate({ id: m.id, approve: false })}>
                <X className="h-3.5 w-3.5" />
              </Button>
            </>
          )}
          <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground" onClick={() => deleteMutation.mutate(m.id)}>
