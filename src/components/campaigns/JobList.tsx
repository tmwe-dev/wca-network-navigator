...
import { Mail, Phone, Search, CheckCircle2, Clock, Circle } from "lucide-react";
          ...
          {filtered.map(job => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">{job.city}, {job.country_name}</p>
              </div>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {job.email && <Mail className="w-3 h-3 text-emerald-500" />}
                {job.phone && <Phone className="w-3 h-3 text-blue-500" />}
              </div>
            </button>
          ))}
          {filtered.length === 0 && (
