...
export function CityMarkers({ partners, isVisible }: Props) {
  ...
  const positions = useMemo(() => partners.map((p) => latLngToVector3(p.lat, p.lng, 1.025)), [partners]);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < partners.length; i++) {
      ...
      scalesRef.current[i] = THREE.MathUtils.lerp(scalesRef.current[i] || 0, targetScale, isVisible ? 0.15 : 0.2);
  ...
  return <instancedMesh ref={meshRef} args={[CITY_GEOMETRY, CITY_MATERIAL, partners.length]} />;
}
