...
export function FlyingAirplanes({ countries, isActive }: Props) {
  ...
  const countriesWithPartners = useMemo(() => countries.filter((c) => c.count > 0), [countries]);
  ...
  useFrame((state) => {
    ...
    // Generate new routes every ~10 seconds (staggered with 2-3 planes)
    ...
    routesRef.current.forEach((route, index) => {
      ...
      const position = new THREE.Vector3().lerpVectors(route.arcPoints[arcIndex], route.arcPoints[nextIndex], localT);
      ...
      if (nextIndex > arcIndex) {
        const direction = new THREE.Vector3()
          .subVectors(route.arcPoints[nextIndex], route.arcPoints[arcIndex])
          .normalize();
