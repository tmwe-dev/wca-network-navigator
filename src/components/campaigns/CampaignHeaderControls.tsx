          ...
          <Button variant="outline" role="combobox" aria-expanded={comboOpen} className="w-56 justify-between">
            {selectedName ? (
              <span className="truncate">
                {getCountryFlag(selectedCountry!)} {selectedName}
              </span>
            ) : (
            ...
            <div className="flex items-center gap-1 px-2 py-1 border-b border-border/30">
              ...
              <button
                ...
                className={cn(
                  ...
                  countrySortBy === "name" ? "bg-primary/20 text-primary" : "text-muted-foreground hover:bg-muted/30",
                )}
              >
                A→Z
              </button>
              <button
                ...
                className={cn(
                  "px-2 py-0.5 rounded text-[10px] transition-colors",
                  countrySortBy === "count" ? "bg-primary/20 text-primary" : "text-muted-foreground hover:bg-muted/30",
                )}
              >
                #Partner
              </button>
                ...
                {sortedCountries.map((country) => (
                  ...
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        selectedCountry === country.code ? "opacity-100 text-primary" : "opacity-0",
                      )}
                    />
                    <span>{getCountryFlag(country.code)}</span>
                    <span className="ml-1.5 truncate">{country.name}</span>
                    <span
                      className={cn("ml-auto text-xs", country.count > 0 ? "text-primary" : "text-muted-foreground")}
                    >
                      {country.count}
                    </span>
                  </CommandItem>
      ...
      <div className="flex items-center gap-3 text-sm">
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted border border-border">
          <span className="font-mono text-foreground">
            {source === "bca" ? Object.keys(bcaCountryCounts).length : countries.filter((c) => c.count > 0).length}
          </span>
