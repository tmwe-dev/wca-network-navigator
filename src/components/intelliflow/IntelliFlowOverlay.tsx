            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-border/70 flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-0.5 bg-secondary/50 rounded-lg p-0.5 text-[10px]">
                  <button
                    onClick={() => ov.setMode("operational")}
                  ...
                  <button
                    onClick={() => ov.setMode("conversational")}
                ...
                <button
                  onClick={() => ov.setShowPanel(!ov.showPanel)}
                ...
                <button
                  onClick={() => ov.setShowHistory(!ov.showHistory)}
                ...
                <span className="text-[10px] text-muted-foreground font-mono tracking-wider hidden lg:block">
                  {ov.statsLine}
                </span>
                <button
                  onClick={props.onClose}
                  className="text-muted-foreground hover:text-foreground transition-colors p-1.5 rounded-lg hover:bg-secondary/30"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
                ...
                {ov.showHistory && (
                  ...
                  >
                    <div className="w-[260px] h-full overflow-y-auto py-3 px-2 space-y-1">
                      <p className="text-[10px] text-muted-foreground font-medium px-2 mb-2 uppercase tracking-wider">
                        Conversazioni recenti
                      </p>
                      {ov.conversations.length === 0 && (
                        <p className="text-[11px] text-muted-foreground px-2">Nessuna conversazione salvata</p>
                      )}
              ...
              <ResizablePanelGroup direction="horizontal" className="flex-1">
                <ResizablePanel defaultSize={ov.showPanel && ov.hasPanelContent ? 55 : 100} minSize={40}>
                  <div className="flex flex-col h-full min-h-0">
                    {ov.isEmpty ? (
                      <OverlayQuickPromptBar
                        statsLine={ov.statsLine}
                        pageLabel={ov.pageLabel}
                        quickPrompts={ov.quickPrompts}
                        onSend={ov.sendMessage}
                      />
                    ) : (
                      <OverlayConversationThread
                        messages={ov.messages}
                        loading={ov.loading}
                        chatEndRef={ov.chatEndRef}
                      />
                    )}
                        ...
                        >
                          ...
                          <input
                            ...
                            placeholder={
                              ov.speech.listening
                                ? "🎙 Sto ascoltando…"
                                : ov.mode === "conversational"
                                  ? "Discutiamo strategia…"
                                  : "Scrivi un obiettivo…"
                            }
                            value={
                              ov.speech.listening
                                ? ov.input + (ov.speech.interimText ? ` ${ov.speech.interimText}` : "")
                                : ov.input
                            }
                          ...
                          />
                          <button
                            onClick={() => ov.sendMessage()}
                            disabled={!ov.input.trim() || ov.loading}
