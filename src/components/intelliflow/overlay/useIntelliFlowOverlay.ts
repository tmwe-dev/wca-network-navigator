...
const QUICK_PROMPTS: Record<string, string[]> = {
  ...
  [`/${ROUTE_NETWORK}`]: [
    "Deep search partner USA",
    "Classifica per servizio",
    "Verifica email partner USA",
    "Genera alias commerciali",
  ],
  [`/${ROUTE_CRM}`]: [
...
export function useIntelliFlowOverlay({ open, onClose, cockpitContacts, onCockpitAIActions }: OverlayProps) {
  const { messages, addMessages, newConversation, conversations, resumeConversation, deleteConversation } =
    ...
    async (text?: string) => {
      ...
      try {
        ...
        await addMessages([
          {
            role: "assistant",
            content: "⚠️ " + (e instanceof Error ? e.message : "Errore di comunicazione"),
            timestamp: ts(),
          },
        ]);
      } finally {
