...
export type PartnerCreateRequested = DomainEvent<
  "partner.create.requested",
  { readonly companyName: string; readonly countryCode: string }
>;
...
export type PartnerUpdated = DomainEvent<
  "partner.updated",
  { readonly partnerId: string; readonly fields: ReadonlyArray<string> }
>;
...
export type ContactMatched = DomainEvent<
  "contact.matched",
  { readonly contactId: string; readonly partnerId: string; readonly confidence: number }
>;
...
export type AgentTaskCompleted = DomainEvent<
  "agent.task.completed",
  { readonly taskId: string; readonly summary: string }
>;
...
export type CampaignCompleted = DomainEvent<
  "campaign.completed",
  { readonly campaignId: string; readonly sentCount: number }
>;

// ── Download events ──────────────────────────────────────────────────

export type DownloadStarted = DomainEvent<"download.started", { readonly jobId: string; readonly countryCode: string }>;
export type DownloadCompleted = DomainEvent<
  "download.completed",
  { readonly jobId: string; readonly totalFound: number }
>;

// ── Health events ────────────────────────────────────────────────────

export type HealthCheckCompleted = DomainEvent<
  "health.check.completed",
  { readonly healthy: boolean; readonly services: Record<string, boolean> }
>;

// ── Union of all events ──────────────────────────────────────────────

export type AppEvent =
  | PartnerCreateRequested
  | PartnerCreated
  | PartnerCreateFailed
  | PartnerUpdated
  | PartnerDeleted
