  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-semibold text-foreground truncate">{contact.name || "—"}</div>
        {contact.role && <div className="text-[10px] text-muted-foreground truncate">{contact.role}</div>}
      </div>
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {channels.email && <Mail className="w-3 h-3 text-primary" />}
        ...
        {channels.phone && !channels.email && !channels.whatsapp && <Phone className="w-3 h-3 text-muted-foreground" />}
