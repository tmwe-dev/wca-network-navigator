/**
 * BulkMergeOriginsDialog — Riassegna in massa il campo `origin` per TUTTI
 * i contatti CRM che oggi hanno una delle origini selezionate (es. dalla
 * sidebar filtri). Permette di "fondere" più origini sotto un'unica
 * etichetta esistente o nuova.
 */
...
export interface BulkMergeOriginsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Origini sorgente da fondere (con conteggio righe attuale). */
  sourceOrigins: ReadonlyArray<{ origin: string; count: number }>;
  /** Origini esistenti per autocomplete (tutto l'elenco). */
...
export function BulkMergeOriginsDialog({
  open,
  onOpenChange,
  sourceOrigins,
  availableOrigins,
  onConfirm,
  onSuccess,
}: BulkMergeOriginsDialogProps): React.ReactElement {
      ...
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={!canConfirm}>
            ...
            Sposta {totalRows} contatt{totalRows === 1 ? "o" : "i"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default BulkMergeOriginsDialog;