...
import { ChevronDown, ChevronUp, BookOpen, User, Scroll, History, Wand2, Search } from "lucide-react";
    ...
    <>
      <div className="border-t border-border bg-card/40 shrink-0">
        <div className="flex items-center justify-between px-3 py-1.5 gap-2">
          <div className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">Cosa legge l'AI</div>

          <div className="flex items-center gap-1.5">
            {/* CTA principale: apre Sherlock */}
            <Button size="sm" onClick={() => setCanvasOpen(true)} className="h-7 px-2.5 text-[11px] gap-1.5">
        ...
        {open && (
          <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="px-2 pb-2">
            <TabsList className="h-7">
              <TabsTrigger value="kb" className="text-[10px] h-6 px-2">
                <BookOpen className="w-3 h-3 mr-1" />
                KB email
              </TabsTrigger>
              <TabsTrigger value="sender" className="text-[10px] h-6 px-2">
                <User className="w-3 h-3 mr-1" />
                Mittente
              </TabsTrigger>
              <TabsTrigger value="doctrine" className="text-[10px] h-6 px-2">
                <Scroll className="w-3 h-3 mr-1" />
                Dottrina
              </TabsTrigger>
              <TabsTrigger value="prompts" className="text-[10px] h-6 px-2">
                <Wand2 className="w-3 h-3 mr-1" />
                Prompt
              </TabsTrigger>
              <TabsTrigger value="history" className="text-[10px] h-6 px-2">
      ...
      <SherlockCanvas open={canvasOpen} onOpenChange={setCanvasOpen} recipient={recipient} />
    </>
  );
}
