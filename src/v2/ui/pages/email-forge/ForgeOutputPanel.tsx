      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="flex flex-col h-full">
          ...
          {result && dataPointsBadge && (
            <div className="px-3 pt-2 shrink-0">
              ...
              >
                ...
                {isGeneric ? (
                  <span>
                    <strong>[GENERIC]</strong> — l'AI non ha trovato dati specifici, ha usato presentazione standard.
                    Arricchisci il partner per output più mirato.
                  </span>
                ) : (
                  <span>
                    <strong>{dataPointsBadge.used}</strong> data point{dataPointsBadge.used === 1 ? "" : "s"} iniettat
                    {dataPointsBadge.used === 1 ? "o" : "i"}
        ...
        <TabsContent value="ai" className="flex-1 overflow-auto mt-0 p-3">
          <div className="flex items-center justify-between mb-2">
            <div className="text-xs font-medium text-foreground uppercase tracking-wide">Contesto disponibile</div>
          ...
          <Tabs value={aiTab} onValueChange={(v) => setAiTab(v as typeof aiTab)}>
            <TabsList className="h-7">
              <TabsTrigger value="kb" className="text-xs h-6 px-2">
                <BookOpen className="w-3 h-3 mr-1" />
                KB email
              </TabsTrigger>
              <TabsTrigger value="sender" className="text-xs h-6 px-2">
                <User className="w-3 h-3 mr-1" />
                Mittente
              </TabsTrigger>
              <TabsTrigger value="doctrine" className="text-xs h-6 px-2">
                <Scroll className="w-3 h-3 mr-1" />
                Dottrina
              </TabsTrigger>
              <TabsTrigger value="prompts" className="text-xs h-6 px-2">
            ...
            <TabsContent value="sender" className="mt-2">
              <SenderProfileTab />
            </TabsContent>
            <TabsContent value="doctrine" className="mt-2">
              <DoctrineTab />
            </TabsContent>
