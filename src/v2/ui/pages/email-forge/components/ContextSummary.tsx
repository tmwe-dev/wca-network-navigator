...
function CompactSummary({ pre, post }: { pre: PreGenerationContext; post?: PostGenerationContext }) {
  const enrichLevels = [pre.enrichment.base, pre.enrichment.deep_search, pre.enrichment.sherlock].filter(
    Boolean,
  ).length;
  return (
    <div className="flex items-center gap-1.5 flex-wrap text-[10px]">
      ...
      />
      <Pill icon={Brain} label={`${pre.memory.count} mem`} status={pre.memory.count > 0 ? "active" : "empty"} />
      ...
      {post?.journalist && (
        <Pill
          icon={FileText}
          label={`${post.journalist.role} · ${post.journalist.verdict}`}
          status={
            post.journalist.verdict === "block" ? "error" : post.journalist.verdict === "warn" ? "warning" : "active"
          }
        />
      )}
      ...
      <div className="space-y-0.5">
        {items.map((it, i) => (
          <div key={i} className="flex items-baseline justify-between gap-2 text-[10px]">
            ...
            <span
              className={cn("text-right truncate", it.status === "ok" ? "text-foreground" : "text-foreground italic")}
            >
              {it.value}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Section
          ...
          items={[
            ...
            {
              label: "Touch count",
              value: String(pre.relationship.touch_count),
              status: pre.relationship.touch_count > 0 ? "ok" : "missing",
            },
            {
              label: "Ha risposto",
              value: pre.relationship.has_replied ? "Sì" : "No",
              status: pre.relationship.has_replied ? "ok" : "missing",
            },
            {
              label: "Ultimo contatto",
              value: pre.relationship.days_since_last != null ? `${pre.relationship.days_since_last} gg fa` : "Mai",
              status: pre.relationship.days_since_last != null ? "ok" : "missing",
            },
          ]}
        />
        <Section
          icon={Search}
          title="Enrichment + Memory"
          items={[
            { label: "Base", value: pre.enrichment.base ? "Sì" : "No", status: pre.enrichment.base ? "ok" : "missing" },
            {
              label: "Deep Search",
              value: pre.enrichment.deep_search ? "Sì" : "No",
              status: pre.enrichment.deep_search ? "ok" : "missing",
            },
            {
              label: "Sherlock",
              value: pre.enrichment.sherlock ? "Sì" : "No",
              status: pre.enrichment.sherlock ? "ok" : "missing",
            },
        ...
        <Section
          ...
          items={[
            {
              label: "Sezioni KB",
              value: pre.kb.sections_available.length > 0 ? pre.kb.sections_available.join(", ") : "Nessuna",
              status: pre.kb.sections_available.length > 0 ? "ok" : "missing",
            },
      ...
      {pre.type_resolution && pre.type_resolution.conflicts.length > 0 && (
        <div className="space-y-1 pt-2 border-t border-border/40">
          ...
          {pre.type_resolution.conflicts.map((c, i) => (
            ...
            >
              <div>
                <span className="font-mono opacity-60">[{c.type}]</span> {c.description}
              </div>
      ...
      {post && post.warnings.length > 0 && (
        <div className="space-y-1 pt-2 border-t border-border/40">
          ...
          {post.warnings.map((w, i) => (
            <div key={i} className="text-[10px] text-warning">
              ⚠ {w}
            </div>
          ))}
        </div>
