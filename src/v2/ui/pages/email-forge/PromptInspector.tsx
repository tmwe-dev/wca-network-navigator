...
export function PromptInspector({ systemPrompt, userPrompt, systemBlocks, blocks, isLoading, onRerun }: Props) {
  ...
  if (isLoading && !systemBlocks?.length && !blocks?.length) {
    return (
      <div className="flex items-center justify-center h-full text-sm text-foreground gap-2">
  ...
  if (!systemBlocks?.length && !blocks?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center text-sm text-foreground p-6">
        <FileText className="w-10 h-10 mb-3 opacity-40" />
        <p className="font-medium">Nessun prompt ancora generato</p>
        <p className="text-xs mt-1 max-w-xs">
          Configura le scelte nel pannello Oracolo a sinistra e clicca <strong>Genera</strong> per ispezionare e{" "}
          <strong>modificare</strong> i blocchi che compongono il prompt.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {dirtyCount > 0 && (
            <Button
              ...
              title="Azzera tutte le modifiche"
            >
      ...
      <div className="flex-1 overflow-auto p-3 space-y-4">
        ...
        {blocks && blocks.length > 0 && (
          <section>
            <h3 className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground mb-2 flex items-center gap-1.5">
              <Badge className="bg-secondary text-secondary-foreground hover:bg-secondary border-0 text-[9px]">
                USER
              </Badge>
        ...
        <div className="text-xs text-foreground border-t border-border/30 pt-3 flex items-start gap-2">
          <Save className="w-3 h-3 mt-0.5 shrink-0" />
          <span>
            Le modifiche restano salvate localmente nel browser. Usa <strong>Rigenera con modifiche</strong> per inviare
            al modello AI il prompt modificato; usa <strong>Ripristina</strong> sul singolo blocco o{" "}
