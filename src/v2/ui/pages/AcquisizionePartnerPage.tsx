...
export function AcquisizionePartner() {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-3 relative overflow-hidden">
      ...
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-primary/[0.05] via-transparent to-transparent dark:block hidden animate-pulse"
      ...
      <div className="relative z-10 flex flex-col h-full gap-2 p-3">
        ...
        <div className="flex-1 grid grid-cols-[35%_1fr] gap-3 min-h-0">
          ...
          <div className="flex flex-col gap-2 min-h-0">
            ...
            <div className="p-3 bg-card/60 backdrop-blur-xl border border-border rounded-2xl shadow-lg shadow-black/[0.05]">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <Button
                  onClick={handleScan}
                  disabled={
                    selectedCountries.length === 0 || pipelineStatus === "scanning" || pipelineStatus === "running"
                  }
                  variant="outline"
                  size="sm"
                  className="text-xs"
                ...
                <div
                  className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded ${extensionAvailable ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}
                >
                  ...
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${
                        sessionHealth === "active"
                          ? "bg-success"
                          : sessionHealth === "checking" || sessionHealth === "recovering"
              ...
              {scanStats && (
                <div className="flex items-center gap-3 mt-1.5 text-[10px] text-muted-foreground">
                  ...
                  <span>
                    Già: <strong className="text-success">{scanStats.existing}</strong>
                  </span>
                  <span>
            ...
            {/* Live Stats */}
            {(pipelineStatus === "running" || pipelineStatus === "paused" || pipelineStatus === "done") &&
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-2 rounded-xl bg-card/60 backdrop-blur-xl border border-border text-[10px]">
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <strong className="text-foreground">
                      {liveStats.processed}/{queue.filter((q) => selectedIds.has(q.wca_id)).length}
                    </strong>
                  </div>
                  <div className="h-1.5 flex-1 min-w-[60px] max-w-[120px] bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(liveStats.processed / Math.max(queue.filter((q) => selectedIds.has(q.wca_id)).length, 1)) * 100}%`,
                      }}
                    />
                  </div>
          ...
          <div className="flex flex-col gap-2 min-h-0">
            <div className="flex-1 flex flex-col bg-card/60 backdrop-blur-xl border border-border rounded-2xl overflow-hidden shadow-lg shadow-black/[0.05]">
              <PartnerCanvas data={canvasData} phase={canvasPhase} isAnimatingOut={isAnimatingOut} />
            </div>
          </div>
        </div>
        ...
        <AlertDialog open={showSessionAlert} onOpenChange={setShowSessionAlert}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              <AlertDialogAction onClick={() => setShowSessionAlert(false)}>Ho capito</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
