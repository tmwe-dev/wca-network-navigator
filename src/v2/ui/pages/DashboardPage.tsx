...
const SmartActions = lazyRetry(() =>
  import("@/components/home/SmartActions").then((m) => ({ default: m.SmartActions })),
);
const OperativeMetricsGrid = lazyRetry(() =>
  import("@/components/home/OperativeMetricsGrid").then((m) => ({ default: m.OperativeMetricsGrid })),
);
const AgentStatusPanel = lazyRetry(() =>
  import("@/components/home/AgentStatusPanel").then((m) => ({ default: m.AgentStatusPanel })),
);
const DashboardCharts = lazyRetry(() =>
  import("@/components/analytics/DashboardCharts").then((m) => ({ default: m.DashboardCharts })),
);
const ResponseRateCard = lazyRetry(() =>
  import("@/components/analytics/ResponseRateCard").then((m) => ({ default: m.ResponseRateCard })),
);
const EmailObservabilityPanel = lazyRetry(() =>
  import("@/v2/ui/components/dashboard/EmailObservabilityPanel").then((m) => ({ default: m.EmailObservabilityPanel })),
);
const SystemDiagnosticsBadge = lazyRetry(() =>
  import("@/v2/ui/components/admin/SystemDiagnosticsBadge").then((m) => ({ default: m.SystemDiagnosticsBadge })),
);
...
const NAV_CARDS = [
  ...
  { key: "pipeline", title: "Pipeline", description: "Contatti, kanban, agenda", route: "/v2/pipeline", icon: Kanban },
  {
    key: "communicate",
    title: "Comunica",
    description: "Inbox, outreach, composer",
    route: "/v2/communicate",
    icon: Radar,
  },
  {
    key: "intelligence",
    title: "Intelligence",
    description: "Agenti, KB, analytics",
    route: "/v2/intelligence",
    icon: Brain,
  },
] as const;
...
export function DashboardPage() {
  ...
  const statForCard = (key: string) => {
    switch (key) {
      case "explore":
        return `${formatCompact(dashData?.partnerCount ?? 0)} partner`;
      case "pipeline":
        return `${formatCompact(dashData?.prospectTotal ?? 0)} prospect`;
      case "communicate":
        return `${formatCompact(dashData?.readyContactsCount ?? 0)} pronti`;
      case "intelligence":
        return `${formatCompact(dashData?.openActivitiesCount ?? 0)} aperte`;
      default:
        return "";
    }
  };
        ...
        <div className="mx-auto max-w-5xl space-y-4 px-4 py-6 sm:px-6">
          <section className="space-y-3">
            ...
            <HomeAIPrompt
              systemStats={{
                activeJobs,
                pendingActivities: dashData?.openActivitiesCount ?? 0,
                totalPartners: dashData?.partnerCount ?? 0,
              }}
