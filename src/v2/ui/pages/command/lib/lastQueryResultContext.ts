...
export function extractPartnerIdsFromResult(result: unknown): string[] {
  ...
  if (r.kind === "table" && Array.isArray(r.rows)) {
    ...
    const looksLikePartners = typeof meta?.sourceLabel === "string" && /partner/i.test(meta.sourceLabel);
...
export function isProceedIntent(prompt: string): boolean {
  ...
  return (
    ...
    /\b(prepara|prepari|preparare|fai|scrivi|componi|genera|generami)\s+(?:la|il|un'?|una|\d+)?\s*(bozza|lettera|lettere|mail|email|e-mail|messaggio|messaggi)\b/i.test(
      p,
    ) ||
    ...
    /\b(prepara|prepari|preparare|fai|scrivi|manda|mandagli|invia|inviagli|genera|generami)\s+(?:un'?|una|gli|loro|\d+)?\s*(invito|inviti|invit[oa]|lettera|lettere)\b/i.test(
      p,
    ) ||
