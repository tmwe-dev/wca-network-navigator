...
export const enrichProspectFromWebsiteTool: Tool = {
  ...
  description: "Scrapa il sito del prospect via edge function, propone aggiornamenti",
  match: (p: string) => /(arricchisci|enrich|analizza.*sito).*prospect/i.test(p),
  execute: async (prompt, context) => {
    ...
    if (!prospectId) throw new Error("Prospect ID non trovato. Specifica un UUID.");

    if (!context?.confirmed) {
      return {
        kind: "approval",
        title: "Arricchimento prospect dal sito web",
        description: "Scrapa il sito web del prospect → estrae contatti → propone aggiornamento.",
        details: [
    ...
    if (fetchErr || !prospect) throw new Error(fetchErr?.message ?? "Prospect non trovato");

    const rec = prospect as Record<string, unknown>;
    const website = rec.website as string | undefined;
    if (!website) throw new Error("Prospect senza sito web — impossibile fare scraping");
    ...
    if (scraped.emails.length > 0 && !rec.email) updates.email = scraped.emails[0];
    if (scraped.phones.length > 0 && !rec.phone) updates.phone = scraped.phones[0];
