...
export const enrichPartnerFromWebsiteTool: Tool = {
  ...
  description: "Scrapa il sito del partner via edge function, propone aggiornamenti (email, phone, descrizione)",
  match: (p: string) => /(arricchisci|enrich|analizza.*sito).*partner/i.test(p),
  execute: async (prompt, context) => {
    ...
    if (!context?.confirmed) {
      return {
        ...
        description: "Scrapa il sito web del partner → estrae contatti e descrizione → propone aggiornamento record.",
        details: [
    ...
    if ((scraped.description || scraped.ogDescription) && !partnerRec.profile_description)
      updates.profile_description = scraped.description || scraped.ogDescription;
