          ...
          {canvas === "live-table" && liveResult && liveResult.kind === "table" && (
            <div className="float-panel p-6 rounded-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={handleClose} className="text-muted-foreground hover:text-foreground text-[10px]">✕</button>
              </div>
              <TableCanvas
          ...
          {canvas === "live-timeline" && liveResult && liveResult.kind === "timeline" && (
            <div className="float-panel p-6 rounded-2xl">
              ...
              {liveResult.meta?.sourceLabel && (
                <div className="text-[9px] text-muted-foreground font-mono mb-3">{liveResult.meta.sourceLabel} · {liveResult.meta.count} record</div>
              )}
              {liveResult.events.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <p className="text-[12px] text-muted-foreground font-light">Nessuna attività registrata</p>
                </div>
              ) : (
          ...
          {canvas === "live-flow" && liveResult && liveResult.kind === "flow" && (
            <div className="float-panel p-6 rounded-2xl">
              ...
              {liveResult.nodes.length <= 1 && liveResult.nodes[0]?.type === "end" ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <p className="text-[12px] text-muted-foreground font-light">Nessun flusso in corso</p>
                </div>
              ) : (
