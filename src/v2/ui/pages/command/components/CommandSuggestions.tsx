...
export default function CommandSuggestions({ onSend }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-8">
      ...
      <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, ease }} className="text-2xl font-extralight tracking-tight text-foreground mb-2">
        Cosa vuoi ottenere?
      </motion.h2>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="flex flex-col items-center gap-2">
        {allPrompts.map((p, i) => (
          <motion.button
            ...
            whileHover={{ x: 4 }}
            className="text-[12px] px-4 py-2.5 rounded-2xl text-muted-foreground hover:text-muted-foreground hover:bg-secondary/[0.1] transition-all duration-700 text-left flex items-center gap-2"
          >
            <span>{p.icon}</span>
            <span>{p.text}</span>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }} className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-12">
        {capabilities.map((cap, i) => (
          <motion.span key={cap} className="text-[9px] text-muted-foreground font-light" animate={{ opacity: [0.4, 0.7, 0.4] }} transition={{ duration: 4, repeat: Infinity, delay: i * 0.5 }}>
            {cap}
          </motion.span>
        ))}
      </motion.div>
