      ...
      <button type="button" onClick={onSelect} className="flex-1 min-w-0 text-left">
        <div className="flex items-start gap-2 mb-1">
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold leading-tight line-clamp-2">{item.subject ?? "(senza oggetto)"}</div>
        ...
        <div className="flex items-center gap-1.5 mt-2">
          <span title={ch.label} className="inline-flex">
            <ch.Icon className={cn("h-3.5 w-3.5 shrink-0", ch.tone)} />
          </span>
          <Badge variant="outline" className={cn("text-[9px] px-1.5 py-0 border", st.tone)}>
            {st.label}
          </Badge>
          {pt && <span className={cn("px-1.5 py-0.5 rounded border font-medium text-[9px]", pt.tone)}>{pt.label}</span>}
          {departingSoon && (
