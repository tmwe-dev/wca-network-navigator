    ...
    <div className="space-y-3">
      {item.channel === "email" && (
        <div className="rounded-md border bg-muted/20 p-3 text-xs space-y-1">
          <div className="flex gap-2">
            ...
            <span className="font-medium truncate">{item.recipientHandle ?? "—"}</span>
          </div>
          <div className="flex gap-2">
            <span className="text-muted-foreground w-12 shrink-0">Oggetto:</span>
      ...
      {(item.channel === "whatsapp" || item.channel === "linkedin") && (
        <div className="flex items-center gap-2 text-xs">
          ...
          {limit && (
            <span
              className={cn("ml-auto text-[10px]", charCount > limit ? "text-destructive" : "text-muted-foreground")}
            >
    ...
    <div className="space-y-3">
          ...
          <div className="space-y-1.5 text-xs border-l-2 border-primary/40 pl-3 mt-2">
            {oc.eventName && (
              <div className="flex items-center gap-1.5">
                <Sparkles className="h-3 w-3" /> {oc.eventName}
              </div>
            )}
            {oc.meetingLocation && (
              <div className="flex items-center gap-1.5">
                <MapPin className="h-3 w-3" /> {oc.meetingLocation}
              </div>
            )}
            {oc.acquiredAt && (
              <div className="flex items-center gap-1.5">
                <Calendar className="h-3 w-3" /> Incontrato il{" "}
                {format(new Date(oc.acquiredAt), "dd MMM yyyy", { locale: itLocale })}
              </div>
            )}
          </div>
        )}
      ...
      {item.previousMessage && (
        <div className="rounded-lg border bg-card p-4 space-y-2">
          ...
          {item.previousMessage.subject && <div className="text-sm font-medium">{item.previousMessage.subject}</div>}
        ...
        <li key={idx} className="ml-4 pb-1">
          ...
          <div className="flex items-center gap-2 text-xs mb-1">
            ...
            <span className="text-[10px] px-1.5 py-0.5 rounded border text-muted-foreground">
              {i.direction === "in" ? "in" : i.direction === "out" ? "out" : "nota"}
            </span>
            <span className="ml-auto text-[10px] text-muted-foreground">
    ...
    <div className="space-y-3">
      <div className="rounded-lg border bg-card p-4 space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <CheckRow
            ok={!!item.deepSearchDoneAt}
            label="Deep Search Sherlock"
            detail={
              item.deepSearchDoneAt
                ? `Fatta ${format(new Date(item.deepSearchDoneAt), "dd MMM", { locale: itLocale })}`
                : "Mai eseguita"
            }
          />
          ...
          <CheckRow
            ok={item.triggerKind === "campaign" || item.triggerKind === "inbound_reply"}
            label={`Routing: ${tr.label}`}
          />
          <CheckRow
    ...
    <div className="space-y-3">
      <div className="rounded-lg border bg-card p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {pt && (
            <Badge variant="outline" className={cn("text-[10px] border", pt.tone)}>
              {pt.label}
            </Badge>
          )}
          {item.partnerLeadStatus && (
            <Badge variant="outline" className="text-[10px]">
              Lead status: {item.partnerLeadStatus}
            </Badge>
          )}
        </div>
