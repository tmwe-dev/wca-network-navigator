...
function AddressRulesSection() {
    ...
    async (id: string) => {
      ...
      try {
        ...
        await logSupervisorAudit({
          ...
          target_table: "email_address_rules",
          target_id: b.source.id,
        });
        state.markClean(id);
  ...
  const filtered = q.length >= 4 ? state.blocks.filter((b) => b.label.toLowerCase().includes(q)) : state.blocks;
...
export function EmailPromptsTab() {
  return (
    <Tabs defaultValue="types" className="w-full flex flex-col h-full min-h-0">
      ...
      <TabsContent value="types" className="mt-2 flex-1 min-h-0 data-[state=active]:flex">
        <EmailTypesSection />
      </TabsContent>
      <TabsContent value="global" className="mt-2 flex-1 min-h-0 data-[state=active]:flex">
        <GlobalPromptsSection />
      </TabsContent>
      <TabsContent value="rules" className="mt-2 flex-1 min-h-0 data-[state=active]:flex">
        <AddressRulesSection />
      </TabsContent>
    </Tabs>
  );
