...
export function HarmonizeSystemDialog({ open, onOpenChange }: Props) {
  ...
  const sessionBootstrapEntities = (ingestion.state.session?.entities_created ?? []).filter(
    (e) => e.created_in_chunk < 0,
  ).length;
  const sessionRunCreatedEntities = (ingestion.state.session?.entities_created ?? []).filter(
    (e) => e.created_in_chunk >= 0,
  ).length;
    ...
    <Dialog open={open} onOpenChange={(o) => (o ? onOpenChange(o) : handleClose())}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Confronta lo stato reale del DB con lo stato desiderato della libreria TMWE e propone azioni tipizzate
            (UPDATE / INSERT / MOVE / DELETE) con evidenza e classificazione del gap.
          </DialogDescription>
          <div className="pt-2">
            <a
          ...
          <TabsContent value="classic" className="flex-1 overflow-auto pr-2 mt-4">
              ...
              <div className="flex flex-col items-center justify-center py-8 gap-3">
                ...
                {state.collector && (
                  <div className="text-xs text-muted-foreground space-y-1 text-center">
                    <div>
                      Reale: {state.collector.realSummary.total} elementi · Desiderato:{" "}
                      {state.collector.desiredSummary.total} sezioni
                    </div>
                    <div className="flex gap-2 justify-center flex-wrap">
                      <Badge variant="outline">Testo: {state.collector.classification.text_only}</Badge>
                      <Badge variant="outline">
                        KB governance: {state.collector.classification.needs_kb_governance}
                      </Badge>
                ...
                {state.progress.total > 0 && (
                  <p className="text-xs text-muted-foreground">
                    {state.progress.current} / {state.progress.total} chunk
                  </p>
                )}
            ...
            {state.phase === "executing" && (
              <div className="flex flex-col items-center justify-center py-8 gap-3">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
                <p className="text-sm">
                  Esecuzione: {state.executedCount} ok · {state.failedCount} falliti
                </p>
              </div>
            )}
          ...
          <TabsContent value="ingestion" className="flex-1 overflow-auto pr-2 mt-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="text-xs">
                    ...
                    <p className="text-muted-foreground mt-1">
                      File: <code>{ingestion.state.resumable.source_file}</code> · Chunk completati:{" "}
                      {ingestion.state.resumable.current_chunk}/{ingestion.state.resumable.total_chunks}. Ricarica lo
                      stesso file e clicca "Riprendi".
                    </p>
                  </div>
            ...
            {(ingestion.state.phase === "idle" || ingestion.state.phase === "starting") && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ref={ingestionFileRef}
                    type="file"
                    accept={ACCEPT_STRING}
                    onChange={handleIngestionUpload}
                    className="hidden"
                  />
                    ...
                    {TMWE_CHUNKS.map((c) => (
                      <li key={c.index} className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">
                          L{c.sourceLines[0]}–{c.sourceLines[1]}
                        </span>
                      </li>
                    ))}
                  </ul>
                  ...
                  {ingestion.state.chunks.map((c) => {
                    ...
                    return (
                      <div key={c.chunkIndex} className="rounded border border-border p-2">
                        ...
                        {c.errorMsg && <p className="text-[11px] text-destructive mt-1">{c.errorMsg}</p>}
            ...
            {ingestion.state.phase === "error" && (
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {ingestion.state.chunks
                    .filter((c) => c.status === "error")
                    .map((c) => (
                      <div
                        key={c.chunkIndex}
                        className="flex items-center justify-between text-xs border border-destructive/40 rounded p-2"
                      >
                        <span>
                          #{c.chunkIndex} {c.chunkName}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"
                          ...
                          onClick={() =>
                            ingestionFile &&
                            void ingestion.retryChunk(c.chunkIndex, ingestionFile.content, ingestionGoal)
                          }
                        >
            ...
            {ingestion.state.phase === "review" && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-success">
                  ...
                  <p className="text-sm font-semibold">
                    Pipeline completata · {ingestion.state.totalProposals} proposte pronte
                  </p>
                </div>
                <div className="rounded border border-border p-3 text-xs space-y-1">
                  ...
                  <p>
                    <strong>File:</strong> {ingestion.state.session?.source_file}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Fatti registrati:</strong> {sessionFactsCount}
                  </p>
                  <p>
          ...
          <TabsContent value="agentic" className="flex-1 overflow-auto pr-2 mt-4">
            <div className="rounded-md border border-primary/30 bg-primary/5 p-3 text-xs mb-4">
              <p className="font-semibold mb-1 flex items-center gap-1">
                <Sparkles className="h-3.5 w-3.5" /> Pipeline V2 — Entity by Entity
              </p>
              <p className="text-muted-foreground">
                Splitta il documento per heading, costruisce un Compact Index del DB (~5KB), processa ogni entità con
                una micro-call AI dedicata (~2K token). Token overflow impossibile, retry granulare per entità.
              </p>
            </div>

            {/* Banner ripristino dopo refresh: stato in cache, lavoro precedente recuperato. */}
            {(agentic.state.phase === "cancelled" || agentic.state.phase === "done") &&
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                      ...
                      {agentic.state.output ? (
                        <>
                          {" "}
                          (run id <code>{agentic.state.output.runId.slice(0, 8)}…</code>)
                        </>
                      ) : null}
                      .
                      {lastAgenticFileName ? (
                        <>
                          {" "}
                          Per ripartire ricarica <code>{lastAgenticFileName}</code>.
                        </>
                      ) : null}
                    </p>
                  </div>
            ...
            {agentic.state.phase === "idle" && (
              <div className="space-y-4">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  {agentic.state.entities.length > 0 && (
                    <Button
                      onClick={handleAgenticResume}
                      disabled={!userId || !canResumeAgentic}
                      variant="outline"
                      className="w-full"
                    >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.total}</strong> totali
                  </div>
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.inserts}</strong> INSERT
                  </div>
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.updates}</strong> UPDATE
                  </div>
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.skips}</strong> SKIP
                  </div>
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.needsReview}</strong> REVIEW
                  </div>
                  <div className="rounded border p-2">
                    <strong>{agentic.state.stats.errors}</strong> errori
                  </div>
                </div>
                <div className="text-xs text-muted-foreground">
                  Insert rate: {(agentic.state.stats.insertRate * 100).toFixed(0)}% · Fatti estratti:{" "}
                  {agentic.state.stats.factsExtracted}
                </div>
                ...
                {(agentic.state.output || agentic.state.reviewRun) && (
                  <p className="text-xs text-muted-foreground">
                    Run id:{" "}
                    <code>{(agentic.state.output?.runId ?? agentic.state.reviewRun?.id ?? "").slice(0, 8)}…</code> ·
                    Proposte salvate: {agentic.state.reviewRun?.proposals.length ?? 0}
                  </p>
                )}
                <div className="flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    onClick={handleOpenAgenticReview}
                    disabled={!agentic.state.reviewRun || agentic.state.reviewRun.proposals.length === 0}
                  >
                    Apri review e salva nel DB
                  </Button>
                  <Button variant="outline" size="sm" onClick={agentic.reset}>
                    Nuova pipeline
                  </Button>
                </div>
              </div>
