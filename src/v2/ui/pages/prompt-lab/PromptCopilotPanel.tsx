...
export default function PromptCopilotPanel(props: PromptCopilotPanelProps) {
  ...
  async function saveGlobalBatch() {
    ...
    try {
      ...
      toast.success(
        `${okCount}/${globalProposal.global_replacements.length} proposte salvate (batch ${batchId.slice(0, 8)})`,
      );
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b px-3 py-2 bg-muted/30 space-y-1.5">
        <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
          ...
          {mode === "block" ? (
            <>
              Blocco <span className="font-mono text-foreground">{blockName}</span> ·{" "}
      ...
      <div className={cn("flex-1 min-h-0 flex", compactWidth ? "flex-row" : "flex-col")}>
        {/* PROPOSTA: a sinistra (compactWidth) o sopra (mobile) */}
        <div
          className={cn(
            "bg-card overflow-hidden flex flex-col",
            compactWidth ? "w-1/2 border-r min-h-0" : cn("border-b", expanded ? "max-h-[60%]" : "max-h-[45%]"),
          )}
        >
            ...
            {mode === "global" ? (
              !globalProposal?.global_replacements?.length ? (
                <div className="p-4 text-[11px] text-muted-foreground italic">
                  Scrivi cosa cercare e cosa cambiare. Esempio:
                  <br />
                  <em>
                    "Ovunque dica che Luca è direttore del CRM di WSI Network Navigator, correggi: direttore del CRM di
                    Transport Management Worldwide Express (TMWE)."
                  </em>
                  <br />
                  <br />
                  L'AI cerca in tutti i prompt e in tutte le KB, esamina ogni occorrenza nel contesto e propone solo
                  dove ha senso.
                </div>
              ) : (
                <div className="p-3 space-y-2">
                  {globalProposal.global_replacements.map((r, i) => (
                    <div key={i} className="rounded border bg-background p-2 space-y-1 text-[11px]">
                      ...
                      <div className="bg-success/10 rounded px-2 py-1 text-[11px]">{r.new_excerpt}</div>
                    ...
                    <details className="text-[10px] text-muted-foreground">
                      ...
                      <ul className="mt-1 space-y-0.5">
                        {globalProposal.skipped.map((s, i) => (
                          <li key={i}>
                            <span className="font-mono">{s.source_id.slice(0, 8)}</span>: {s.reason}
                          </li>
                        ))}
                      </ul>
                    </details>
              ...
              <div className="p-4 text-[11px] text-muted-foreground italic">
                Nessuna proposta ancora. Scrivi nella chat sotto cosa vuoi migliorare (es.{" "}
                <em>"leggi il prompt e rendi più severo il guardrail"</em>) — l'AI consulta da sola la KB pertinente e
                propone qui sopra il nuovo testo.
              </div>
            ) : (
              <div className="p-3 space-y-2">
                ...
                <div className="flex gap-1.5 pt-1">
                  <Button
                    size="sm"
                    className="flex-1 h-7 gap-1.5 text-[11px]"
                    onClick={savePromptProposal}
                    disabled={savingPrompt || !promptId}
                  >
              ...
              {kbProposal && (
                <div className="rounded border bg-warning/10 p-2 mr-6 text-[11px] space-y-1.5">
                  ...
                  <div>
                    ...
                    <span className="font-mono">{kbProposal.suggested_category ?? "—"}</span> /{" "}
                    {kbProposal.suggested_chapter ?? "—"}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Titolo:</span> {kbProposal.suggested_title ?? "—"}
                  </div>
                  {kbProposal.duplicates_of && (
                    <div className="text-warning dark:text-warning">
                      ⚠ Possibile duplicato di: {kbProposal.duplicates_of}
                    </div>
                  )}
                  ...
                  <div className="flex gap-1.5 pt-1">
                    <Button
                      ...
                      onClick={saveKbProposal}
                      disabled={savingKb}
                    >
                      {savingKb ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3 w-3" />}
                      Salva proposta KB
                    </Button>
                    <Button size="sm" variant="ghost" className="h-7 text-[11px]" onClick={() => setKbProposal(null)}>
                      Scarta
                    </Button>
                  </div>
                </div>
          ...
          <div className="border-t p-2 space-y-1.5">
            ...
            <div className="flex gap-1.5 items-end">
              ...
              <Textarea
                ...
                rows={1}
                placeholder={
                  attachedMaterial ? "Spiega cosa è e premi invio…" : "Parla all'AI: leggi, migliora, valuta…"
                }
              ...
              <Button
                size="sm"
                onClick={send}
                disabled={busy || (!input.trim() && !attachedMaterial.trim())}
                className="h-9 w-9 p-0 flex-shrink-0"
              >
