...
export function validateAgainstRubric(text: string, r: PromptRubric): string[] {
  ...
  if (
    r.kindLabel.toLowerCase().includes("email") &&
    !/\?|risponda|conferm|fissi|prenoti|scelga|mi dica|valuti/i.test(text)
  ) {
...
const RUBRIC_SYSTEM_PROMPT_BLOCK: PromptRubric = {
  ...
  purpose:
    "Componente del prompt assemblato in tutti gli agenti AI prima di ogni generazione (LUCA, Director, agenti commerciali).",
  ...
  goodExample:
    "Mai inviare comunicazioni senza approvazione esplicita. Per email: bozza → review → invio. Per WhatsApp: solo se lead_status >= engaged.",
  badExample: {
    text: "Sii sempre gentile e prova a fare del tuo meglio.",
    why: "non operativo, non misurabile, nessun vincolo",
  },
};
...
const RUBRIC_KB_DOCTRINE: PromptRubric = {
  ...
  goodExample:
    "REGOLA: Nessun outbound a partner blacklisted. PROCEDURA: pre-send check su blacklist_entries.matched_partner_id. ECCEZIONE: solo email di chiarimento legale autorizzata da admin.",
  badExample: {
    text: "È importante essere sempre cauti con i partner sospetti.",
    why: "non specifica trigger, procedura né eccezione",
  },
};
...
const RUBRIC_OPERATIVE: Record<"objective" | "procedure" | "criteria" | "context" | "examples", PromptRubric> = {
  objective: {
    ...
    goodExample:
      "Permettere all'agente commerciale di generare un primo-contatto via email che ottenga risposta entro 7 giorni.",
  ...
  procedure: {
    ...
    goodExample:
      "1) Leggi partner.country e contatto.role 2) Carica KB doctrine per quel paese 3) Componi subject in lingua locale 4) Body con CTA singola.",
  ...
  criteria: {
    ...
    structure:
      "Lista di check binari: «✓ subject < 60 char», «✓ una sola CTA», «✗ non contiene placeholder non risolti».",
  ...
  context: {
    ...
    goodExample:
      "Leggi partners.country, partners.last_contact_at, partner_contacts.role. Considera solo lead in stato holding o engaged.",
  ...
  examples: {
    ...
    structure: "INPUT: <…>\\nOUTPUT: <…>\\n---\\nINPUT: <…>\\nOUTPUT: <…>",
    goodExample:
      'INPUT: partner FR, primo contatto. OUTPUT subject: "Optimisation logistique pour {{company}}" body: ...',
...
const RUBRIC_EMAIL_TYPE: PromptRubric = {
  ...
  goodExample:
    "TIPO: follow-up dopo silenzio 7gg. TONO: professionale, curioso. STRUTTURA: 1 frase hook su loro contesto, 1 frase valore concreto, 1 domanda chiusa. VINCOLI: ≤80 parole, no allegati.",
...
const RUBRIC_EMAIL_INSTRUCTIONS: PromptRubric = {
  ...
  goodExample:
    "Lingua: stessa del destinatario. Tono: pari-pari, no superiorità. Corpo ≤80 parole. Esattamente 1 CTA. Subject ≤60 char senza emoji.",
...
const RUBRIC_EMAIL_ADDRESS_RULE: PromptRubric = {
  ...
  goodExample:
    "QUANDO: destinatario @procurement.*. OVERRIDE: tono più formale, niente claim commerciali, includere SLA.",
...
const RUBRIC_PLAYBOOK_PROMPT: PromptRubric = {
  ...
  goodExample:
    "TRIGGER: lead engaged senza risposta da 14gg. STRATEGIA: prova canale alternativo (WhatsApp se autorizzato). AZIONI: 1) verifica gate WA 2) bozza msg breve 3) attendi 48h. VINCOLI: max 1 tentativo.",
...
const RUBRIC_PLAYBOOK_DESCRIPTION: PromptRubric = {
  ...
  goodExample:
    "Recupera lead engaged silenti da 14gg con un singolo messaggio breve sul canale alternativo autorizzato.",
...
const RUBRIC_PERSONA_TONE: PromptRubric = {
  ...
  purpose:
    "Definisce tono e stile dell'agente per tutte le generazioni TESTUALI (email, chat, WA). NON usato per voce.",
  targetLengthChars: { min: 80, max: 500 },
  ...
  structure: "TONO: <…>\\nAPERTURE: <es.>\\nCHIUSURE: <es.>\\nVOCABOLARIO SÌ/NO: <…>",
  goodExample:
    'TONO: diretto, professionale, mai paternalistico. APERTURE: "Buongiorno [Nome],". CHIUSURE: "Resto a disposizione.". SÌ: opportunità, valore. NO: "vorrei proporle", "in bocca al lupo".',
...
const RUBRIC_VOICE: PromptRubric = {
  ...
  structure:
    "# Personality / # Environment / # Tone / # Goal / # Tools / # Guardrails / # Pronunciation & Language / # When to end the call. Vedi template Aurora/Bruce/Robin in docs/PROMPT_11LABS_*.md.",
  goodExample:
  ...
  badExample: {
    text: "* Sei un agente vocale\\n* Devi essere gentile\\n## Regole\\n- Non urlare",
    why: "usa bullet markdown e heading ## che ElevenLabs interpreta male, manca la struttura canonica # Personality/# Environment, manca # When to end the call",
  },
};
...
const RUBRIC_AGENT_SYSTEM_PROMPT: PromptRubric = {
  ...
  mustHave: [
    "Identità agente",
    "Scope (cosa può/non può fare)",
    "Riferimento ai tool disponibili",
    "Regole specifiche del ruolo",
  ],
  mustNotHave: ["Duplicazione del system prompt globale", "Regole che contraddicono la dottrina 9 stati"],
  structure: "RUOLO\\nSCOPE\\nTOOL DISPONIBILI\\nREGOLE\\nDOTTRINA SPECIFICA",
  goodExample:
    "RUOLO: agente commerciale per partner FR/BE. SCOPE: outbound email + follow-up. TOOL: send_email, schedule_followup. REGOLE: francese sempre, mai inviare prima delle 9 locali.",
