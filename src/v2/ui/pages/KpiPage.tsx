          ...
          <>
            <section className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                />
                <StatCard label="Partner totali" value={data.enrichment.total.toLocaleString("it-IT")} />
                <StatCard label="Arricchiti" value={data.enrichment.enriched.toLocaleString("it-IT")} />
              </div>
            </section>
            ...
            <section className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                />
                <StatCard label="Inviate (sent)" value={data.email.sent30d.toLocaleString("it-IT")} />
                <StatCard
            ...
            <section className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                />
                <StatCard label="Completed" value={data.agents.completed.toLocaleString("it-IT")} />
                <StatCard
                  label="Failed"
            ...
            <section className="space-y-2">
              ...
              <div className="rounded-xl border border-border/60 bg-card/60 p-4">
                {Object.keys(data.funnel).length === 0 ? (
                  <p className="text-sm text-muted-foreground">Nessun dato lead_status disponibile.</p>
                ) : (
                  <div className="space-y-1.5">
                    {Object.entries(data.funnel)
                      .sort((a, b) => b[1] - a[1])
                      .map(([status, count]) => {
                        const total = Object.values(data.funnel).reduce((s, n) => s + n, 0);
                        ...
                        return (
                          <div key={status}>
                            ...
                            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted">
                              <div className="h-full bg-primary" style={{ width: `${Math.max(p, 1)}%` }} />
                            </div>
                          </div>
                        );
                      })}
