...
const TOOLS = [
  {
    to: "/v2/ai-staff/prompt-lab",
    icon: Library,
    label: "Prompt Lab",
    desc: "Editor prompt operativi, personas, capabilities, routing, journalist.",
  },
  {
    to: "/v2/prompt-lab/catalog",
    icon: BookOpen,
    label: "Catalogo prompt",
    desc: "Vista unificata dei prompt operativi con versione e orchestratori.",
  },
  {
    to: "/v2/prompt-lab/tests",
    icon: FlaskConical,
    label: "Test prompt",
    desc: "Scenari salvati con assertions pass/fail.",
  },
  { to: "/v2/ai-test-hub", icon: Sparkles, label: "AI Test Hub", desc: "Esecuzione massiva di scenari sulle edge AI." },
  {
    to: "/v2/prompt-lab/atlas",
    icon: GitBranch,
    label: "Agent Atlas",
    desc: "Mappa visuale degli agenti e delle loro capacità.",
  },
  {
    to: "/v2/pipeline-traces",
    icon: Activity,
    label: "Pipeline Traces",
    desc: "Stage-by-stage di tutte le edge function (live, per trace, per step).",
  },
  {
    to: "/v2/ai-interactions-log",
    icon: MessageCircle,
    label: "AI Interaction Log",
    desc: "Storico messaggi AI con feedback thumbs e export.",
  },
          ...
          <Button variant="ghost" className="h-auto w-full justify-start gap-2 rounded-md px-3 py-2 text-left">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-foreground">
                8 pagine collegate per editing, versioning, test, tracing e log dei prompt.
              </div>
            </div>
          </Button>
        </CollapsibleTrigger>
          ...
          <CardContent className="grid gap-2 pt-0 sm:grid-cols-2 lg:grid-cols-4">
            {TOOLS.map((t) => (
              <Link
                key={t.to}
                to={t.to}
                className="group rounded-md border border-border/50 bg-background/40 p-3 transition hover:border-primary/40 hover:bg-primary/5"
              >
