...
function VerdictBadge({ verdict, score }: { verdict?: string; score?: number }) {
  ...
  const map: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
    pass: {
      color: "bg-emerald-500/15 text-success border-emerald-500/40",
      icon: <ShieldCheck className="h-3 w-3" />,
      label: "Pass",
    },
    pass_with_edits: {
      color: "bg-sky-500/15 text-chart-3 border-sky-500/40",
      icon: <Sparkles className="h-3 w-3" />,
      label: "Pass + edits",
    },
    warn: {
      color: "bg-amber-500/15 text-warning border-amber-500/40",
      icon: <AlertTriangle className="h-3 w-3" />,
      label: "Warn",
    },
    block: {
      color: "bg-rose-500/15 text-destructive border-rose-500/40",
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="h-7 w-full justify-start gap-1.5 px-2 text-xs text-foreground hover:text-foreground"
        >
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-sm font-semibold text-foreground">
            <span className="text-foreground mr-1">#{index + 1}</span>
            {iteration.label}
          </CardTitle>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-foreground">
          ...
          {dbg?.ai_latency_ms != null && (
            <>
              <span>·</span>
              <span className="flex items-center gap-1">
                <Clock className="h-3 w-3" /> {dbg.ai_latency_ms}ms
              </span>
            </>
          )}
          <span>·</span>
          ...
          {(dbg?.tokens_in != null || dbg?.tokens_out != null) && (
            <>
              <span>·</span>
              <span>{(dbg?.tokens_in ?? 0) + (dbg?.tokens_out ?? 0)} tok</span>
            </>
          )}
        </div>
      </CardHeader>
      <CardContent className="space-y-3 pt-0">
        <div>
          ...
          <div className="text-sm font-medium text-foreground">
            {r.subject || <em className="text-foreground">vuoto</em>}
          </div>
        </div>
        ...
        {review?.warnings && review.warnings.length > 0 && (
          <div className="space-y-1">
            <div className="text-[10px] uppercase tracking-wide text-foreground">
              Journalist · {review.warnings.length} warning
            </div>
        ...
        {dbg?.systemPrompt && (
          <CollapsibleSection
            icon={FileText}
            label={`System prompt (${Math.round((dbg.systemPrompt.length || 0) / 100) / 10}k char)`}
          >
            ...
            <div className="space-y-2">
              {dbg.blocks.map((b, i) => (
                <div key={i}>
                  ...
                  <pre className="max-h-40 overflow-auto whitespace-pre-wrap text-[11px] text-foreground">
                    {b.content}
                  </pre>
                </div>
              ))}
