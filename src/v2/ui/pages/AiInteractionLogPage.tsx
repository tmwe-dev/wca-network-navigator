...
export default function AiInteractionLogPage() {
  ...
  const handleRate = async (interaction_id: string, rating: -1 | 1) => {
    ...
    try {
      if (existing && existing.rating === rating) {
        ...
        const m = new Map(feedbackMap);
        m.delete(interaction_id);
        setFeedbackMap(m);
      } else {
  ...
  const exportJson = () => {
    ...
    downloadFile(
      `ai-interactions-${new Date().toISOString().slice(0, 10)}.json`,
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="text-xs text-muted-foreground">Chat testo</div>
            <div className="text-2xl font-bold">{stats.byType.get("chat_text") ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="text-xs text-muted-foreground">Voce AI (TTS)</div>
            <div className="text-2xl font-bold">{stats.byType.get("voice_tts") ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="text-xs text-muted-foreground">Voce utente</div>
            <div className="text-2xl font-bold">{stats.byType.get("voice_stt") ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            ...
            <div className="text-2xl font-bold">
              {stats.pos} / <span className="text-destructive">{stats.neg}</span>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        {rows.length === 0 && !loading && (
          <Card>
            <CardContent className="py-12 text-center text-sm text-muted-foreground">
              Nessuna interazione registrata con questi filtri.
            </CardContent>
          </Card>
        )}
        {rows.map((r) => {
              ...
              <CardContent className="pt-4 space-y-2">
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <Badge variant="outline" className="gap-1">
                    {typeIcon(toAiInteractionType(r.interaction_type))}{" "}
                    {TYPE_LABELS[toAiInteractionType(r.interaction_type) ?? "chat_text"]}
                  </Badge>
                ...
                {r.role === "assistant" && (
                  <div className="flex items-center gap-2 pt-1 border-t">
                    <Button
                      size="sm"
                      variant={fb?.rating === 1 ? "default" : "ghost"}
                      onClick={() => void handleRate(r.id, 1)}
                    >
                      <ThumbsUp className="h-3.5 w-3.5" />
                    </Button>
                    <Button
                      size="sm"
                      variant={fb?.rating === -1 ? "destructive" : "ghost"}
                      onClick={() => void handleRate(r.id, -1)}
                    >
                ...
                {openNoteFor === r.id && (
                  <div className="space-y-2 pt-2">
                    <Textarea
                      ...
                      placeholder="Cosa è andato storto? Cosa avresti voluto come risposta?"
                      rows={3}
                    />
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => void saveNote(r.id)}>
                        Salva nota
                      </Button>
                      <Button
                        size="sm"
