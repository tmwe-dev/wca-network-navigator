        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">Tono e Lingua</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">Regole di Stile</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="Es: Usa frasi brevi e dirette"
                  onKeyDown={(e) => e.key === "Enter" && addToList(styleRules, setStyleRules, newRule, setNewRule)}
                />
                <Button
                  size="icon"
                  variant="outline"
                  onClick={() => addToList(styleRules, setStyleRules, newRule, setNewRule)}
                >
                ...
                {styleRules.map((r, i) => (
                  <Badge key={i} variant="secondary" className="gap-1">
                    ...
                    <X
                      ...
                      onClick={() => removeFromList(styleRules, setStyleRules, i)}
                    />
                  </Badge>
                ))}
              </div>
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">Vocabolario</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input
                    value={newVocDo}
                    onChange={(e) => setNewVocDo(e.target.value)}
                    placeholder="Es: partnership, collaborazione"
                    onKeyDown={(e) => e.key === "Enter" && addToList(vocDo, setVocDo, newVocDo, setNewVocDo)}
                  />
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => addToList(vocDo, setVocDo, newVocDo, setNewVocDo)}
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input
                    value={newVocDont}
                    onChange={(e) => setNewVocDont(e.target.value)}
                    placeholder="Es: SPAM, urgente!!!"
                    onKeyDown={(e) => e.key === "Enter" && addToList(vocDont, setVocDont, newVocDont, setNewVocDont)}
                  />
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => addToList(vocDont, setVocDont, newVocDont, setNewVocDont)}
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
                <div className="flex flex-wrap gap-1 mt-1">
                  {vocDont.map((v, i) => (
                    <Badge key={i} variant="destructive" className="gap-1">
                      {v}{" "}
                      <X className="h-3 w-3 cursor-pointer" onClick={() => removeFromList(vocDont, setVocDont, i)} />
                    </Badge>
                  ))}
                </div>
              </div>
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-sm">Messaggi d'Esempio</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {examples.map((ex, i) => (
                <div key={i} className="flex gap-2 items-start">
                  <Badge variant="outline" className="mt-1 min-w-[60px] justify-center">
                    {ex.role}
                  </Badge>
                  <Textarea
                    value={ex.content}
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setExamples([...examples, { role: "agent", content: "" }])}
                >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setExamples([...examples, { role: "user", content: "" }])}
                >
