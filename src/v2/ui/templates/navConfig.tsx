...
export const FULL_NAV_ITEMS: readonly NavItemDef[] = [
  ...
  {
    labelKey: "nav.email_intelligence",
  ...
  {
    labelKey: "nav.rubrica_linkedin",
    path: "/v2/rubrica/linkedin",
 ...
 */
export const navGroupsDef: readonly NavGroupDef[] = [{ titleKey: "nav.group_main", items: navItemsDef }];
...
export const mobileBottomNavPaths = ["/v2/command", "/v2/inbox", "/v2/cockpit", "/v2/settings"] as const;
 ...
 */
export type MacroAreaKey = "comando" | "esplora" | "pipeline" | "comunica" | "cervello" | "lab" | "config";
...
export const macroAreaGroups: readonly MacroAreaGroup[] = MACRO_AREAS.map((area) => ({
  key: area.key,
  label: area.label,
  items: area.paths.map((p) => ITEM_BY_PATH.get(p)).filter((i): i is NavItemDef => Boolean(i)),
