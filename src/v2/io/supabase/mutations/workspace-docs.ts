...
export async function createWorkspaceDoc(doc: WorkspaceDocInsert): Promise<Result<void, AppError>> {
  try {
    const { error } = await supabase.from("workspace_documents").insert(doc);
    if (error)
      return err(ioError("DATABASE_ERROR", error.message, { table: "workspace_documents" }, "createWorkspaceDoc"));
    return ok(undefined);
...
export async function deleteWorkspaceDoc(id: string): Promise<Result<void, AppError>> {
  try {
    ...
    if (error)
      return err(ioError("DATABASE_ERROR", error.message, { table: "workspace_documents", id }, "deleteWorkspaceDoc"));
    return ok(undefined);
  } catch (caught: unknown) {
