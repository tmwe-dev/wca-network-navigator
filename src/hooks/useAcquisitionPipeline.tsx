...
export function useAcquisitionPipeline() {
  ...
  useAcquisitionResume({
    ...
    setCompletedCount: state.setCompletedCount,
    setLiveStats: state.setLiveStats,
  ...
  const runExtensionLoop = useCallback(
    async (jobId: string, items: QueueItem[], startFrom = 0): Promise<LiveStats> => {
      ...
      for (let i = startFrom; i < items.length; i++) {
        ...
        if (state.excludedNetworksRef.current.size > 0 && item.networks?.length) {
          if (item.networks.every((n) => state.excludedNetworksRef.current.has(n))) {
            state.setQueue((prev) =>
              prev.map((q) => (q.wca_id === item.wca_id ? { ...q, status: "done" as const, skippedNetwork: true } : q)),
            );
        ...
        if (partnerId) {
          const [nets, svcs] = await Promise.all([findPartnerNetworks(partnerId), findPartnerServices(partnerId)]);
          canvas.networks = nets.map((n) => n.network_name);
          canvas.services = svcs.map((s) => s.service_category);
          if (canvas.networks.length > 0) {
            state.setQueue((prev) =>
              prev.map((q) => (q.wca_id === item.wca_id ? { ...q, networks: canvas.networks } : q)),
            );
          }
        }
        ...
        if (extensionAvailable || (await checkExtension())) {
          try {
            ...
            if (!bridgeResult.bridgeHealthy || !extResult || extResult.state === "not_loaded") {
              ...
              processedSet.add(item.wca_id);
              state.setQueue((prev) =>
                prev.map((q) => (q.wca_id === item.wca_id ? { ...q, status: "done" as const } : q)),
              );
              await updateDownloadJob(jobId, {
            ...
            if (extResult.success && extResult.contacts?.length) {
              ...
              if (extResult.companyName && !extResult.companyName.startsWith("WCA ")) {
                ...
                if (partnerId) {
                  const { updatePartner: up } = await import("@/data/partners");
                  await up(partnerId, { company_name: extResult.companyName });
                }
              }
              state.setCanvasData({ ...canvas });
            }
          } catch (extErr) {
            log.warn("extension call failed", {
              wcaId: item.wca_id,
              message: extErr instanceof Error ? extErr.message : String(extErr),
            });
          }
            ...
            try {
              ...
              if (dbContacts.length && dbContacts.some((c) => c.email || c.direct_phone || c.mobile)) {
                canvas.contacts = dbContacts.map((c) => ({
                  name: String(c.name ?? ""),
                  title: String(c.title ?? "") || undefined,
                  email: String(c.email ?? "") || undefined,
                  direct_phone: String(c.direct_phone ?? "") || undefined,
                  mobile: String(c.mobile ?? "") || undefined,
                }));
                canvas.contactSource = "extension";
                state.setCanvasData({ ...canvas });
              }
          ...
          if (!state.extensionWarningShown.current) {
            ...
            toast({
              ...
              description: "Installa l'estensione WCA per estrarre i contatti.",
              variant: "destructive",
            });
          }
              ...
              try {
                ...
                if (fsAvailable) {
                  try {
                    ...
                    if (r.success && r.markdown?.length > 50) {
                      enrichBody.markdown = r.markdown;
                      enrichBody.sourceUrl = r.metadata?.url || url;
                    }
                  } catch {
                    /* fallback */
                  }
            ...
            (async () => {
              try {
                await invokeEdge("ai-utility", {
                  body: { action: "deep_search", partnerId },
                  context: "pipeline.deep_search",
                });
              } catch {
                /* non-blocking */
              }
        ...
        if (parallelTasks.length > 0) {
          state.setCanvasPhase("enriching");
        ...
        state.setIsAnimatingOut(true);
        state.setShowComet(true);
        await new Promise((r) => setTimeout(r, 600));
        state.setShowComet(false);
        state.setIsAnimatingOut(false);
        state.setCompletedCount((c) => c + 1);

        const hasAnyContact = canvas.contacts.some(
          (c) => c.email?.trim() || c.direct_phone?.trim() || c.mobile?.trim(),
        );
        ...
        const hasComplete = canvas.contacts.some(
          (c) => !!c.email?.trim() && !!(c.direct_phone?.trim() || c.mobile?.trim()),
        );
        ...
        state.setLiveStats(localStats);
        if (hasComplete) state.setQualityComplete((v) => v + 1);
        else state.setQualityIncomplete((v) => v + 1);

        processedSet.add(item.wca_id);
        const contactResult =
          contactsWithEmail.length > 0 && contactsWithPhone.length > 0
            ? "email+phone"
  ...
  const actions = useAcquisitionPipelineActions(state, {
    ...
    waitForExtension,
    verifySession,
    runExtensionLoop: runExtensionLoop as unknown as (
      jobId: string,
  ...
  return {
    ...
    setDelaySeconds: state.setDelaySeconds,
    includeEnrich: state.includeEnrich,
    setIncludeEnrich: state.setIncludeEnrich,
    includeDeepSearch: state.includeDeepSearch,
    setIncludeDeepSearch: state.setIncludeDeepSearch,
    // Pipeline
    ...
    canvasPhase: state.canvasPhase,
    isAnimatingOut: state.isAnimatingOut,
    completedCount: state.completedCount,
    qualityComplete: state.qualityComplete,
    qualityIncomplete: state.qualityIncomplete,
    showComet: state.showComet,
    showSessionAlert: state.showSessionAlert,
    setShowSessionAlert: state.setShowSessionAlert,
    ...
    excludedNetworks: state.excludedNetworks,
