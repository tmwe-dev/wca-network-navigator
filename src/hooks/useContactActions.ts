...
export function useContactActions(deps: Deps) {
  ...
  const handleBulkCampaign = useCallback(async () => {
    ...
    try {
      ...
      const jobs = contacts.map((ct: unknown) => ({
...
async function exportContactsCsv(contactIds: string[]) {
  ...
  const rows = data.map((r: unknown) => [r.company_name, r.name, r.email, r.phone, r.mobile, r.country, r.city, r.address, r.zip_code, r.origin, r.lead_status, r.position, r.note].map(esc).join(";"));
...
async function sendToWorkspace(contactIds: string[], navigate: ReturnType<typeof useNavigate>) {
  if (!contactIds.length) return;
  const contacts = (await getContactsByIds(contactIds.slice(0, 200), "id, company_name, name, email, country, city")).filter((c: unknown) => c.email);
  if (!contacts?.length) { toast({ title: "Nessun contatto con email", variant: "destructive" }); return; }

  if (contacts.length === 1) {
    const ct = contacts[0] as unknown;
  ...
  const recipients = contacts.map((ct: unknown) => ({
    email: ct.email,
    name: ct.name || undefined,
    company: ct.company_name || undefined,
    contactId: ct.id,
