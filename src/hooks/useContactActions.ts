...
export function useContactActions(deps: Deps) {
  ...
  const handleDeepSearch = useCallback(
    async (contactIds: string[]) => {
      ...
      toast({
        title: "Deep Search disponibile tramite Partner Connect",
        description:
          "Usa il pulsante Deep Search nella vista Cockpit o Network per arricchire i contatti tramite l'estensione Partner Connect.",
      });
  ...
  const handleGroupDeepSearch = useCallback(
    async (group: ContactGroupCount) => {
  ...
  const handleGroupAlias = useCallback(
    async (group: ContactGroupCount) => {
      ...
      try {
        const ids =
          selection.count > 0
            ...
            : await fetchGroupContactIds(currentGroupBy, group.group_key, holdingPattern);
        if (!ids.length) {
        ...
        toast({
          title: processed === 0 ? "Alias già presenti" : "✨ Alias generati",
          description: processed === 0 ? "Tutti i contatti hanno già un alias" : `${processed} contatti elaborati`,
        });
        invalidateContacts();
      } catch (e: unknown) {
        toast({
          title: "Errore generazione alias",
    ...
    async (group: ContactGroupCount) => {
      ...
      setSelectedGroups((prev) => {
        ...
        if (next.has(key)) {
          ...
          setOpenGroups((p) => new Set(p).add(group.group_key));
        }
        return next;
      });
    },
  ...
  const handleLinkedInLookup = useCallback(
    async (contactIds: string[], lookupFn: (ids: string[]) => Promise<void>) => {
      if (linkedInLookupLoading || !contactIds.length) return;
      setLinkedInLookupLoading(true);
      try {
        await lookupFn(contactIds);
      } finally {
  ...
  const handleGroupLinkedInLookup = useCallback(
    async (group: ContactGroupCount, lookupFn: (ids: string[]) => Promise<void>) => {
      const ids = await fetchGroupContactIds(currentGroupBy, group.group_key, holdingPattern);
  ...
  const handleBulkCampaign = useCallback(async () => {
    ...
    try {
      ...
      const jobs = contacts.map((ct) => ({
        partner_id: String(ct.id),
        company_name: String(ct.company_name || ct.name || "Contatto"),
        country_code: String(ct.country || "XX"),
        country_name: String(ct.country || "Sconosciuto"),
        city: ct.city ? String(ct.city) : null,
        email: ct.email ? String(ct.email) : null,
        phone: ct.phone ? String(ct.phone) : null,
      ...
      const exec = async (c: AICommand) => {
        switch (c.type) {
          ...
          case "update_status":
            ...
            break;
          case "export_csv":
            await exportContactsCsv(c.contact_ids || []);
            break;
          case "send_to_workspace":
            await sendToWorkspace(c.contact_ids || [], navigate);
            break;
          case "create_jobs":
            await createCampaignJobsAction(c.contact_ids || [], selection, setSelectedGroups, navigate);
            break;
          case "multi":
            if (c.commands) {
  ...
  return {
    handleDeepSearch,
    handleGroupDeepSearch,
    handleGroupAlias,
    handleToggleGroupSelect,
    handleBulkCampaign,
    handleAICommand,
    handleLinkedInLookup,
    handleGroupLinkedInLookup,
    deepSearchLoading,
    aliasLoading,
...
async function exportContactsCsv(contactIds: string[]) {
  ...
  const data = await getContactsByIds(
    contactIds.slice(0, 500),
    "company_name, name, email, phone, mobile, country, city, address, zip_code, origin, lead_status, position, note",
  );
...
async function sendToWorkspace(contactIds: string[], navigate: ReturnType<typeof useNavigate>) {
  if (!contactIds.length) return;
  const contacts = (
    await getContactsByIds(contactIds.slice(0, 200), "id, company_name, name, email, country, city")
  ).filter((c) => c.email);
  if (!contacts?.length) {
    toast({ title: "Nessun contatto con email", variant: "destructive" });
    return;
  }
...
async function createCampaignJobsAction(
  contactIds: string[],
  selection: ReturnType<typeof useSelection>,
  setSelectedGroups: React.Dispatch<React.SetStateAction<Set<string>>>,
  ...
  const contacts = await getContactsByIds(
    contactIds.slice(0, 200),
    "id, company_name, name, email, phone, country, city",
  );
  if (!contacts?.length) {
    toast({ title: "Nessun contatto trovato", variant: "destructive" });
    return;
