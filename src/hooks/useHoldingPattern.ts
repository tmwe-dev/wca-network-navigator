...
export function useHoldingPatternList() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Partners
      const partners = await getPartnersByLeadStatus(ACTIVE_STATUSES, "id, company_name, country_name, country_code, city, email, lead_status, last_interaction_at, interaction_count") as any[]; // eslint-disable-line @typescript-eslint/no-explicit-any -- dynamic Supabase select
