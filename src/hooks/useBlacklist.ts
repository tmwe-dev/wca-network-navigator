...
export function useBlacklistStats() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const matched = entries?.filter((e: { matched_partner_id: string | null }) => e.matched_partner_id).length || 0;
      ...
      const { data: logs } = await supabase
        ...
        .order("created_at", { ascending: false })
        .limit(1);
      const lastUpdated = (logs as BlacklistSyncLog[] | null)?.[0]?.created_at || null;
...
export function useBlacklistByPartnerIds(partnerIds: string[]) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      const ids = new Set((data || []).map((d: { matched_partner_id: string | null }) => d.matched_partner_id));
      return ids;
    },
  });
}
    ...
    mutationFn: async (entries: Omit<BlacklistEntry, "id" | "created_at" | "updated_at">[]) => {
      ...
      if (allEntries && partners) {
        for (const entry of allEntries) {
          ...
          const match = partners.find((p: { id: string; company_name: string; country_name: string }) => {
