...
export function useActionPanelLogic({
  ...
  const { data: dbPartners = [], isLoading: loadingDb } = useQuery({
    ...
    queryFn: async () => {
      if (countryCodes.length === 0) return [];
      const data = await getPartnersByCountries(
        countryCodes,
        "wca_id, company_name, city, country_code, country_name, updated_at",
      );
      return (data || []).map((p) => ({
        wca_id: p.wca_id!,
  ...
  const cachedMembers: DirectoryMember[] = cachedEntries.flatMap((entry) => {
    ...
    return (members || []).map((raw) => {
      ...
      return {
        ...
        country_code: String(m.country_code || entry.country_code),
        wca_id: m.wca_id as number,
      };
    });
  });
  ...
  const noProfileInDirectoryCount = useMemo(
    () => uniqueIds.filter((id) => noProfileWcaSet.has(id)).length,
  ...
  // ── Scan ──
  const saveScanToCache = useCallback(
    async (countryCode: string, netKey: string, scanned: DirectoryMember[], total: number, pages: number) => {
  ...
  const executeDownload = useCallback(async () => {
    ...
    try {
      ...
      if (!cookie) {
        ...
        if (!data.success || !data.cookies) {
          toast({ title: "Login WCA fallito", description: data.error || "Riprova.", variant: "destructive" });
          return;
        }
      ...
      toast({ title: "Connessione WCA fallita", variant: "destructive" });
      return;
    }
    ...
    if (activeJobs.length > 0) {
      ...
      if (job.status === "running" && ageMs > 120_000) {
        ...
        toast({ title: "Job già in corso", description: "Attendi il completamento.", variant: "destructive" });
        return;
      }
    }
    ...
    for (const country of selectedCountries) {
      ...
      const jobId = await createJob.mutateAsync({
        ...
        wca_ids: countryIds,
        delay_seconds: Math.max(delay, 10),
      });
  ...
  useEffect(() => {
    ...
    const runCleanupAndDownload = async () => {
      ...
      if (freshWcaIds.size === 0) {
        toast({ title: "⚠️ Nessun partner trovato", variant: "destructive" });
        return;
      }
  ...
  return {
    ...
    setDirThenDownload,
    autoDownloadPending,
    setAutoDownloadPending,
    hasCache,
    totalCount,
    downloadedCount,
    missingIds,
    noProfileInDirectoryCount,
    idsToDownload,
    estimateLabel,
