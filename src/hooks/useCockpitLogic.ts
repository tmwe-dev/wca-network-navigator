...
import { briefToText } from "@/types/email-brief";
...
import type { CockpitAIAction, SourceTab } from "@/types/cockpit";
...
export function useCockpitLogic() {
  ...
  useEffect(() => {
    ...
    import("@/lib/cockpitPreselection").then(({ consumeCockpitPreselection }) => {
      ...
      const matchingIds = contacts.filter((c) => pendingIds.includes(c.sourceId)).map((c) => c.id);
  ...
  const autoAssign = useCallback(
    async (sourceId: string, sourceType: string) => {
  ...
  const executeAIActions = useCallback(
    (actions: CockpitAIAction[], message: string) => {
      for (const action of actions) {
        switch (action.type) {
          ...
          case "single_action": {
            ...
            if (contact)
              toast.info(`${action.action === "deep_search" ? "Deep Search" : "Genera Alias"} per ${contact.name}`);
  ...
  const handleDrop = useCallback(
    (channel: DraftChannel, _contactId: string, _contactName: string) => {
      ...
      const sourceType =
        contact.origin === "report_aziende" ? "prospect" : contact.origin === "import" ? "contact" : "partner";
  ...
  const handleStartGeneration = useCallback(async () => {
    ...
    if (pendingBulkIds.length > 0 && !signal.aborted) {
      ...
      setPendingBulkIds([]);
      if (!signal.aborted) {
        toast.success(`Bulk generazione completata`, {
          description: `${okCount}/${rest.length + 1} OK, ${errCount} errori`,
        });
      }
    }
  }, [
  ...
  const handleImprove = useCallback(async () => {
    ...
    if (cfg.selectedType?.prompt) goalParts.push(cfg.selectedType.prompt);
    goalParts.push(
      "MIGLIORA la bozza qui sotto mantenendo voce, intento e personalità dell'autore. Non riscrivere da zero.",
    );
    if (draftState.links && draftState.links.length > 0) {
      const linksList = draftState.links.map((l) => `- [${l.label}](${l.url})`).join("\n");
      goalParts.push(
        `Includi nel testo, in modo naturale e contestuale, i seguenti link (formato HTML <a href>):\n${linksList}`,
      );
    }
  ...
  const showQueuedDraft = useCallback(
  ...
  const handleBulkDeepSearch = useCallback(() => {
    ...
    if (!ids.length) {
      toast.info("Seleziona almeno un contatto");
      return;
    }
  ...
  const handleSingleLinkedInLookup = useCallback(
    (id: string) => {
  ...
  return {
    ...
    pendingBulkCount: pendingBulkIds.length,
    handleGenerateAfterReview,
    handleRegenerate,
    handleImprove,
    showQueuedDraft,
    contacts,
    ...
    handleBulkAlias,
    handleBulkLinkedInLookup,
    handleSingleDeepSearch,
    handleSingleAlias,
    handleSingleLinkedInLookup,
    handleBulkDelete,
    confirmBulkDelete,
    showDeleteConfirm,
