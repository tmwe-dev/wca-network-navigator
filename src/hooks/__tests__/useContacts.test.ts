...
describe("useContacts", () => {
  it("fetches contacts with default filters", async () => {
    vi.mocked(findContacts).mockResolvedValue({ items: [{ id: "c1" }], totalCount: 1, page: 1, pageSize: 50 } as unknown);
  ...
  it("passes filters to findContacts", async () => {
    vi.mocked(findContacts).mockResolvedValue({ items: [], totalCount: 0, page: 1, pageSize: 50 } as unknown);
  ...
  it("exposes loading state", () => {
    vi.mocked(findContacts).mockReturnValue(new Promise(() => {}) as unknown);
    const { result } = renderHook(() => useContacts(), { wrapper });
...
describe("useContactFilterOptions", () => {
  it("fetches filter options", async () => {
    vi.mocked(getContactFilterOptions).mockResolvedValue({ countries: ["IT", "DE"] } as unknown);
