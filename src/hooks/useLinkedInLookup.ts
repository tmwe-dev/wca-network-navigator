...
const getExistingLinkedInUrl = (enrichmentData: Record<string, unknown> | null): string | null => {
  ...
  return (
    (enrichmentData.linkedin_profile_url as string) ||
...
const INITIAL_PROGRESS: LookupProgress = {
  current: 0,
  total: 0,
  currentName: "",
  found: 0,
...
export function useLinkedInLookup() {
  ...
  const searchSingle = useCallback(
    async (contact: {
      ...
      try {
        ...
        const googleQueries = buildLinkedInGoogleQueries(
          contact.name,
          contact.company,
          contact.email,
          contact.role,
  ...
  const lookupBatch = useCallback(
    async (contactIds: string[]) => {
      if (!pcBridge.isAvailable) {
        toast({
          title: "Partner Connect non disponibile",
          description: "Installa l'estensione Partner Connect",
          variant: "destructive",
        });
        return;
      }
      ...
      const contacts = await getContactsByIds(
        ...
        "id, name, company_name, email, enrichment_data",
      );
      const error = null;
      ...
      toast({
        title: "Trova profilo LinkedIn completato",
        description: `${found} trovati, ${notFound} non trovati, ${skippedCount} già risolti`,
      });
    },
    [pcBridge],
  );
