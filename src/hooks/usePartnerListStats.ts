...
export function usePartnerListStats({ countryCodes, partners }: UsePartnerListStatsArgs) {
  ...
  const stats = useMemo(() => {
    ...
    list.forEach((p: unknown) => {
      ...
      if (p.email || (p.partner_contacts || []).some((c: unknown) => c.email)) withEmail++;
      if (p.phone || (p.partner_contacts || []).some((c: unknown) => c.direct_phone || c.mobile)) withPhone++;
      if (p.company_alias) withAliasCo++;
      if ((p.partner_contacts || []).some((c: unknown) => c.contact_alias)) withAliasCt++;
    });
  ...
  const verified = useMemo(() => {
    const list = partners || [];
    const missingEmailList = list.filter((p: unknown) => !p.email && !(p.partner_contacts || []).some((c: unknown) => c.email));
    const emailVerified = missingEmailList.length === 0 || missingEmailList.every((p: unknown) => !!p.raw_profile_html);
    const missingPhoneList = list.filter((p: unknown) => !p.phone && !(p.partner_contacts || []).some((c: unknown) => c.direct_phone || c.mobile));
    const phoneVerified = missingPhoneList.length === 0 || missingPhoneList.every((p: unknown) => !!p.raw_profile_html);
    const missingDeepList = list.filter((p: unknown) => !asEnrichment(p.enrichment_data)?.deep_search_at);
    const deepVerified = missingDeepList.length === 0 || missingDeepList.every((p: unknown) => !!asEnrichment(p.enrichment_data)?.deep_search_at);
    const missingAliasCoList = list.filter((p: unknown) => !p.company_alias);
    const aliasCoVerified = missingAliasCoList.length === 0 || missingAliasCoList.every((p: unknown) => !!p.ai_parsed_at);
    const missingAliasCtList = list.filter((p: unknown) => !(p.partner_contacts || []).some((c: unknown) => c.contact_alias));
    const aliasCtVerified = missingAliasCtList.length === 0 || missingAliasCtList.every((p: unknown) => !!p.ai_parsed_at);
