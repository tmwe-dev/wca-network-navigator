...
export function useDeepSearchLocal() {
  ...
  const googleSearch = useCallback(
    async (query: string, limit = 5): Promise<GoogleSearchResult[]> => {
      ...
      try {
        ...
        /* fallback al metodo navigate+extract */
      }
  ...
  const searchLinkedInForContacts = useCallback(
    async (
      contacts: Array<{
        id: string;
        name: string;
        ...
        if (enableLinkedIn && !existingSet.has(`${contact.id}_linkedin`)) {
          ...
          if (results.length > 0) {
            ...
            if (answer && answer !== "NONE" && answer.includes("linkedin.com/in/")) {
              ...
              if (m) {
                const { error } = await insertPartnerSocialLink({
  ...
  const searchCompanyLinkedIn = useCallback(
    async (companyName: string, partnerId: string, existingSet: Set<string>) => {
  ...
  const scrapeWebsite = useCallback(
    async (website: string | null, partnerId: string, contacts?: Array<{ email?: string | null }>) => {
  ...
  const searchPartner = useCallback(
    async (partnerId: string) => {
      const failResult = {
        success: false,
        socialLinksFound: 0,
        logoFound: false,
        contactProfilesFound: 0,
      ...
      const missing = {
        needCompanyLinkedIn:
          !existingED.linkedin_url && !existingED.linkedin_summary && !existingSet.has("company_linkedin"),
      ...
      if (totalSkipped > 0) {
        toast.info(
          `Deep Search incrementale: ${totalMissing} ricerche da fare, ${totalSkipped} già disponibili (skip).`,
        );
      }
      ...
      if (missing.needGoogleGeneral && cfg("googleGeneral", false) && contacts.length > 0) {
        ...
        for (const c of contacts.slice(0, 3)) {
          if (!c.name) continue;
          const m = await searchGoogleGeneral(
            fs,
            cleanPersonName(c.name),
      ...
      if (missing.needGoogleMaps && cfg("googleMaps", false)) {
        const gm = await scrapeGoogleMaps(
          fs,
          partner.company_name as string,
          (partner.city as string) || "",
          (partner.country_name as string) || "",
        );
      ...
      if (missing.needReputation && cfg("reputation", false)) {
        const rep = await scrapeReputation(
          fs,
          partner.company_name as string,
          partner.website as string | null,
          googleSearch,
        );
      ...
      const fieldsSkipped = Object.entries(missing)
        .filter(([, needed]) => !needed)
        .map(([k]) => k);
      const previousRuns = Array.isArray(existingED.deep_search_runs) ? (existingED.deep_search_runs as unknown[]) : [];
      const updated = {
        ...existingED,
        ...newFields,
      ...
      const rating = await calculateRating(
        ...
        websiteQualityScore,
        partner.website as string | null,
        partner.member_since as string | null,
        partner.branch_cities as string | null,
      );
      await updatePartner(partnerId, { rating });

      return {
        success: true,
        socialLinksFound: contactLinks + companyLinks,
        logoFound,
  ...
  const searchContact = useCallback(
    async (contactId: string) => {
      ...
      const contacts = (await getContactsByIds(
        [contactId],
        "id, name, company_name, email, phone, mobile, country, city, position, enrichment_data",
      ...
      if (cfg("scrapeWebsite", true) && websiteUrl) {
        ...
        if (scraped) {
          try {
            ...
            if (`https://www.google.com/s2/favicons?domain=${domain}&sz=128`) logoFound = true;
          } catch {
            /* best-effort */
          }
          if (scraped.markdown && scraped.markdown.length > 100) {
