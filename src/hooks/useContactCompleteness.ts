...
export function getPartnerContactQuality(partnerContacts: { email?: string | null; direct_phone?: string | null; mobile?: string | null }[] | undefined): "complete" | "partial" | "missing" {
  if (!partnerContacts || partnerContacts.length === 0) return "missing";
  const hasEmail = partnerContacts.some((c: { email?: string | null }) => !!c.email);
  const hasPhone = partnerContacts.some((c: { direct_phone?: string | null; mobile?: string | null }) => !!c.direct_phone || !!c.mobile);
