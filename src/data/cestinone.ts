...
function countryName(code: string | null | undefined): string | null {
  if (!code) return null;
  try {
    return COUNTRY_NAMER?.of(code.toUpperCase()) ?? code.toUpperCase();
  } catch {
...
export async function fetchCestinone(): Promise<CestinoItem[]> {
  ...
  const [emailQ, campaignQ, cockpitQ, outreachQ] = await Promise.allSettled([
    supabase
      ...
      .select(
        "id, partner_id, recipient_name, recipient_email, subject, html_body, status, scheduled_at, created_at, retry_count, error_message, operator_id",
      )
    ...
    supabase
      .from("campaign_jobs")
      .select(
        "id, partner_id, company_name, country_code, email, phone, job_type, status, batch_id, created_at, operator_id, notes, assigned_to",
      )
      .eq("status", "pending")
      .order("created_at", { ascending: false })
      .limit(500),
  ...
  if (campaignQ.status === "fulfilled" && campaignQ.value.data) {
    for (const r of campaignQ.value.data as Array<Record<string, unknown>>) {
      ...
      out.push({
        ...
        originContext: {
          source: "campaign",
          label: r.batch_id ? `Job campagna · Batch ${String(r.batch_id).slice(0, 8)}` : "Job campagna",
        },
        previousMessage: null,
        recentInteractions: [],
        bodyText: notes,
    ...
    for (const r of outreachQ.value.data as Array<Record<string, unknown>>) {
      ...
      const handle =
        ch === "whatsapp"
          ...
          : ch === "linkedin"
            ? (r.recipient_linkedin_url as string)
      ...
      out.push({
        ...
        originContext: {
          source: isReply ? "inbound_reply" : "manual",
          label: isReply ? "Risposta a messaggio inbound" : "Outreach manuale multicanale",
        },
        previousMessage: null,
        recentInteractions: [],
        bodyText: body,
  ...
  const operatorIds = Array.from(
    new Set(out.map((i) => i.agentName).filter((v): v is string => !!v && /^[0-9a-f-]{36}$/i.test(v))),
  );
  ...
  const [partnersRes, profilesRes, sherlockRes, bcaRes, interactionsRes, lastInboundRes] = await Promise.allSettled([
    partnerIds.length
      ? supabase
          .from("partners")
          .select("id, company_name, country_code, country_name, city, lead_status, partner_type, wca_id")
          .in("id", partnerIds)
    ...
    partnerIds.length
      ? supabase
          .from("business_cards")
          .select("matched_partner_id, event_name, location, met_at")
          .in("matched_partner_id", partnerIds)
      : Promise.resolve({ data: [] as Array<Record<string, unknown>> }),
    partnerIds.length
      ? supabase
          .from("interactions")
          .select("partner_id, interaction_type, interaction_date, subject, notes")
          .in("partner_id", partnerIds)
    ...
    partnerIds.length
      ? supabase
          ...
          .select("partner_id, channel, direction, subject, body_text, created_at")
          .in("partner_id", partnerIds)
  ...
  if (operatorIds.length) {
    try {
      const { data: agentsData } = await supabase.from("agents").select("id, name").in("id", operatorIds);
      if (agentsData) {
      ...
      /* tabella opzionale: nessuna interruzione */
    }
  }
...
export async function cancelCestinoItem(item: CestinoItem): Promise<void> {
  ...
  switch (item.source) {
    ...
    case "cockpit_queue": {
      const { error } = await supabase.from("cockpit_queue").delete().eq("id", realId);
      if (error) throw error;
      break;
    }
    case "outreach_queue": {
