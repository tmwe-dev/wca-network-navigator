...
export async function findLiveProfilePartners(wcaIds: number[]): Promise<unknown[]> {
  const { data } = await supabase
    .from("partners")
    .select(
      "id, company_name, city, country_code, email, phone, website, wca_id, partner_networks(network_name), partner_contacts(name, email, title)",
    )
    .in("wca_id", wcaIds);
  return data ?? [];
}
...
export async function findAgendaPartnersByCountry(countryCode: string): Promise<unknown[]> {
  const { data } = await supabase
    .from("partners")
    .select(
      "id, company_name, city, country_code, email, phone, wca_id, partner_networks(network_name), partner_contacts(name, email, mobile)",
    )
