...
export async function findMemoryTypesByUser(userId: string): Promise<{ memory_type: string }[]> {
  const { data } = await supabase.from("ai_memory").select("memory_type").eq("user_id", userId);
  return data ?? [];
}
...
export async function deleteEpisodicMemoriesForUser(userId: string): Promise<void> {
  const { error } = await supabase.from("ai_memory").delete().eq("user_id", userId).eq("memory_type", "episodic");
  if (error) throw error;
}
