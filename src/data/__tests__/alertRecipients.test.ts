...
describe("DAL — alertRecipients", () => {
  describe("listAlertRecipients", () => {
    it("returns recipients for user", async () => {
      const recipients = [{ id: "r1", user_id: "u1", name: "Test", role: null, whatsapp_e164: "+1", email: null, categories: [], min_urgency_score: 0, is_active: true, quiet_hours_start: null, quiet_hours_end: null, timezone: "UTC", notes: null, created_at: "t", updated_at: "t" }];
  ...
  describe("listAlertDispatchLog", () => {
    it("returns dispatch log entries", async () => {
      const logs = [{ id: "d1", user_id: "u1", recipient_id: "r1", message_id: null, channel: "whatsapp", business_category: null, urgency_score: null, alert_categories: null, payload: {}, status: "sent", error: null, created_at: "t" }];
