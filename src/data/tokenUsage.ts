...
export async function getTodayUsage(userId: string): Promise<number> {
  ...
  if (error) {
    console.error("[tokenUsage] Error getting today usage:", error);
    return 0;
  }
...
export async function getMonthUsage(userId: string): Promise<number> {
  ...
  if (error) {
    console.error("[tokenUsage] Error getting month usage:", error);
    return 0;
  }
...
export async function getUsageByFunction(userId: string, days: number = 7): Promise<Record<string, number>> {
  ...
  if (error) {
    console.error("[tokenUsage] Error getting usage by function:", error);
    return {};
  }
...
export async function getTokenSettings(userId: string): Promise<Record<string, string>> {
  ...
  if (error) {
    console.error("[tokenUsage] Error getting token settings:", error);
    return {};
  }
