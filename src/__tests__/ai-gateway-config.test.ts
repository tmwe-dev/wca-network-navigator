...
describe("aiGatewayConfig", () => {
  describe("PROVIDER_CONFIG", () => {
    ...
    it("should include all expected provider keys", () => {
      ...
      expectedProviders.forEach((_provider) => {
        expect(actualProviders).toContain(_provider);
      });
    });

    it("each provider should have url and authHeader properties", () => {
      Object.entries(PROVIDER_CONFIG).forEach(([_provider, config]) => {
    ...
    it("lovable provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.lovable.url).toBe("https://ai.gateway.lovable.dev/v1/chat/completions");
    });

    it("openrouter provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.openrouter.url).toBe("https://openrouter.ai/api/v1/chat/completions");
    });

    it("openai provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.openai.url).toBe("https://api.openai.com/v1/chat/completions");
    });

    it("anthropic provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.anthropic.url).toBe("https://api.anthropic.com/v1/messages");
    });
    ...
    it("grok provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.grok.url).toBe("https://api.x.ai/v1/chat/completions");
    });

    it("qwen provider should have correct URL", () => {
      expect(PROVIDER_CONFIG.qwen.url).toBe("https://dashscope-intl.aliyuncs.com/compatible-mode/v1/chat/completions");
    });

    it("authHeader should generate Bearer token for most providers", () => {
      const testKey = "test-api-key-123";
      expect(PROVIDER_CONFIG.lovable.authHeader(testKey)).toBe("Bearer test-api-key-123");
      expect(PROVIDER_CONFIG.openrouter.authHeader(testKey)).toBe("Bearer test-api-key-123");
      expect(PROVIDER_CONFIG.openai.authHeader(testKey)).toBe("Bearer test-api-key-123");
      expect(PROVIDER_CONFIG.google.authHeader(testKey)).toBe("Bearer test-api-key-123");
      expect(PROVIDER_CONFIG.grok.authHeader(testKey)).toBe("Bearer test-api-key-123");
      expect(PROVIDER_CONFIG.qwen.authHeader(testKey)).toBe("Bearer test-api-key-123");
    });

    it("anthropic authHeader should return just the key without Bearer prefix", () => {
      const testKey = "test-api-key-123";
      expect(PROVIDER_CONFIG.anthropic.authHeader(testKey)).toBe("test-api-key-123");
    });
    ...
    it("authHeader should handle special characters in key", () => {
      ...
      expect(PROVIDER_CONFIG.openai.authHeader(specialKey)).toBe("Bearer key-with-special_chars.123");
    });
  });
  ...
  describe("MODEL_MAP", () => {
    ...
    it("should include all expected providers in MODEL_MAP", () => {
      ...
      expectedProviders.forEach((_provider) => {
        expect(MODEL_MAP).toHaveProperty(_provider);
      });
    });
    ...
    it("google should map google/gemini-2.5-flash to gemini-2.5-flash", () => {
      expect(MODEL_MAP.google["google/gemini-2.5-flash"]).toBe("gemini-2.5-flash");
    });

    it("google should map google/gemini-2.5-flash-lite to gemini-2.5-flash-lite", () => {
      expect(MODEL_MAP.google["google/gemini-2.5-flash-lite"]).toBe("gemini-2.5-flash-lite");
    });

    it("grok should map multiple models to grok-3-mini-fast", () => {
      expect(MODEL_MAP.grok["google/gemini-2.5-flash"]).toBe("grok-3-mini-fast");
      expect(MODEL_MAP.grok["openai/gpt-5"]).toBe("grok-3-mini-fast");
      expect(MODEL_MAP.grok["google/gemini-2.5-flash-lite"]).toBe("grok-3-mini-fast");
    });

    it("qwen should map models correctly", () => {
      expect(MODEL_MAP.qwen["google/gemini-3-flash-preview"]).toBe("qwen-plus");
      expect(MODEL_MAP.qwen["google/gemini-2.5-flash"]).toBe("qwen-turbo");
    });

    it("all values in MODEL_MAP should be strings", () => {
      Object.entries(MODEL_MAP).forEach(([_provider, modelMap]) => {
