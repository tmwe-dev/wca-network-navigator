...
export const PROCEDURES_PART2: OperationProcedure[] = [
  {
    ...
    steps: [
      {
        order: 1,
        action: "Verifica sito web",
        tool: "get_partner_detail",
        detail: "Conferma che il campo website sia presente.",
      },
      {
        order: 2,
        action: "Avvia enrichment",
        tool: "enrich_partner_website",
        detail: "Scraping e analisi AI del sito.",
      },
  ...
  // ══════════════════════════════════════
  {
    ...
    steps: [
      {
        order: 1,
        action: "Carica il file",
        tool: null,
        detail: "L'utente carica il file CSV/Excel dalla pagina Import.",
      },
      {
        order: 2,
        action: "Analizza struttura",
        tool: null,
        detail: "Il sistema analizza automaticamente le colonne con AI.",
      },
      {
        order: 3,
        action: "Mappa le colonne",
        tool: null,
        detail: "L'utente conferma o corregge la mappatura colonne.",
      },
  ...
  },
  {
    ...
    steps: [
      { order: 1, action: "Identifica il contatto", tool: "get_contact_detail", detail: "Ottieni dettagli completi." },
      {
        ...
        tool: "deep_search_contact",
        detail: "Cerca LinkedIn, social, info web.",
      },
  ...
  },
  {
    ...
    description:
      "[STUB] Migrata in KB. Vedi procedures/lead-qualification-v2 (9 stati, exit_reason obbligatorio per archived/blacklisted).",
    tags: ["lead", "status", "stub", "kb-redirect"],
    category: "crm",
    prerequisites: [{ check: "kb_consult", label: "Consulta KB procedures/lead-qualification-v2", tool: "search_kb" }],
    steps: [
      {
        order: 1,
        action: "Consulta KB procedures/lead-qualification-v2",
        tool: "search_kb",
        detail:
          "9 stati: new, first_touch_sent, holding, engaged, qualified, negotiation, converted, archived, blacklisted. Exit_reason obbligatorio per uscite.",
      },
      {
        order: 2,
        action: "Bulk >5 record → CONFERMA esplicita utente",
        tool: null,
        detail: "Mai update massivi senza approvazione.",
      },
    ],
  ...
  },
  {
    ...
    steps: [
      {
        order: 1,
        action: "Filtra contatti",
        tool: "search_contacts",
        detail: "Applica filtri per selezionare i contatti da esportare.",
      },
  ...
  },
  {
    ...
    steps: [
      ...
      {
        order: 2,
        action: "Crea l'attività",
        tool: "create_activity",
        detail: "Tipo, titolo, descrizione, due_date, priority.",
      },
      {
        order: 3,
        action: "Conferma creazione",
        tool: "list_activities",
        detail: "Verifica che l'attività sia stata creata.",
      },
    ],
  ...
  // ══════════════════════════════════════
  {
    ...
    steps: [
      ...
      {
        order: 2,
        action: "Crea attività follow-up",
        tool: "create_activity",
        detail: "activity_type='follow_up', due_date tra 3-7 giorni.",
      },
      {
        order: 3,
        action: "Crea reminder",
        tool: "create_reminder",
        detail: "Reminder con due_date uguale all'attività.",
        optional: true,
      },
    ],
  ...
  },
  {
    ...
    steps: [
      {
        order: 1,
        action: "Identifica partecipanti",
        tool: "get_partner_detail",
        detail: "Ottieni contatti del partner.",
      },
      {
        order: 2,
        action: "Crea attività meeting",
        tool: "create_activity",
        detail: "activity_type='meeting', scheduled_at, due_date.",
      },
      {
        order: 3,
        action: "Genera email di invito",
        tool: "generate_outreach",
        detail: "Email di conferma appuntamento.",
        optional: true,
      },
    ],
  ...
  },
  {
    ...
    steps: [
      {
        order: 1,
        action: "Elenca reminder attivi",
        tool: "list_reminders",
        detail: "Mostra tutti i reminder pending/completati.",
      },
      {
        order: 2,
        action: "Crea/aggiorna reminder",
        tool: "create_reminder",
        detail: "Nuovo reminder con titolo, data, priorità.",
      },
  ...
  // ══════════════════════════════════════
  {
    ...
    steps: [
      {
        order: 1,
        action: "Seleziona target",
        tool: "search_partners",
        detail: "Filtra partner senza alias (company_alias IS NULL).",
      },
      {
        order: 2,
        action: "Genera alias",
        tool: "generate_aliases",
        detail: "Passa partner_ids o country_code. Type: 'company' o 'contact'.",
      },
    ...
    tips: ["Genera per paese per risultati più consistenti", "Limita a 20 partner per batch per evitare timeout"],
  },
  {
    id: "blacklist_check",
    ...
    steps: [
      ...
      {
        order: 2,
        action: "Mostra risultati",
        tool: null,
        detail: "Se trovato, mostra dettagli: importo dovuto, numero claims, status.",
      },
    ],
  ...
  },
  {
    ...
    steps: [
      {
        order: 1,
        action: "Definisci filtro",
        tool: "search_partners",
        detail: "Filtra i partner da aggiornare. Mostra conteggio.",
      },
      {
        order: 2,
        action: "Chiedi conferma",
        tool: null,
        detail: "OBBLIGATORIO: mostra quanti record verranno modificati.",
      },
      {
        order: 3,
        action: "Esegui aggiornamento",
        tool: "bulk_update_partners",
        detail: "Applica le modifiche in blocco.",
      },
      {
        order: 4,
        action: "Verifica esito",
        tool: "search_partners",
        detail: "Ricarica i dati per confermare le modifiche.",
      },
    ],
    related_pages: ["/partner-hub"],
