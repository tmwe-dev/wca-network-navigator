...
for (const f of files) {
  ...
  for (const [k, re] of Object.entries(pats)) {
    ...
    totals[k] += n; row[k] = n; if (k !== "eslintDisable") sum += n;
  }
...
if (process.argv.includes("--top")) {
  console.log("\nTOP 25:");
  for (const r of perFile.slice(0,25)) console.log(`${String(r.sum).padStart(4)}  uf=${r.untypedFrom} nev=${r.asNever} uk=${r.asUnknownAs} any=${r.explicitAny}  ${r.file}`);
}
