...
2. **28 valori di category enumerati nel JSON di output** (righe 110-111) — è uno _schema di output_ quindi il principio dice "ok, è contratto" MA: la lista è duplicata in `VALID_CATEGORIES` (riga 127-133) e dovrebbe arrivare da `pg_enum` (`email_classification_category`) — stesso pattern di `ai-query-planner`.
