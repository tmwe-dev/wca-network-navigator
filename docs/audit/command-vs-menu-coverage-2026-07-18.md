...
Meta-pagina: la copertura _è_ l'oggetto dell'audit. Nessuna operazione UI-only propria.
...
| **Spostare attività su altra data** | `schedule-activity` con `activity_id`? | 🟡 payload attuale accetta solo _create_, non _reschedule_                            |
