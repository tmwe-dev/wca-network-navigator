...
test.describe("campaign-queue-lifecycle", () => {
...
deepTest.describe("Deep invariants: /v2/outreach", () => {
  ...
  deepTest("redirect coerente a /auth se non autenticato (o pagina renderizzata)", async ({ page }) => {
    ...
    const isAuthOr = url.pathname.includes("/auth") || url.pathname.includes("/v2/login") || url.pathname.startsWith("/v2/outreach".split("/").slice(0, 3).join("/"));
