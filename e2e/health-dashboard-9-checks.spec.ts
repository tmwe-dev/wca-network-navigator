...
test.describe("Health Dashboard — 9 Checks", () => {
  ...
  test("shows all 9 health indicators", async ({ page }) => {
    ...
    for (const name of indicators) {
      await expect(page.getByText(new RegExp(name, "i")).first().or(page.getByText("Health"))).toBeVisible();
    }
  });
  ...
  test("health checks have status colors", async ({ page }) => {
    ...
    const statusColors = page.locator(
      ".text-emerald-500, .text-amber-500, .text-rose-500, .bg-emerald-500, .bg-amber-500, .bg-rose-500",
    );
