...
test.describe("Email Composer Flow", () => {
  ...
  test("Oracle panel is visible (sidebar with email type chips)", async ({ page }) => {
    ...
    const oraclePanel = page
      .locator("div")
      .filter({ has: page.locator("button").filter({ hasText: /generat|improve|template/i }) });
  ...
  test("Recipient field is present", async ({ page }) => {
    ...
    const recipientVisible =
      (await recipientInput.isVisible({ timeout: 10000 }).catch(() => false)) ||
      (await page
        .getByText(/a:|to:/i)
  ...
  test("Subject field is present", async ({ page }) => {
    ...
    const subjectVisible =
      (await subjectInput.isVisible({ timeout: 10000 }).catch(() => false)) ||
      (await page
        .getByText(/oggetto|subject/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false));
  ...
  test("Subject field accepts input", async ({ page }) => {
    ...
    if (inputVisible) {
      await subjectInput
        .first()
        .fill("Test Subject", { timeout: 5000 })
        .catch(() => {});
      const value = await subjectInput
        .first()
        .inputValue()
        .catch(() => "");
  ...
  test("PermissionGate for send button", async ({ page }) => {
    ...
    const sendBtnExists = await sendButton
      .count()
      .then((c) => c > 0)
      .catch(() => false);
    const permissionMsgExists = await permissionMessage
      .count()
      .then((c) => c > 0)
      .catch(() => false);
  ...
  test("responsive layout - tablet", async ({ page }) => {
    ...
    const subjectField = page
      .locator('input[placeholder*="oggetto"]')
      .or(page.locator('input[placeholder*="subject"]'));

    const fieldExists =
      (await subjectField.isVisible({ timeout: 5000 }).catch(() => false)) ||
      (await page
        .getByText(/oggetto|subject/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false));
  ...
  test("responsive layout - mobile", async ({ page }) => {
    ...
    const contentVisible =
      (await mainContent.isVisible({ timeout: 10000 }).catch(() => false)) ||
      (await page
        .getByText(/oggetto|subject|destinatar/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false));
  ...
  test("email toolbar renders", async ({ page }) => {
    ...
    const toolbar = page
      .locator("div")
      .filter({ has: page.locator("button").filter({ hasText: /variabil|link|attachment|preview/i }) });
  ...
  test("page structure has correct flex layout", async ({ page }) => {
    const mainContainer = page
      .locator("div")
      .filter({ has: page.locator('[contenteditable="true"]').or(page.locator("textarea")) });
