...
test.describe("Outreach & Holding Pattern", () => {
  ...
  test("navigare a Outreach e vedere il Command Center", async ({ page }) => {
    await page.goto("/v2/outreach");
    await page.waitForLoadState("networkidle");
    const content = page.locator("text=Holding").or(page.locator("text=Attesa")).or(page.locator('[data-testid="holding-pattern"]')).or(page.locator('[role="tablist"]'));
  ...
  test("switchare canali senza crash", async ({ page }) => {
    ...
    if (count >= 2) { await channelTabs.nth(1).click(); await page.waitForTimeout(500); }
