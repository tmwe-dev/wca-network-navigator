...
test.describe("smoke: contact CRUD", () => {
  test("CRM page loads with contact list", async ({ authedPage: page }) => {
    ...
    const hasContent =
      (await page
        .locator(
          "table, [role='grid'], input[placeholder*='cerca'], input[placeholder*='search'], [data-testid='contact-list']",
        )
        .count()) > 0;
  ...
  test("contacts page has no console errors", async ({ authedPage: page }) => {
    ...
    const critical = errors.filter(
      (e) => !e.includes("ResizeObserver") && !e.includes("AbortError") && !e.includes("Failed to fetch"),
    );
