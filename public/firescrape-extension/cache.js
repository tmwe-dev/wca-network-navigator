...
const Cache = {
  ...
  // Enforza quota: se troppe entry, rimuovi le più vecchie con meno hit
  async _enforceQuota() {
    const all = await chrome.storage.local.get(null);
    const entries = [];
    ...
    for (const [key, entry] of Object.entries(all)) {
      if (!key.startsWith(this._prefix)) continue;
      entries.push({ key, cachedAt: entry.cachedAt || 0, hits: entry.hits || 0 });
    }
    ...
    // Ordina: prima quelli con meno hit e più vecchi
    entries.sort((a, b) => {
