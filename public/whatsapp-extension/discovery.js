      ...
      return function () {
        function scanShadowRoots(root, roots, seen) {
          try {
            ...
            console.debug("[WA Discovery]", err?.message);
          }
        }
        ...
        function filterVisible(els) {
          return Array.from(els || []).filter(function (el) {
            try {
              ...
              console.debug("[WA Discovery] rect:", err?.message);
              return true;
            }
          });
        }
        ...
        if (result.chatItems === 0 && !result.hasQR) {
          ...
          for (const sp of allSpansWithTitle) {
            const p =
              sp.parentElement && sp.parentElement.parentElement && sp.parentElement.parentElement.parentElement;
            if (p) {
        ...
        try {
          ...
          result.storageMarkers = storageKeys
            .filter(function (key) {
              const lower = String(key || "").toLowerCase();
              return authMarkerPatterns.some(function (m) {
        ...
        if (result.chatItems > 0 && result.chatItemsMethod) {
          try {
            ...
            if (result.chatItemsMethod === "role-container-children") {
              ...
              firstItem =
                ((qsDeep("span[title]") || {}).closest && qsDeep("span[title]").closest("[tabindex]")) ||
