...
var Actions =
  ...
  (function () {
        ...
        func: function () {
          ...
          function scanShadowRoots(root, roots, seen) {
            try {
              ...
              console.debug("[WA Actions] page helper setup:", err?.message);
            }
          }
          ...
          function setupCacheObserver() {
            ...
            try {
              ...
              console.debug("[WA Actions] cache observer setup:", err?.message);
            }
          }
          ...
          function queryAllFromRoots(roots, sel) {
            ...
            for (const root of roots) {
              try {
                ...
                console.debug("[WA Actions] selector failed:", err?.message);
              }
            }
            return out;
          }
          ...
          function qsaDeep(sel) {
            return queryAllFromRoots(getGlobalRoots(), sel);
          }
          function qsDeep(sel) {
            return qsaDeep(sel)[0] || null;
          }
          function qsaWithin(root, sel) {
            return queryAllFromRoots(getNestedRoots(root), sel);
          }
          function qsWithin(root, sel) {
            return qsaWithin(root, sel)[0] || null;
          }
          ...
          function filterVisible(els) {
            return Array.from(els || []).filter(function (el) {
              try {
                ...
                console.debug("[WA Actions] rect check:", err?.message);
                return true;
              }
            });
          }
    ...
    async function verifySession() {
      try {
        ...
        const shouldHydrate =
          ...
          (!result ||
            (!result.hasQR &&
              !Discovery.hasRenderableWaUi(result) &&
              (Discovery.hasShellSignals(result) || result.appLoaded)));

        if (shouldHydrate) {
          result =
            (await TabManager.withTemporarilyVisibleTab(waTab.id, async function () {
        ...
        if (result && result.hasQR)
          return { success: true, authenticated: false, reason: "qr_required", diagnostic: diag };
        ...
        if (result && Array.isArray(result.dataTestIds) && result.dataTestIds.indexOf("confirm-popup") !== -1) {
          ...
          try {
            ...
            if (after && after.sidebar)
              return { success: true, authenticated: true, method: "sidebar-after-popup", diagnostic: diag2 };
            if (((after && after.chatItems) || 0) > 0)
              return { success: true, authenticated: true, method: "chat-items-after-popup", diagnostic: diag2 };
            if (after && (after.hasComposeBox || (after.textboxCount || 0) > 0))
              return { success: true, authenticated: true, method: "compose-after-popup", diagnostic: diag2 };
            if (Discovery.hasShellSignals(after))
              return { success: true, authenticated: true, method: "shell-after-popup", diagnostic: diag2 };
            return {
              success: true,
              authenticated: false,
              reason: "confirm_popup",
              message:
                "Popup 'Usa qui' rilevato e cliccato, ma WhatsApp non si è ancora stabilizzato. Riprova fra qualche secondo.",
              diagnostic: diag2,
            };
          } catch (e) {
            return {
              ...
              reason: "confirm_popup",
              message:
                "WhatsApp Web mostra un popup di conferma ('Usa qui'). Click automatico fallito: " +
        ...
        if (result && (result.hasLoadingScreen || !result.appLoaded || result.bodyChildCount < 3))
          return { success: true, authenticated: false, reason: "loading", diagnostic: diag };
        if (result && result.sidebar)
          return {
            success: true,
            authenticated: true,
            method: "sidebar:" + (result.sidebarSelector || "discovered"),
            diagnostic: diag,
          };
        if (((result && result.chatItems) || 0) > 0)
          return {
            success: true,
            authenticated: true,
            method: "chat-items:" + (result.chatItemsMethod || "discovered"),
            diagnostic: diag,
          };
        if (result && (result.hasComposeBox || (result.textboxCount || 0) > 0))
          return {
            success: true,
            authenticated: true,
            method: result.hasComposeBox ? "compose-box" : "textbox",
            diagnostic: diag,
          };
        if (Discovery.hasShellSignals(result))
          return { success: true, authenticated: true, method: "app-shell-fallback", diagnostic: diag };
    ...
    function _pageReadUnreadDOM(schema) {
      ...
      if (chatItems.length === 0) {
        const containers = H.filterVisible(
          H.qsaDeep('[role="list"],[role="listbox"],[role="grid"],[role="navigation"]'),
        );
      ...
      for (let i = 0; i < chatItems.length; i++) {
        ...
        badge =
          H.qsWithin(chat, '[data-testid="icon-unread-count"]') || H.qsWithin(chat, '[data-testid="unread-count"]');
        ...
        if (!badge) {
          const allEls = H.qsaWithin(
            chat,
            '[class*="unread"], [class*="badge"], [aria-label*="unread"], [aria-label*="non lett"]',
          );
        ...
        const titleEl =
          H.qsWithin(chat, 'span[title][dir="auto"]') ||
          H.qsWithin(chat, "span[title]") ||
          H.qsWithin(chat, '[data-testid="cell-frame-title"] span') ||
          H.qsWithin(chat, '[role="gridcell"] span[dir="auto"]');
        ...
        let msgEl =
          H.qsWithin(chat, '[data-testid="last-msg-status"]') ||
        ...
        let timeEl =
          H.qsWithin(chat, '[data-testid="cell-frame-primary-detail"]') ||
    ...
    function _pageUnifiedExtract() {
      ...
      for (let i = 0; i < chatItems.length; i++) {
        ...
        const cstr = [
          {
            fn: function () {
              return H.qsWithin(chat, 'span[title][dir="auto"]');
            },
            get: function (el) {
          ...
          {
            fn: function () {
              return H.qsWithin(chat, '[data-testid="cell-frame-title"] span');
            },
            get: function (el) {
          ...
          {
            fn: function () {
              return H.qsWithin(chat, "span[title]");
            },
            get: function (el) {
              return el.getAttribute("title");
            },
          ...
          {
            fn: function () {
              return H.qsWithin(chat, '[role="gridcell"] span[dir="auto"]');
            },
            get: function (el) {
              return el.textContent && el.textContent.trim();
            },
        ...
        const bstr = [
          {
            fn: function () {
              return (
                H.qsWithin(chat, '[data-testid="icon-unread-count"]') ||
                H.qsWithin(chat, '[data-testid="unread-count"]')
              );
            },
            get: function (el) {
          ...
          },
          {
            fn: function () {
              ...
              for (let k = 0; k < spans.length; k++) {
                ...
                const pbg = spans[k].parentElement
                  ? window.getComputedStyle(spans[k].parentElement).backgroundColor
                  : "";
        ...
        for (let bi = 0; bi < bstr.length; bi++) {
          const el = bstr[bi].fn();
          if (el) {
            item.unreadCount = bstr[bi].get(el);
            item.hasUnread = true;
            badgeScore = bstr[bi].score;
            break;
          }
        }
        ...
        if (!msgEl) {
          ...
          for (let k = 0; k < sps.length; k++) {
            ...
            if (t && t.length > 3 && t !== item.contactName && !/^\d+$/.test(t) && !/^\d{1,2}[:.]\d{2}/.test(t))
              cands.push(sps[k]);
          }
        ...
        if (!timeEl) {
          ...
          for (let k = 0; k < sps.length; k++) {
            ...
            if (st && /^(ieri|oggi|luned|marted|mercoled|gioved|venerd|sabato|domenic)/i.test(st)) {
              timeEl = sps[k];
              break;
            }
          }
      ...
      if (itemsWithContact === 0 && chatItems.length > 0) {
        ...
        diagnosticInfo = {
          ...
          dataTestIds: Array.from(
            new Set(
              Array.from(document.querySelectorAll("[data-testid]")).map(function (e) {
          ...
          roles: Array.from(
            new Set(
              Array.from(document.querySelectorAll("[role]")).map(function (e) {
    ...
    async function aiRelearnAndExtract(tabId, diagnosticInfo) {
      ...
      try {
        ...
        console.debug("[WA V2] cache save failed:", e?.message);
      }
    ...
    function _pageValidateAndExtract(selectors) {
      ...
      const containerSel =
        (validatedSelectors.chatItem && validatedSelectors.chatItem.selector) ||
      ...
      for (let i = 0; i < chatItems.length; i++) {
        ...
        if (badgeSel) {
          ...
          if (el) {
            item.unreadCount = parseInt(el.textContent) || 1;
            item.hasUnread = true;
          }
        }
        if (!item.hasUnread) {
          ...
          for (let k = 0; k < ar.length; k++) {
            ...
            if (lbl.includes("unread") || lbl.includes("non lett") || lbl.includes("da leggere")) {
              item.unreadCount = parseInt(ar[k].textContent) || 1;
              item.hasUnread = true;
              break;
            }
          }
        ...
        const msgEl =
          (msgSel && H.qsWithin(chat, msgSel)) || H.qsWithin(chat, '[data-testid="cell-frame-secondary"] span');
        if (msgEl) item.lastMessage = (msgEl.textContent || "").trim();

        const timeSel = validatedSelectors.timestamp && validatedSelectors.timestamp.selector;
        const timeEl =
          (timeSel && H.qsWithin(chat, timeSel)) ||
          H.qsWithin(chat, '[data-testid="cell-frame-primary-detail"]') ||
          H.qsWithin(chat, "time");
        if (timeEl) item.timestamp = (timeEl.textContent || "").trim();
    ...
    function convertSchemaToOptimusPlan(schema) {
      ...
      for (const [aiKey, selector] of Object.entries(schema)) {
        ...
        const primary =
          typeof selector === "string" ? selector : selector.primary || selector.selector || String(selector);
    ...
    async function loadCachedLearnedPlan() {
      try {
        ...
        const schema =
          typeof stored[LEARNED_PLAN_CACHE_KEY] === "string"
    ...
    async function tryOptimusReadUnread(tabId, previousFailed, failureContext) {
          ...
          try {
            ...
            if (cacheExec && cacheExec.success && cacheExec.items && cacheExec.items.length > 0) {
              return {
                ...
                confidence: 0.85,
                latencyMs: 0,
                items: cacheExec.items,
                candidates: cacheExec.candidates || 0,
                dropped: cacheExec.dropped || 0,
              };
            }
      ...
      if (!snap || !snap.ok)
        return { success: false, error: (snap && snap.error) || "snapshot_failed", optimusUnavailable: false };
      ...
      if (!execRes || !execRes.success)
        return { success: false, error: (execRes && execRes.error) || "execute_failed", optimusUnavailable: false };
    ...
    async function readThread(contactName, maxMessages, contactJid) {
      ...
      try {
        ...
        if (optimus.success && optimus.items.length === 0 && optimus.cached) {
          optimus = await tryOptimusReadThread(
            r.tab.id,
            true,
            "Cached plan returned 0 messages from thread panel for " + contactName,
          );
        }
    ...
    function _pageSendUrlFallback(messageText) {
      ...
      function findComposer() {
        return (
          H.qsDeep('footer [contenteditable="true"][data-tab]') ||
      ...
      function dismissPopups() {
        ...
        const okBtn =
          H.qsDeep('[data-testid="popup-controls-ok"]') ||
          ...
          const tick = function () {
            ...
            if (attempts >= maxAttempts) {
              ...
              if (btn) {
                ...
                resolve({ success: true, sent: true, method: "url-fallback-enter" });
    ...
    function _pageSendWhatsApp(target, messageText, plan) {
      ...
      function tryPlan(field) {
        ...
        try {
          if (f.primary) {
            const el = H.qsDeep(f.primary);
            if (el) return el;
          }
          if (f.fallback) {
            const el = H.qsDeep(f.fallback);
            if (el) return el;
          }
        } catch (e) {
          /* invalid selector */
      ...
      const searchBox =
        ...
        H.qsDeep('[title*="earch" i]') ||
        H.qsDeep('[title*="erca" i]');
      ...
      return new Promise(function (resolve) {
        setTimeout(function () {
          ...
          if (planChatItem && planChatItem.primary) {
            try {
              chats = H.qsaDeep(planChatItem.primary) || [];
            } catch (e) {
              chats = [];
            }
          }
          if (chats.length === 0 && planChatItem && planChatItem.fallback) {
            try {
              chats = H.qsaDeep(planChatItem.fallback) || [];
            } catch (e) {
              chats = [];
            }
          }
          ...
          const targetLower = String(target || "")
            .trim()
            .toLowerCase();
          const candidates = [];
          ...
          (
            ...
            c.closest('[data-testid="chat-cell-wrapper"]') ||
            c
          ).click();
          if (clearBtn) clearBtn.click();
          ...
          setTimeout(function () {
            ...
            function readHeader() {
              const h =
                H.qsDeep("#main header span[title]") ||
            ...
            awaitHeader(function (ok) {
              if (!ok) {
                resolve({
                  success: false,
                  error:
                    'Header chat non corrisponde al destinatario "' +
                    target +
                    '" (header="' +
                    readHeader() +
                    '"). Invio bloccato.',
            ...
            function proceedCompose() {
              const composer =
                ...
                H.qsDeep('[data-testid="compose-box-input"]');
              if (!composer) {
                resolve({ success: false, error: "Composer not found", needsRemap: true });
                return;
              }
              H.modernClearAndType(composer, messageText);
      ...
      try {
        ...
        if (existingTabs.length > 0) {
          ...
          if (isPhoneNumber) {
            ...
            var sendUrlFirst =
              Config.WA_BASE + "/send?phone=" + numericPhoneFirst + "&text=" + encodeURIComponent(text);
            ...
            return urlResultsFirst && urlResultsFirst[0]
              ? urlResultsFirst[0].result
              : { success: false, error: "URL send failed" };
          }
        ...
        if (isPhoneNumber) {
          ...
          if (!loaded) {
            ...
            return { success: false, error: "WA non caricato" };
          }
    ...
    function _pageScrollAndRead(contact, lastText) {
      ...
      for (const el of msgEls) {
        const isOut = !!(
          el.querySelector('[data-testid="msg-dblcheck"]') || el.querySelector('[data-testid="msg-check"]')
        );
        const textEl =
          el.querySelector('[data-testid="balloon-text"] span') ||
            ...
            if (items.length === 0 && cached) {
              ...
              if (freshSnap && freshSnap.ok) {
                const freshRes = await Optimus.getPlan({
                  ...
                  failureContext:
                    "Cached plan returned 0 messages during backfill scroll " + scrollIdx + " for " + contactName,
                });
    ...
    async function diagnostic() {
      try {
        ...
        if (shouldHydrate) {
          result =
            (await TabManager.withTemporarilyVisibleTab(r.tab.id, async function () {
    ...
    async function remapSendDom() {
      try {
        ...
        if (!snap || !snap.ok)
          return { success: false, error: "snapshot failed: " + ((snap && snap.error) || "unknown") };
        ...
        if (!planRes || planRes.success === false) {
          return {
            success: false,
            error: "optimus failed: " + ((planRes && (planRes.error || planRes.code)) || "unknown"),
          };
        }
        ...
        ["search_box", "chat_item", "chat_header", "composer", "send_button"].forEach(function (k) {
          if (sels[k])
            fields[k] = {
              primary: sels[k].primary || null,
              fallback: sels[k].fallback || null,
              confidence: sels[k].confidence || null,
            };
        });
