...
var Actions = (function () {
  ...
  function _pageReadUnreadDOM(schema) {
    ...
    if (chatItems.length === 0) {
      var containers = H.filterVisible(H.qsaDeep('[role="list"],[role="listbox"],[role="grid"],[role="navigation"]'));
      for (var cont of containers) {
        var vc = H.filterVisible(Array.from(cont.children || []));
    ...
    // S5: Heuristic
    if (chatItems.length === 0) {
      var allTitled = H.filterVisible(H.qsaDeep('span[title]'));
    ...
    for (var i = 0; i < chatItems.length; i++) {
      ...
      if (!badge) {
        var ariaEls = H.qsaWithin(chat, 'span[aria-label]');
      ...
      if (!badge) {
        var spans = H.qsaWithin(chat, 'span');
        for (var s of spans) {
      ...
      var titleEl = H.qsWithin(chat, 'span[title][dir="auto"]') || H.qsWithin(chat, 'span[title]') || H.qsWithin(chat, '[data-testid="cell-frame-title"] span');
      if (titleEl) contactName = titleEl.getAttribute("title") || titleEl.textContent?.trim() || "Sconosciuto";

      var lastMessage = "";
      var msgEl = H.qsWithin(chat, '[data-testid="last-msg-status"]') || H.qsWithin(chat, '[data-testid="cell-frame-secondary"] span[title]') || H.qsWithin(chat, '[data-testid="cell-frame-secondary"] span') || H.qsWithin(chat, '[data-testid="last-msg"] span');
      if (!msgEl) {
        var allSpans = H.qsaWithin(chat, 'span');
        var candidates = [];
      ...
      var timeEl = H.qsWithin(chat, '[data-testid="cell-frame-primary-detail"]') || H.qsWithin(chat, '[data-testid="msg-time"]') || H.qsWithin(chat, 'time');
      if (!timeEl) {
        var smallSpans = H.qsaWithin(chat, 'span');
  ...
  async function _pageSendWhatsApp(contact, msg) {
    ...
    async function openChat(name) {
      var searchBox = H.qsDeep('[data-testid="chat-list-search"]') ||
      ...
      if (searchBox) {
        ...
        var chatResults = H.qsaDeep('[data-testid="cell-frame-container"],[data-testid="chat-cell-wrapper"],[role="listitem"],[role="row"],[role="option"]');
        ...
        for (var i = 0; i < chatResults.length; i++) {
          ...
          if (title.toLowerCase().includes(name.toLowerCase())) {
            ...
            var clearBtn = H.qsDeep('[data-testid="x-alt"]') || H.qsDeep('[data-testid="search-close"]') || H.qsDeep('[data-testid="search-input-clear"]');
            if (clearBtn) clearBtn.click();
            return true;
          }
        }
        var esc = H.qsDeep('[data-testid="x-alt"]') || H.qsDeep('[data-testid="search-close"]') || H.qsDeep('[data-testid="search-input-clear"]');
        if (esc) esc.click();
      }
      return false;
    }
    ...
    var opened = false;
    var currentHeader = H.qsDeep('#main header span[title]') ||
    ...
    if (opened) {
      var inputBox = H.qsDeep('[data-testid="conversation-compose-box-input"]') || H.qsDeep('#main [contenteditable="true"]') || H.qsDeep('[role="textbox"][contenteditable="true"]');
      if (inputBox) {
    ...
    while (Date.now() - start < 10000) {
      var btn = H.qsDeep('span[data-icon="send"]') || H.qsDeep('[data-testid="send"]') || H.qsDeep('[data-testid="compose-btn-send"]') || H.qsDeep('button[aria-label*="end" i]') || H.qsDeep('button[aria-label*="nvia" i]');
      if (btn) {
  ...
  async function _pageOpenAndReadThread(target) {
    ...
    function currentChatMatches(name) {
      var header = H.qsDeep('#main header span[title]') || H.qsDeep('#main header [dir="auto"]') || H.qsDeep('[data-testid="conversation-header"] span[title]');
    ...
    if (!currentChatMatches(target)) {
      var searchBox = H.qsDeep('[data-testid="chat-list-search"] [contenteditable="true"]') || H.qsDeep('[data-testid="chat-list-search"]') || H.qsDeep('[title*="earch" i]') || H.qsDeep('[title*="erca" i]') || H.qsDeep('[data-testid="search"]') || H.qsDeep('[aria-label*="search" i]') || H.qsDeep('[aria-label*="cerca" i]') || H.qsDeep('[role="textbox"][contenteditable="true"]');
      if (!searchBox) return { success: false, error: "Search box not found" };
      H.modernClearAndType(searchBox, target);
      await new Promise(function(r) { setTimeout(r, 1500); });

      var chats = H.qsaDeep('[data-testid="cell-frame-container"],[data-testid="chat-cell-wrapper"],[role="listitem"],[role="row"]');
      var clicked = false;
      for (var c of chats) {
      ...
      var clearBtn = H.qsDeep('[data-testid="search-input-clear"]') || H.qsDeep('[data-testid="x-alt"]') || H.qsDeep('[data-testid="search-close"]');
      if (clearBtn) clearBtn.click();
      await new Promise(function(r) { setTimeout(r, 2000); });
    }

    var panel = H.qsDeep('[data-testid="conversation-panel-messages"]') || H.qsDeep('#main [role="application"]') || H.qsDeep('#main');
    return { success: true, html: panel ? panel.outerHTML : null };
  }

  function _pageDomReadMessages(target, limit) {
    var H = window.__waH;
    var msgEls = H.qsaDeep('[data-testid="msg-container"]');
  ...
  async function _pageOpenChatForBackfill(target) {
    ...
    var searchBox = H.qsDeep('[data-testid="chat-list-search"] [contenteditable="true"]') || H.qsDeep('[data-testid="chat-list-search"]') || H.qsDeep('[title*="earch" i]') || H.qsDeep('[title*="erca" i]') || H.qsDeep('[aria-label*="search" i]') || H.qsDeep('[aria-label*="cerca" i]');
    if (!searchBox) return { success: false, error: "Search box not found" };
