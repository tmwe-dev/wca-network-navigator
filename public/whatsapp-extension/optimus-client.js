  ...
  (function () {
    ...
    function _findWebappTab() {
      try {
        ...
        console.debug("[WA Optimus] plan request:", err?.message);
        ...
        _findWebappTab()
          .then(function (tab) {
            ...
            try {
              ...
              resolve({
                success: false,
                error: "send_failed: " + (err && err.message ? err.message : String(err)),
                code: "SEND_FAILED",
              });
            }
          })
    ...
    function _pageExecutePlan(rootSelector, fallbackBody, plan) {
      function trySel(root, sel) {
        ...
        try {
          ...
          console.debug("[WA Optimus] sel:", err?.message);
          return null;
        }
      }
      function tryAll(root, sel) {
        ...
        try {
          ...
          console.debug("[WA Optimus] selAll:", err?.message);
          return [];
        }
      }
      ...
      return chrome.scripting
        ...
        .catch(function (err) {
          return {
            success: false,
            items: [],
            error: "exec_failed: " + (err && err.message ? err.message : String(err)),
          };
        });
    }
