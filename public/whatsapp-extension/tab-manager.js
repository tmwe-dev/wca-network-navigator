  ...
  (function () {
    ...
    async function safeRemoveTab(tabId) {
      try {
        ...
        console.debug("[WA Tab]", err?.message);
      }
    }
    ...
    async function activateAndStabilize(tabId, maxWaitMs) {
      ...
      try {
        ...
        if (tab.windowId === _automationWindowId) {
          ...
          console.warn(
            "[WA TabMgr] Tab " + tabId + " not in automation window — skipping activate to preserve user focus",
          );
        }
      } catch (err) {
        console.debug("[WA Tab] V2.1 activate:", err?.message);
      }
      ...
      while (Date.now() - startTime < maxWait) {
        try {
          const check = await chrome.scripting.executeScript({
            ...
            func: function () {
              return {
                ...
                loading: !!(
                  document.querySelector('[data-testid="startup"]') || document.querySelector(".landing-window")
                ),
              };
            },
          });
          ...
          if (r && r.ready && r.hasContent && !r.loading) {
            stable = true;
            break;
          }
        } catch (err) {
          console.debug("[WA Tab] V2.1 probe:", err?.message);
        }
        await sleep(300);
      }
