  ...
  (function () {
    ...
    function noExistingLinkedInTab(errorCode, actionLabel) {
      return Config.errorResponse(
        errorCode,
        "no_existing_linkedin_tab: apri LinkedIn una volta in Chrome; " +
          actionLabel +
          " non apre nuove tab e non cambia pagina",
      );
    }
        ...
        if (!tab || !tab.id) {
          return {
            ...
            result: Config.errorResponse(
              ...
              "no_existing_linkedin_tab: apri LinkedIn una volta in Chrome; l'invio non apre nuove tab e non cambia pagina",
            ),
          };
        }
        ...
        try {
          ...
          if (!isThreadUrl && !onTarget) {
            ...
            return {
              ...
              result: Config.errorResponse(
                Config.ERROR.MESSAGE_FAILED,
                "wrong_recipient: tab non sul profilo richiesto (" + currentUrl + ")",
              ),
            };
          }
        } catch (e) {
          /* se tabs.get fallisce, lasciamo procedere */
        }
        if (!isThreadUrl && !composerAlreadyOpen) {
          ...
          if (!clickResult || !clickResult.success) {
            return {
              ...
              result: Config.errorResponse(
                Config.ERROR.MESSAGE_FAILED,
                (clickResult && clickResult.error) || "Message button not found",
              ),
            };
          }
    ...
    async function findLinkedInTabWithOpenComposer(targetClean) {
      let tabs = [];
      try {
        tabs = await chrome.tabs.query({ url: "*://*.linkedin.com/*" });
      } catch (e) {
        tabs = [];
      }
      let best = null;
      for (let i = 0; i < tabs.length; i++) {
        ...
        try {
          const probe = await chrome.scripting.executeScript({
            ...
            func: function (targetUrl) {
              ...
              for (var s = 0; s < scopes.length; s++) {
                ...
                visibleScopes++;
                var boxes = scopes[s].querySelectorAll(
          ...
          /* tab non iniettabile/discarded: ignora */
        }
      }
      return best;
    }
    ...
    async function sendLinkedInMessageWithMethod(profileUrl, message, method) {
      ...
      try {
        ...
        if (stored && stored.url === targetClean && stored.msg === message && now - stored.at < 2000) {
          return Config.errorResponse(
            Config.ERROR.MESSAGE_FAILED,
            "duplicate_send_blocked: invio identico entro 2s, atteso debounce",
          );
        }
      ...
      if (!tab || !tab.id) {
        return Config.errorResponse(
          Config.ERROR.MESSAGE_FAILED,
          "no_existing_linkedin_tab: apri LinkedIn una volta in Chrome; il test non apre nuove tab",
        );
      }
          ...
          const probe = await chrome.scripting.executeScript({
            ...
            func: function () {
              ...
              function deepQueryAll(selector, root) {
                ...
                var r = root || document;
                try {
                  out.push.apply(out, r.querySelectorAll(selector));
                } catch (e) {}
                var all = r.querySelectorAll ? r.querySelectorAll("*") : [];
                for (var i = 0; i < all.length; i++) {
                  if (all[i].shadowRoot) {
                    try {
                      out.push.apply(out, deepQueryAll(selector, all[i].shadowRoot));
                    } catch (e) {}
                  }
                }
                return out;
            ...
            func: function (timeoutMs) {
              ...
              function findMessageBtn() {
                ...
                for (var i = 0; i < btns.length; i++) {
                  ...
                  if (
                    /messaggia|messaggio|message|send message|nachricht|mensaje|mensagem|wiadomo|envoyer/i.test(label)
                  )
                    return b;
                }
                return null;
              ...
              return (async function () {
                ...
                while (Date.now() - started < limit) {
                  ...
                  var shells = deepQueryAll(
                    ".msg-form, [class*='msg-form'], [role='dialog'], .msg-overlay-conversation-bubble, [class*='msg-overlay-conversation']",
                  );
                  last.shells = shells.filter(visible).length;
                  last.boxes = deepQueryAll("[contenteditable='true'], div[role='textbox'], [role='textbox']").filter(
                    visible,
                  ).length;
          ...
          const r = await chrome.scripting.executeScript({
            ...
            func: function () {
              ...
              for (var j = 0; j < btns.length; j++) {
                ...
                if (
                  /messaggi|message|messa|invia messaggio|nachricht|enviar mensaje|envoyer un message|mensagem|wiadomo/i.test(
                    label,
                  )
                ) {
      ...
      if (!composerAlreadyOpen && !isThreadUrl) {
        ...
        if (!profileReady && !composerAlreadyOpen) {
          ...
          if (!optimistic || !optimistic.success) {
            ...
            return Config.errorResponse(
              Config.ERROR.MESSAGE_FAILED,
              "profile_not_ready: profilo LinkedIn non pronto in background (status=" +
                lastTabStatus +
                ", url=" +
                shortUrl +
                "), riprova",
      ...
      if (!composerAlreadyOpen && !isThreadUrl) {
        ...
        if (!clickResult || !clickResult.success) {
          return Config.errorResponse(
            Config.ERROR.MESSAGE_FAILED,
            (clickResult && clickResult.error) || "open_composer_failed: bottone Messaggia non trovato sul profilo",
          );
        }
        ...
        if (finalGate && finalGate.success) {
          ...
          if (fallbackResult && fallbackResult.success) {
            return Object.assign({}, fallbackResult, {
              method: (fallbackResult.method || "fallback") + "_after_composer_gate",
    ...
    async function sendConnectionRequest(profileUrl, note) {
      ...
      if (!clickResult || !clickResult.success)
        return Config.errorResponse(
          Config.ERROR.CONNECT_FAILED,
          (clickResult && clickResult.error) || "Connect button not found",
        );
      ...
      try {
        ...
        return Config.errorResponse(Config.ERROR.CONNECT_FAILED, e.message);
      }
    }
    ...
    async function searchProfile(query) {
      ...
      try {
            ...
            for (let i = 0; i < allLinks.length; i++) {
              ...
              if (/linkedin\.com\/in\/[^/]+/.test(href) && !/\/in\/miniprofile/.test(href) && !/\/in\/ACo/.test(href)) {
                ...
                if (container) {
                  ...
                  if (!name) {
                    const dirEl = container.querySelector("h3 span[dir='ltr'], a span[dir='ltr'], span[dir='ltr']");
                    if (dirEl) name = dirEl.textContent.trim();
                  }
                  if (!name && allLinks[i].textContent) {
                    const lt = allLinks[i].textContent.replace(/\s+/g, " ").trim();
                    if (lt.length > 1 && lt.length < 80) name = lt;
                  }
                  if (!name) {
                  ...
                  if (!headline) {
                    const ps = container.querySelectorAll("p, div[class*='t-']");
                    for (let pp = 0; pp < ps.length; pp++) {
                      const pt = ps[pp].textContent.replace(/\s+/g, " ").trim();
                      if (pt && pt !== name && pt.length > 3 && pt.length < 200) {
                        headline = pt;
                        break;
                      }
                    }
                ...
                if (!name) {
                  const al = allLinks[i].getAttribute("aria-label") || "";
                  if (al.length > 1) name = al.split(",")[0].trim();
                }
        ...
        return Config.errorResponse(Config.ERROR.SEARCH_FAILED, err.message);
      }
    }
    ...
    function buildPlanFromKeyMap(schema, keyMap) {
      ...
      for (const schemaKey in schema) {
        ...
        plan.fields[optimusKey] = {
          primary: typeof selector === "string" ? selector : selector.primary || selector.selector || String(selector),
          fallback: typeof selector === "object" ? selector.fallback || selector.alt || null : null,
        };
      }
    ...
    async function tryOptimusInbox(tabId, previousFailed, failureContext) {
      const inboxSelector =
        '[class*="msg-overlay-list-bubble"], [class*="msg-conversations-container"], main, [role="main"]';
      ...
      const results = await chrome.scripting.executeScript({
        ...
        func: async function (maxScrollPasses) {
          ...
          function cleanUrl(v) {
            if (!v) return "";
            try {
              return new URL(String(v), location.origin).href.split("?")[0].replace(/\/$/, "");
            } catch (e) {
          ...
          function ids(threadUrl, profileUrl) {
            ...
            return {
              profileId: p ? decodeURIComponent(p[1]) : "",
              threadId: t ? decodeURIComponent(t[1]) : "",
              linkedinId: p ? decodeURIComponent(p[1]) : t ? decodeURIComponent(t[1]) : "",
            };
          }
          var out = [];
          var seen = {};
          function collect() {
            var cards = document.querySelectorAll(
              '[class*="msg-conversation-card"], [class*="msg-convo-wrapper"], [data-control-name*="conversation"], li',
            );
            cards.forEach(function (card) {
              ...
              var h = card.querySelector(
                'h3, h4, [class*="msg-conversation-card__participant-names"], [class*="participant"]',
              );
              if (h) name = cleanText(h.textContent);
              if (!name) {
                var img = card.querySelector("img[alt]");
                if (img)
                  name = cleanText(img.getAttribute("alt"))
                    .replace(/'s profile photo|foto del profilo|profile photo/gi, "")
                    .trim();
              }
              if (!name) {
                ...
                for (var i = 0; i < Math.min(spans.length, 8); i++) {
                  ...
                  if (s.length > 1 && s.length < 80 && !/^\d{1,2}[:/.]/.test(s)) {
                    name = s;
                    break;
                  }
                }
              ...
              if ((threadUrl || profileUrl) && (name || text))
                out.push({
                  name: name,
                  text: text,
                  ...
                  threadUrl: threadUrl,
                  profileUrl: profileUrl,
                  linkedinId: idObj.linkedinId,
                  profileId: idObj.profileId,
                  threadId: idObj.threadId,
                });
            });
          }
          collect();
          var scroller =
            ...
            document.querySelector('[class*="msg-conversations-container__conversations-list"]') ||
    ...
    function findUrlForName(urlMap, name) {
      ...
      for (var i = 0; i < urlMap.length; i++) {
        var candidateName = String(urlMap[i].name || "")
          .toLowerCase()
          .replace(/\s+/g, " ")
          .trim();
        var candidateText = String(urlMap[i].text || "")
          .toLowerCase()
          .replace(/\s+/g, " ")
          .trim();
        if (
          (candidateName &&
            (candidateName === n || candidateName.indexOf(n) !== -1 || n.indexOf(candidateName) !== -1)) ||
          candidateText.indexOf(n) !== -1
        )
          return urlMap[i];
      }
      return null;
    ...
    async function readInbox() {
      ...
      if (optimus.success) {
        ...
        threads.forEach(function (t) {
          ...
          if (typeof t.confidence !== "number")
            t.confidence = typeof optimus.confidence === "number" ? optimus.confidence : 0.85;
        });
        ...
        try {
          ...
          console.warn("[LI Inbox] URL harvest failed:", e?.message);
        }
        return {
          success: true,
          threads: threads,
      ...
      try {
          ...
          func: function () {
            ...
            function addThread(name, threadUrl, unread, lastMsg, profileUrl) {
              ...
              var dedupKey =
                ...
                ids.linkedinId ||
                ids.profileId ||
                name.toLowerCase().trim() + "|" + (lastMsg || "").toLowerCase().trim();
            ...
            const modernCards = document.querySelectorAll(
              '[class*="msg-conversation-card"], [class*="msg-convo-wrapper"], [data-control-name*="conversation"]',
            );
            modernCards.forEach(function (card) {
              ...
              const profileUrl =
                profileLink && profileLink.href ? profileLink.href : /\/in\//.test(threadUrl) ? threadUrl : "";
              let name = "";
              const h3 = card.querySelector("h3");
              if (h3) name = h3.textContent.replace(/\s+/g, " ").trim();
              if (!name) {
                const spans = card.querySelectorAll("span");
                for (let s = 0; s < Math.min(spans.length, 10); s++) {
                  const t = (spans[s].textContent || "").trim();
                  if (t.length > 1 && t.length < 60 && !/^\d{1,2}[\/:\.]/.test(t)) {
                    name = t;
                    break;
                  }
                }
            ...
            if (threads.length === 0) {
              ...
              threadLinks.forEach(function (link) {
                ...
                if (h3) {
                  const h3t = h3.textContent.replace(/\s+/g, " ").trim();
                  if (h3t.length > 1 && h3t.length < 80) name = h3t;
                }
                if (!name) {
                  const img = container.querySelector("img[alt]");
                  if (img) {
                    const alt = (img.getAttribute("alt") || "").trim();
                    if (alt.length > 1 && alt.length < 60 && !/photo|foto|avatar/i.test(alt)) name = alt;
                  }
                }
                if (!name) return;
        ...
        const out =
          (results[0] && results[0].result) || Config.errorResponse(Config.ERROR.INBOX_FAILED, "No inbox data");
    ...
    async function readThread(threadUrl) {
      ...
      if (isProfileUrl) {
        try {
          ...
          console.debug("[LI Actions] profile readThread open composer:", err?.message);
        }
      }
      await TabManager.sleep(2500);
      ...
      try {
        ...
        console.debug("[LI Actions]", err?.message);
      }
      ...
      try {
        const results = await chrome.scripting.executeScript({
          ...
          func: function () {
            ...
            items.forEach(function (item) {
              ...
              if (/outbound|from-me|msg-s-event--outbound|msg-s-message-list__event--own/i.test(cls))
                direction = "outbound";
              else if (/inbound|from-them|msg-s-event--inbound/i.test(cls)) direction = "inbound";
              else if (senderLower === "you" || senderLower === "tu" || senderLower === "me" || senderLower === "io")
                direction = "outbound";
              messages.push({
                text: text,
                sender: sender,
                ...
                direction: direction,
                method: "structural",
                confidence: direction === "unknown" ? 0.4 : 0.7,
              });
            });
        ...
        return Config.errorResponse(Config.ERROR.INBOX_FAILED, e.message);
      }
    }
      ...
      try {
        ...
        if (isProfileUrl) {
          try {
            ...
            console.debug("[LI Actions] profile backfill open composer:", err?.message);
          }
        }
        await TabManager.sleep(2500);
        ...
        function pushUnique(msg) {
          ...
          seen[key] = true;
          if (lastKnownText && msg.text && msg.text.trim().toLowerCase() === lastKnownText.trim().toLowerCase())
            return "stop";
            ...
            var domResults = await chrome.scripting.executeScript({
              ...
              func: function () {
                ...
                var items = document.querySelectorAll(
                  "li[class*='msg-'], li[class*='message'], [class*='msg-s-event']",
                );
                if (items.length === 0) items = document.querySelectorAll("main li, [role='main'] li");
                items.forEach(function (item) {
                  ...
                  var dirBack = "unknown";
                  if (/outbound|from-me|msg-s-event--outbound|msg-s-message-list__event--own/i.test(clsBack))
                    dirBack = "outbound";
                  ...
                  if (text)
                    messages.push({
                      ...
                      timestamp: timestamp,
                      direction: dirBack,
                      method: "legacy-structural-backfill",
                      confidence: dirBack === "unknown" ? 0.4 : 0.65,
                    });
                });
    ...
    async function diagnostic() {
      ...
      try {
        axAvailable = await AXTree.isAvailable(tab.id);
      } catch (err) {
        console.debug("[LI Actions] AX check:", err?.message);
      }
      ...
      domResult.aiLearnAge =
        schema && schema.learnedAt ? Math.round((Date.now() - schema.learnedAt) / 60000) + " min ago" : "never";
      return domResult;
    }
    ...
    async function remapSendDom() {
      ...
      try {
        ...
        try {
          ...
          console.debug("[LI remap] open first thread:", e?.message);
        }
        ...
        return {
          success: ok,
          savedAt: Date.now(),
          messagingFields: messagingSchema
            ? Object.keys(messagingSchema).filter(function (k) {
                return k !== "learnedAt" && k !== "pageType";
              })
            : [],
          profileFields: profileSchema
            ? Object.keys(profileSchema).filter(function (k) {
                return k !== "learnedAt" && k !== "pageType";
              })
            : [],
