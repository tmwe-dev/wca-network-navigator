  ...
  (function () {
    ...
    async function getCached(pageType) {
      try {
        ...
        console.debug("[LI Learn]", err?.message);
      }
      return null;
    }
    ...
    async function clearCache(pageType) {
      if (pageType) {
        ...
        const keys = ["profile", "messaging", "search", "inbox"].map(function (t) {
          return cacheKey(t);
        });
      ...
      try {
        ...
        for (let t = 0; t < Math.min(textboxes.length, 10); t++) {
          snapshot.textboxes.push({
            ...
            placeholder:
              textboxes[t].getAttribute("placeholder") || textboxes[t].getAttribute("aria-placeholder") || "",
          });
        }
    ...
    function extractWithSchema(schema) {
      try {
        ...
        if (schema.messageButtonSelector) {
          ...
          if (mb && !document.querySelector(schema.connectButtonSelector || "____"))
