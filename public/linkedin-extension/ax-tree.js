  ...
  (function () {
    ...
    async function withDebugger(tabId, fn) {
      ...
      try {
        ...
        try {
          ...
          console.debug("[LI AXTree] detach:", err?.message);
        }
      }
    }
    ...
    async function clickNode(tabId, backendNodeId) {
      try {
        ...
        try {
          ...
          console.debug("[LI AXTree] node:", err?.message);
        }
      }
      return false;
    }
    ...
    async function clickNodePhysical(tabId, backendNodeId) {
      try {
        ...
        console.debug("[LI AXTree] physical click:", err?.message);
        return false;
      }
    }
    ...
    async function pressCtrlEnter(tabId, isMac) {
      return await withDebugger(tabId, async function (tid) {
        ...
        await cdp(tid, "Input.dispatchKeyEvent", {
          type: "rawKeyDown",
          key: "Enter",
          code: "Enter",
          windowsVirtualKeyCode: 13,
    ...
    async function extractProfile(tabId) {
      return await withDebugger(tabId, async function (tid) {
        ...
        function isNavNoise(s) {
          ...
          if (/^(notif|notification|messag|search|cerca|home|rete|network|lavoro|jobs|me|tu|profilo|premium)$/i.test(t))
            return true;
      ...
      return await withDebugger(tabId, async function (tid) {
        ...
        if (!btn) {
          ...
          if (moreBtn && moreBtn.backendDOMNodeId) {
            ...
            nodes = await getFullTree(tid);
            btn =
              findOne(nodes, "menuitem", /connect|collegati|connetti/i) ||
    ...
    async function addNote(tabId, noteText) {
      return await withDebugger(tabId, async function (tid) {
        ...
        if (!addNoteBtn || !addNoteBtn.backendDOMNodeId)
          return { success: false, error: "AX: Add Note button not found" };
            ...
            try {
              ...
              if (resolved && resolved.object) {
                ...
                if (parsed && parsed.href && /\/messaging\/thread\//.test(parsed.href)) {
                  ...
                  if (/^(passa ai|go to|details|dettagli|conversation|conversazione)/i.test(contactName))
                    contactName = "";
                  if (contactName) {
              ...
              console.debug("[LI AXTree] prop:", err?.message);
            }
          }
        }
        ...
        if (threads.length === 0) {
          ...
          for (let li = 0; li < listItems.length; li++) {
            ...
            for (let c = 0; c < item.childIds.length; c++) {
              ...
              const child = nodes.find(function (n) {
                return n.nodeId === childId;
              });
              if (!child) continue;
              if (child.role && child.role.value === "link" && child.backendDOMNodeId) {
                try {
                  ...
                  console.debug("[LI AXTree] value:", err?.message);
                }
