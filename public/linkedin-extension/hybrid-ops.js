...
var HybridOps =
  ...
  (function () {
    ...
    function nativeInsertText(text) {
      ...
      const inserted = el.dispatchEvent(
    ...
    async function extractProfile(tabId) {
            ...
            const photoRes = await chrome.scripting.executeScript({
              ...
              func: function () {
                const img = document.querySelector(
                  "img[alt*='photo'], img[alt*='foto'], img[class*='profile-photo'], img[class*='pv-top-card']",
                );
      ...
      try {
        ...
        console.warn("[LI-Hybrid] AI Learn failed:", e.message);
      }
      ...
      try {
        const fallbackRes = await chrome.scripting.executeScript({
          ...
          func: function () {
            ...
            function isNavNoise(s) {
              ...
              if (
                /^(notifiche|notifications|messaging|messaggistica|search|cerca|home|rete|network|lavoro|jobs|me|tu|profilo|premium)$/i.test(
                  t,
                )
              )
                return true;
            ...
            const h1Candidates = Array.from(
              main.querySelectorAll("section.pv-top-card h1, h1.inline.t-24, h1.text-heading-xlarge, h1"),
            );
            for (let i = 0; i < h1Candidates.length; i++) {
              const t = (h1Candidates[i].textContent || "").trim().replace(/\s+/g, " ");
              if (!isNavNoise(t) && t.length >= 2 && t.length <= 120) {
                result.name = t;
                break;
              }
            }
            ...
            const allBtns = Array.from(document.querySelectorAll("button")).filter(function (b) {
              return b.offsetParent !== null;
            });
            for (let i = 0; i < allBtns.length; i++) {
              const t = allBtns[i].textContent.trim().toLowerCase();
              if (/^(connect|collegati|connetti)$/.test(t)) {
                result.connectionStatus = "not_connected";
                break;
              }
              if (/^(messag|scrivi)/.test(t)) {
                result.connectionStatus = "connected";
                break;
              }
              if (/^(pending|in attesa)/.test(t)) {
        ...
        console.warn("[LI-Hybrid] Structural fallback failed:", e.message);
      }
      ...
      try {
        ...
        if (!/linkedin\.com\/(in|pub|messaging)\//i.test(currentUrl)) {
          return Config.errorResponse(
            ...
            "navigation_drifted: tab fuori da profilo/messaging (" + currentUrl + ")",
          );
        }
      } catch (e) {
          ...
          func: function (msg, aiSendSelector) {
            ...
            function deepQueryAll(selector, root) {
              ...
              for (const el of all) {
                if (el.shadowRoot) {
                  try {
                    out.push.apply(out, deepQueryAll(selector, el.shadowRoot));
                  } catch (e) {}
                }
              }
              return out;
            ...
            return (async function () {
              ...
              for (let i = 0; i < 40; i++) {
                if (
                  document.querySelector(
                    ".msg-form, [class*='msg-form'], .msg-thread, [class*='msg-thread'], [class*='msg-convo']",
                  )
                )
                  break;
                await sleep(500);
              ...
              if (!msgBox) {
                ...
                const probe = {
                  ...
                  shadowHosts: Array.from(document.querySelectorAll("*")).filter(function (e) {
                    return !!e.shadowRoot;
                  }).length,
                  iframes: document.querySelectorAll("iframe").length,
                  msgFormContainers: document.querySelectorAll(
                    ".msg-form, [class*='msg-form'], [class*='msg-thread'], [class*='msg-convo']",
                  ).length,
                  readyState: document.readyState,
                  msgOverlay: document.querySelectorAll(".msg-overlay-conversation-bubble, [class*='msg-overlay']")
                    .length,
              ...
              try {
                ...
                var targetOverlay = msgBox.closest(
                ...
                for (var oi = 0; oi < overlays.length; oi++) {
                  ...
                  var closeBtn = ov.querySelector(
                    "[aria-label*='Chiudi'], [aria-label*='Close'], [aria-label*='chiudi'], [aria-label*='close'], button.msg-overlay-bubble-header__control",
                  );
                  if (closeBtn) {
                    try {
                      closeBtn.click();
              ...
              function findSendBtn() {
                ...
                if (aiSendSelector && typeof aiSendSelector === "string") {
                  try {
                    ...
                    for (var ai = 0; ai < aiNodes.length; ai++) {
                      ...
                      if (
                        !bb.closest(".msg-form, [class*='msg-form'], [role='dialog'], .msg-overlay-conversation-bubble")
                      )
                        continue;
                    ...
                    /* selector invalido → fallback regex */
                  }
                }
                var scopes = document.querySelectorAll(
                ...
                for (var s = 0; s < scopes.length; s++) {
                  ...
                  for (var i = 0; i < btns.length; i++) {
                    ...
                    if (/msg-form__send-button|msg-form__send|send-button/i.test(cls)) {
                      b.__matchedBy = "regex_class";
                      return b;
                    }
                    if (/^(send|invia|invia messaggio|send message)$/i.test(al)) {
                      b.__matchedBy = "regex_aria";
                      return b;
                    }
                    if (/^(send|invia)$/i.test(t)) {
                      b.__matchedBy = "regex_text";
                      return b;
                    }
                    if (typ === "submit" && /msg-form/i.test(scope.className || "")) {
                      b.__matchedBy = "regex_submit";
                      return b;
                    }
                  }
                }
                ...
                try {
                  ...
                  try {
                    el.dispatchEvent(
                      new PointerEvent(
                        "pointerover",
                        Object.assign({ pointerType: "mouse", pointerId: 1, isPrimary: true }, opts),
                      ),
                    );
                  } catch (e) {}
              ...
              function submitComposer() {
                try {
                  var form =
                    msgBox.closest("form") ||
                    document.querySelector("form.msg-form, .msg-form form, [class*='msg-form'] form");
                  ...
                  var evt;
                  try {
                    evt = new SubmitEvent("submit", {
                      bubbles: true,
                      cancelable: true,
                      submitter: findSendBtn() || undefined,
                    });
                  } catch (e) {
        ...
        if (
          ...
          try {
            ...
            if (fresh && fresh.sendButtonSelector) {
              ...
              const retryRes2 = await chrome.scripting
                .executeScript({
                  target: { tabId: tabId },
                  func: function (msg, sel) {
                    ...
                    return (async function () {
                      ...
                      if (!current || current.indexOf((msg || "").trim().slice(0, 20)) < 0) {
                        return {
                          success: true,
                          method: "skipped_already_sent",
                          verified: false,
                          relearned: true,
                          warning: "composer_empty_skipping_retry",
                        };
                      }
                      var btn = null;
                      for (var i = 0; i < 30; i++) {
                      ...
                      try {
                        ...
                        return { success: false, error: "send_button_click_dispatch_failed", relearned: true };
                      }
                      // textboxCleared
                      var clearedOk = false;
                      for (var j = 0; j < 15; j++) {
            ...
            /* fall through con fbResult originale */
          }
        }
        ...
        if (fbResult) {
          ...
          if (fbResult.success === true && fbResult.verified === false) {
            ...
            if (!fbResult.suggestion) {
              fbResult.suggestion =
                "manual_retry: textbox non svuotato dopo click. Cache AI invalidata: prossimo invio rilearn dello schema. Verifica visivamente prima di ritentare.";
            }
          }
        }
    ...
    async function clickConnect(tabId) {
      ...
      try {
        ...
        console.warn("[LI-Hybrid] AX Tree connect failed:", e.message);
      }
      ...
      try {
        const fbRes = await chrome.scripting.executeScript({
          ...
          func: function () {
            ...
            if (btn) {
              btn.click();
              return { success: true, method: "structural_fallback" };
            }
            ...
            if (moreBtn) {
              ...
              return new Promise(function (resolve) {
                setTimeout(function () {
                  const dropItem = Array.from(
                    document.querySelectorAll("[role='option'], [role='menuitem'], li, span"),
                  ).find(function (el) {
                    return /connect|collegati|connetti/i.test(el.textContent.trim()) && el.offsetParent !== null;
                  });
                  if (dropItem) {
                    dropItem.click();
                    resolve({ success: true, method: "structural_more_dropdown" });
        ...
        return (
          (fbRes[0] && fbRes[0].result) ||
          Config.errorResponse(Config.ERROR.CONNECT_FAILED, "All connect strategies failed")
        );
      } catch (e) {
          ...
          func: function () {
            ...
            return (async function () {
              ...
              for (var es = 0; es < existingScopes.length; es++) {
                ...
                var existingVisible =
                  existingComposer.offsetParent !== null || existingComposer.getClientRects().length > 0;
                if (!existingVisible) continue;
                if (!existingComposer.querySelector("[contenteditable='true'], div[role='textbox'], [role='textbox']"))
                  continue;
              ...
              if (btn) {
                btn.click();
                return { success: true, method: "structural_fallback_main" };
              }
              ...
              if (more) {
                ...
                btn =
                  findProfileMessageBtn(document) ||
                ...
                if (btn) {
                  btn.click();
                  return { success: true, method: "structural_more_dropdown" };
                }
              }
          ...
          func: function (note) {
            ...
            return new Promise(function (resolve) {
              setTimeout(function () {
                ...
                if (!textarea) {
                  resolve({ success: false, error: "Note textarea not found" });
                  return;
                }
                ...
                setTimeout(function () {
                  ...
                  if (sendBtn) {
                    sendBtn.click();
                    resolve({ success: true, method: "structural_fallback" });
      ...
      try {
            ...
            func: function (msg, methodName) {
              ...
              return (async function () {
                ...
                if (!expectedText || (writtenText.indexOf(expectedText) === -1 && writtenText !== expectedText)) {
                  return {
                    success: false,
                    error: "text_not_written",
                    attempted_method: methodName,
                    wrote_length: writtenText.length,
                  };
                }
                ...
                if (methodName === "physical_click") {
                  ...
                  try {
                    ...
                    return {
                      success: false,
                      error: "physical_click_threw: " + e.message,
                      attempted_method: methodName,
                    };
                  }
                  ...
                  var form =
                    msgBox.closest("form") ||
                    document.querySelector(".msg-form, [class*='msg-form'] form, form.msg-form");
                  ...
                  try {
                    ...
                    return {
                      success: false,
                      error: "keyboard_shortcut_threw: " + e.message,
                      attempted_method: methodName,
                    };
                  }
        ...
        if (fbResult && fbResult.pending_cdp && fbResult.attempted_method === "cdp_physical_click") {
          ...
          if (cdpClick && cdpClick.success && (await composerCleared(tabId, 1500)))
            return { success: true, method: "cdp_physical_click" };
          return {
            success: false,
            error: (cdpClick && cdpClick.error) || "cdp_physical_click_failed",
            attempted_method: "cdp_physical_click",
          };
        }
        if (fbResult && fbResult.pending_cdp && fbResult.attempted_method === "cdp_ctrl_enter") {
          ...
          if (cdpKey && cdpKey.success && (await composerCleared(tabId, 1500)))
            return { success: true, method: cdpKey.method || "cdp_ctrl_enter" };
