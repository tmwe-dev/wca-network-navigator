  ...
  (function () {
    ...
    function urlMatchesTarget(tabUrl, targetUrl) {
      ...
      try {
        ...
        console.debug("[LI Tab]", err?.message);
        return false;
      }
    }
          ...
          try {
            ...
            if (t && /linkedin\.com/i.test(t.url || t.pendingUrl || "")) {
              ...
              if (targetUrl && !urlMatchesTarget(t.url || "", wantUrl)) {
                try {
                  await chrome.tabs.update(_workerTabId, { url: wantUrl });
                } catch (e) {
                  /* ignore */
                }
    ...
    async function getLinkedInTab(url, skipNavigateIfSameDomain, allowCreate) {
      ...
      if (url && /linkedin\.com\/(messaging|in\/|pub\/)/i.test(url)) {
        try {
          ...
          console.warn("[LI Tab] worker resolver failed:", e?.message);
        }
      }
      ...
      try {
        ...
        const userTab =
          userTabs &&
          userTabs.find(function (t) {
            return t.windowId !== _automationWindowId;
          });
        if (userTab) {
          _liTabId = userTab.id;
          markOwned(_liTabId);
      ...
      try {
        if (_automationWindowId !== null) {
          const win = await chrome.windows.get(_automationWindowId).catch(function () {
            return null;
          });
          if (win) {
    ...
    async function getLinkedInTabForRead(url) {
      ...
      if (isMessaging) {
        try {
          ...
          console.warn("[LI Tab][READ] worker failed:", e?.message);
        }
      }
    ...
    async function activateAndStabilize(tabId, maxWaitMs) {
      ...
      try {
        ...
        console.debug("[LI Tab] V2.1 activate:", err?.message);
      }
      ...
      while (Date.now() - startTime < maxWait) {
        try {
          ...
          if (r && r.ready && r.hasContent) {
            stable = true;
            break;
          }
        } catch (err) {
          console.debug("[LI Tab] V2.1 probe:", err?.message);
        }
        await sleep(300);
      }
