var OptimusClient =
  ...
  (function () {
      ...
      elements.forEach(function (el) {
        ...
        fields.forEach(function (field) {
          if (sel[field] && sel[field].primary) {
            ...
            if (!found && sel[field].fallback) found = el.querySelector(sel[field].fallback);
            if (found) {
    ...
    return { requestPlan: requestPlan, executePlan: executePlan, simplifyDom: simplifyDom, computeHash: computeHash };
  })();
  ...
  (function () {
    ...
    function _pageSimplifyAndHash(rootSelector, fallbackUseBody, maxDepth, maxLength) {
      ...
      return new Promise(async function (resolve) {
        const root = pickRoot();
        if (!root) {
          resolve({ ok: false, error: "root_not_found" });
          return;
        }
    ...
    async function _findAppTab() {
      try {
        ...
        console.debug("[LI Optimus] plan:", err?.message);
      }
      return null;
    }
    ...
    async function getPlan(params) {
      ...
      return new Promise(async function (resolve) {
        ...
        try {
          ...
          resolve({
            success: false,
            error: "dispatch_failed: " + (e && e.message ? e.message : String(e)),
    ...
    function _pageExecutePlan(rootSelector, fallbackBody, plan) {
      function trySel(root, sel) {
        ...
        try {
          ...
          console.debug("[LI Optimus] sel:", err?.message);
          return null;
        }
      }
      function tryAll(root, sel) {
        ...
        try {
          ...
          console.debug("[LI Optimus] selAll:", err?.message);
          return [];
        }
      }
      ...
      const fieldKeys = Object.keys(sels).filter(function (k) {
      ...
      for (const itemEl of itemEls) {
        ...
        try {
          if (!obj.thread_url) {
            var anchor =
              itemEl.querySelector("a[href*='/messaging/thread/']") || itemEl.querySelector("a[href*='/messaging/']");
