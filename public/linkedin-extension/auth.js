  ...
  (function () {
      ...
      try {
        ...
        if (typeof AiBridge === "undefined" || !AiBridge.liCookieRequest) {
          ...
          try {
            ...
            return Config.errorResponse(
              Config.ERROR.NO_CONFIG,
              "Bridge webapp non disponibile e fallback storage fallito",
            );
          }
        }
        ...
        if (!bridgeResp || bridgeResp.success === false) {
          return Config.errorResponse(
            ...
            (bridgeResp && bridgeResp.error) || "Bridge cookie sync failed",
          );
        }
      ...
      try {
        const results = await chrome.scripting.executeScript({
          ...
          func: function () {
            ...
            const authenticated =
              (onLoggedPage || hasMainContent) && !loginPage && !hasSignIn && !isCheckpoint && !hasLoginForm;
    ...
    function inspectPage() {
      try {
        ...
        const hasTwoStep = !!document.querySelector(
          "input[name='verificationCode'], input[name='otpPin'], input[autocomplete='one-time-code']",
        );
        const googleButton = Array.from(document.querySelectorAll("a, button")).find(function (el) {
          return (
            /continue with google|continua con google|sign in with google|accedi con google/i.test(
              el.textContent || "",
        ...
        return {
          ...
          isChallenge:
            /checkpoint|challenge|security-verification|two-step|verify|captcha/i.test(url) ||
            hasCaptcha ||
            hasPhoneVerify ||
            hasTwoStep,
        };
    ...
    function prepareLoginPage() {
      try {
        ...
        const signInLink = Array.from(document.querySelectorAll("a, button")).find(function (el) {
          ...
          return (
            /^sign in$|^log in$|^accedi$/i.test(text) ||
            (el.tagName === "A" && /\/login/.test(el.getAttribute("href") || ""))
          );
        });
        if (signInLink) {
          signInLink.click();
    ...
    async function pollForAuthCompletion(tabId, timeoutMs, options) {
      ...
      return Config.errorResponse(Config.ERROR.TIMEOUT, sawChallenge ? "Timeout verifica manuale" : "Timeout login");
    }
    ...
    async function autoLogin() {
      ...
      if (existingCookie) {
        ...
        if (sessionCheck.authenticated) {
          ...
          return Config.successResponse({
            ...
            cookieSynced: !!syncAlready.success,
          });
        }
      }
      ...
      if (!credsBridge || credsBridge.success === false) {
        return Config.errorResponse(
          Config.ERROR.LOGIN_FAILED,
          (credsBridge && credsBridge.error) || "Lettura credenziali fallita",
        );
      }
      const creds = credsBridge.data || {};
      if (!creds.email || !creds.password)
        return Config.errorResponse(Config.ERROR.LOGIN_FAILED, "Credenziali LinkedIn non configurate");
      ...
      try {
        ...
        if (!prep || !prep.success)
          return Config.errorResponse(Config.ERROR.LOGIN_FAILED, "Preparazione login fallita");
        ...
        if (!readyState || !readyState.hasLoginForm)
          return Config.errorResponse(Config.ERROR.LOGIN_FAILED, "Form di login non trovato");
        ...
        if (!formResult || !formResult.success)
          return Config.errorResponse(Config.ERROR.LOGIN_FAILED, (formResult && formResult.error) || "Fill fallito");
