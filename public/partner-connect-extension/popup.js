...
$("scrapeDownload")?.addEventListener("click", () => {
  ...
  const title = (scrapeData.metadata.title || "scrape")
...
$("crawlDownload")?.addEventListener("click", () =>
  downloadFile(JSON.stringify(crawlData, null, 2), "crawl-results.json", "application/json"),
);
...
$("mapBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (mapResultsEl) {
      mapResultsEl.innerHTML = mapData
        .map(
          (r) =>
...
$("mapDownload")?.addEventListener("click", () =>
  downloadFile(JSON.stringify(mapData, null, 2), "sitemap.json", "application/json"),
);
...
$("addFieldBtn")?.addEventListener("click", () => {
  ...
  row.innerHTML =
    '<input class="input" placeholder="campo" style="flex:1"><input class="input" placeholder="selettore" style="flex:2"><button class="remove-field">\u00d7</button>';
...
$("extractDownload")?.addEventListener("click", () =>
  downloadFile(JSON.stringify(extractData, null, 2), "extracted-data.json", "application/json"),
);
...
$("batchBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (batchResultsEl) {
      batchResultsEl.innerHTML = batchData
        .map(
          (r) =>
            `<div class="result-item"><span class="badge ${r.error ? "badge-err" : "badge-ok"}">${r.error ? "ERR" : "OK"}</span><span class="url" title="${esc(r.url)}">${esc(r.url)}</span></div>`,
        )
...
$("batchDownload")?.addEventListener("click", () =>
  downloadFile(JSON.stringify(batchData, null, 2), "batch-results.json", "application/json"),
);
...
$("brainAnalyzeBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (brainAnalysis.next_actions?.length > 0) {
      ...
      if (actionsListEl) {
        actionsListEl.innerHTML = brainAnalysis.next_actions
          .map(
            (a) =>
    ...
    setStatus("brainStatus", "\u274c " + e.message, "error");
  }
  setLoading(btn, false);
});
...
$("brainRunActionsBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(
      "brainStatus",
      result.ok ? `\u2705 ${result.totalSteps} azioni eseguite` : `\u274c Fermato: ${result.reason}`,
...
$("brainSaveSettings")?.addEventListener("click", async () => {
  try {
    ...
    setStatus("brainSettingsStatus", "\u274c " + e.message, "error");
  }
});
...
$("agentActionType")?.addEventListener("change", () => {
  ...
  if (needsText) show($("agentTextField"));
  else hide($("agentTextField"));
});
...
$("agentRunBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(
      "agentStatus",
      result.ok ? "\u2705 Azione completata" : "\u274c " + (result.error || "Errore"),
      result.ok ? "success" : "error",
    );
  } catch (e) {
    setStatus("agentStatus", "\u274c " + e.message, "error");
...
$("agentSeqBtn")?.addEventListener("click", async () => {
  ...
  let steps;
  try {
    steps = JSON.parse($("agentSequence")?.value);
  } catch {
    return setStatus("agentSeqStatus", "\u274c JSON non valido", "error");
  }
  ...
  try {
    ...
    setStatus(
      "agentSeqStatus",
      result.ok
        ? `\u2705 ${result.totalSteps} passi completati`
        : `\u274c Fermato al passo ${result.stoppedAt}: ${result.reason}`,
      result.ok ? "success" : "error",
    );
  } catch (e) {
    setStatus("agentSeqStatus", "\u274c " + e.message, "error");
  }
  setLoading(btn, false);
});
...
$("relayStartBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus("relayStatusMsg", "\u274c " + e.message, "error");
  }
  setLoading(btn, false);
});
  ...
  try {
    ...
    if (logContainer) {
      if (s.log && s.log.length > 0) {
        hide($("relayLogEmpty"));
        logContainer.innerHTML = s.log
          .map((entry) => {
            const time = new Date(entry.ts).toLocaleTimeString();
            let icon = "\ud83d\udccb";
            let text = "";
            if (entry.type === "command-in") {
              icon = "\ud83d\udce5";
              text = esc(entry.command?.type || "?");
            } else if (entry.type === "command-out") {
              icon = entry.result?.ok ? "\u2705" : "\u274c";
              text = "risultato";
            } else if (entry.type === "command-rejected") {
              icon = "\ud83d\udeab";
              text = esc(entry.reason || "?");
            } else if (entry.type === "agent-action") {
              icon = "\ud83e\udd16";
              text = esc(entry.step?.action || "?");
            } else if (entry.type === "relay") {
              icon = "\ud83d\udd0c";
              text = esc(entry.event);
            } else if (entry.type === "circuit-breaker") {
              icon = "\u26a1";
              text = esc(entry.event);
            } else if (entry.type === "error") {
              icon = "\u26a0";
              text = esc(entry.message?.slice(0, 40));
            }
  ...
  for (const [domain, data] of Object.entries(stats)) {
    ...
    if (isBlocked) {
      ...
      let reason = "",
        countdown = "";
      if (isDailyBlocked) {
        reason = `Limite giornaliero raggiunto (${dMax}/giorno)`;
        countdown = "reset a mezzanotte";
      } else if (isHourlyBlocked) {
        reason = `Limite orario raggiunto (${hMax}/h)`;
        countdown = "~attesa 1h";
      } else {
        reason = `Cooldown burst (${burstThreshold} consecutive)`;
        countdown = "~5 min";
      }
      blockedList.innerHTML += `
    ...
    if (taskList) {
      if (list.length > 0) {
        ...
        taskList.innerHTML = list
          .map((t) => {
            const statusIcon =
              { running: "🔄", completed: "✅", failed: "❌", paused: "⏸", created: "📝", cancelled: "🚫" }[t.status] ||
              "❓";
...
window.pauseTask = async (id) => {
  await sendMsg({ action: "task-pause", taskId: id });
  refreshTasks();
};
window.resumeTask = async (id) => {
  await sendMsg({ action: "task-start", taskId: id });
  refreshTasks();
};
window.retryTask = async (id) => {
  await sendMsg({ action: "task-retry", taskId: id });
  refreshTasks();
};
...
$("taskCreateBtn")?.addEventListener("click", async () => {
  ...
  let steps;
  try {
    steps = JSON.parse($("taskStepsJson")?.value || "[]");
  } catch {
    return setStatus("taskStatus", "❌ JSON non valido", "error");
  }
  if (!Array.isArray(steps) || steps.length === 0)
    return setStatus("taskStatus", "❌ Inserisci almeno uno step", "error");
  ...
  try {
    ...
    setStatus("taskStatus", "❌ " + e.message, "error");
  }
});
  ...
  try {
    ...
    for (const f of fields) {
      if (f.type === "password") {
        ...
        if (el) {
          el.value = "";
          el.placeholder = "•••• salvata";
        }
      }
    }
  } catch (e) {
    setStatus("connectorStatus", "❌ " + e.message, "error");
  }
});
...
$("connectorTestBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(
      "connectorStatus",
      result?.ok ? "✅ Connessione OK" : "❌ Test fallito",
      result?.ok ? "success" : "error",
    );
  } catch (e) {
    setStatus("connectorStatus", "❌ " + e.message, "error");
...
window.loadTemplate = async (tplId) => {
  try {
    ...
    setStatus("pipelineStatus", "❌ " + e.message, "error");
  }
};
  ...
  try {
    ...
    if (selected.startsWith("tpl:")) {
      ...
      result = await sendMsg({
        ...
        pipelineId: saved?.id || saved?.pipelineId,
        variables: vars,
      });
    } else {
...
$("pipelineSaveBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    return setStatus("pipelineStatus", "❌ JSON non valido", "error");
  }
...
$("canvasTableBtn")?.addEventListener("click", async () => {
  try {
    ...
    canvasAsk(
      `Crea una tabella HTML professionale con i dati principali estratti da questa pagina. Contenuto: ${(scrapeData?.markdown || "").slice(0, 2000)}`,
    );
  } catch {
...
$("canvasChartBtn")?.addEventListener("click", () => {
  canvasAsk(
    "Crea un grafico a barre SVG con i dati disponibili dalla pagina corrente. Se non ci sono dati sufficienti, crea un grafico di esempio su volumi di spedizione per mese.",
  );
});
...
$("canvasSummaryBtn")?.addEventListener("click", async () => {
  try {
    const scrapeData = await sendMsg({ action: "scrape" });
    canvasAsk(
      `Crea un report HTML visuale e professionale riassumendo i punti chiave di questa pagina. Contenuto: ${(scrapeData?.markdown || "").slice(0, 2000)}`,
    );
  } catch {
    canvasAsk(
      "Crea un report HTML di esempio per un'azienda logistica con sezioni: Overview, Servizi, Contatti, Prossimi passi.",
    );
  }
});
...
$("canvasClearBtn")?.addEventListener("click", () => {
  ...
  if (content)
    content.innerHTML =
      '<div style="text-align:center;color:#999;padding:40px 20px;"><div style="font-size:32px;margin-bottom:8px;">🎨</div><div>Canvas pulito. Scrivi un prompt per generare contenuti.</div></div>';
  canvasHtml = "";
...
$("canvasCopyBtn")?.addEventListener("click", () => {
  if (canvasHtml) {
    ...
    setTimeout(() => {
      if ($("canvasCopyBtn")) $("canvasCopyBtn").innerHTML = "📋 Copia HTML";
    }, 1200);
  }
});
...
async function elRefreshVoices(query) {
  ...
  try {
    ...
    if (!voices?.length) {
      list.innerHTML =
        '<div style="text-align:center;color:#555;padding:12px;font-size:11px;">Nessuna voce trovata. Inserisci la API key e riprova.</div>';
...
$("elSpeakBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (result.audioBase64) {
      ...
      setStatus(
        "elTtsStatus",
        `✅ ${result.size ? Math.round(result.size / 1024) + "KB" : "Audio generato"}`,
        "success",
      );
    }
  } catch (e) {
...
async function elRefreshAgents() {
  ...
  try {
    ...
    if (!agents?.length) {
      list.innerHTML =
        '<div style="text-align:center;color:#555;padding:8px;font-size:11px;">Nessun agente configurato</div>';
      return;
    }
