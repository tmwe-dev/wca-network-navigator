// FireScrape v3.2 — ElevenLabs Integration Module
...
const ElevenLabs = (() => {
  ...
  async function _fetch(path, options = {}) {
    ...
    if (!resp.ok) {
      ...
      try {
        errMsg = JSON.parse(errBody)?.detail?.message || errMsg;
      } catch {}
  ...
  async function init() {
    ...
    const stored = await chrome.storage.local.get([
      "fs_elevenlabs_config",
      "fs_elevenlabs_voices",
      "fs_elevenlabs_agents",
    ]);
  ...
  async function getVoicesByLanguage(lang) {
    ...
    return _voices.filter((v) => v.language.toLowerCase().includes(l) || v.labels?.accent?.toLowerCase().includes(l));
  }
  ...
  async function speak(text, options = {}) {
    const voiceId = options.voiceId || _config.defaultVoiceId;
    if (!voiceId) throw new Error("Nessuna voce selezionata. Configura defaultVoiceId.");
    ...
    const resp = await _fetch(`/text-to-speech/${voiceId}?output_format=${format}`, { method: "POST", json: payload });
  ...
  async function speakStream(text, options = {}) {
    const voiceId = options.voiceId || _config.defaultVoiceId;
    if (!voiceId) throw new Error("Nessuna voce selezionata");
    ...
    const format = options.format || _config.outputFormat;
    const resp = await _fetch(`/text-to-speech/${voiceId}/stream?output_format=${format}`, {
      method: "POST",
      json: payload,
    });
  ...
  async function createAgent(agentConfig) {
    const payload = {
      name: agentConfig.name || "FireScrape Agent",
      conversation_config: {
        tts: {
        ...
        agent: {
          prompt: {
            prompt:
              agentConfig.systemPrompt || "Sei un assistente vocale per FireScrape, uno strumento di web intelligence.",
          },
  ...
  async function speakPageSummary(options = {}) {
    ...
    const brainResult = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(
        {
          action: "brain-think",
          prompt: options.prompt || "Riassumi il contenuto principale di questa pagina in 2-3 frasi brevi in italiano.",
        },
