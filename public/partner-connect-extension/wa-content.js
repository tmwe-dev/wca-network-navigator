...
(function () {
  ...
  function isLoggedIn() {
    const sidePanel =
      querySafe('[data-testid="chat-list"]') ||
      querySafe("#pane-side") ||
  ...
  function readSidebar() {
    ...
    for (const item of items) {
      try {
        ...
        const timeEl =
          item.querySelector('[data-testid="cell-frame-primary-detail"]') || item.querySelector("div._ak8i");
  ...
  function readThreadMessages(maxMessages) {
    ...
    for (const item of slice) {
      try {
        ...
        const textEl =
          item.querySelector('[data-testid="balloon-text"] span') ||
          item.querySelector(".selectable-text span") ||
          item.querySelector('[dir="ltr"]');
        const text = textEl?.textContent?.trim() || "";

        const timeEl =
          item.querySelector('[data-testid="msg-meta"] span') ||
  ...
  async function openChat(contactName) {
    const searchBox =
      querySafe('[data-testid="chat-list-search"]') ||
  ...
  async function sendMessage(contactOrPhone, text) {
    ...
    // Type the message
    const inputBox =
      querySafe('[data-testid="conversation-compose-box-input"]') ||
    ...
    try {
      const resp = await fetch(`${supabaseConfig.supabaseUrl}/functions/v1/extension-brain`, {
        ...
        body: JSON.stringify({
          ...
          prompt:
            "Extract all visible chat conversations from this WhatsApp Web sidebar HTML. Return a JSON array where each item has: contact (string), lastMessage (string), time (string), unreadCount (number). Return ONLY the JSON array.",
          data: truncated,
        }),
      });
  ...
  function captureDomSnapshot() {
    ...
    for (const el of elements) {
      ...
      const classes =
        el.className && typeof el.className === "string" ? el.className.split(" ").slice(0, 5).join(" ") : "";
      const id = el.id || "";
    ...
    try {
      const resp = await fetch(`${supabaseConfig.supabaseUrl}/functions/v1/extension-brain`, {
        ...
        body: JSON.stringify({
          ...
          prompt:
            "Analyze this WhatsApp Web DOM snapshot and return a JSON object with CSS selectors for: sidebar, chatList, messageBubble, inputBox, sendButton, unreadBadge, contactName, lastMessage, timestamp, msgText, msgTime, msgOutgoing, searchBox. Return ONLY valid JSON.",
          data: domSnapshot,
        }),
      });
      ...
      try {
        ...
        schema = m ? JSON.parse(m[1] || m[0]) : null;
      }
